...
- The `Thread` value keeps the completed result after the thread ends. Field access `t.result` waits until completion and returns the stored `Result OF Out`. `thread::waitFor(t)` waits until completion and returns `t.result`, auto-unwrapping or auto-propagating like any other function call.
