...
When a thread ends, its `Thread` value keeps `result AS Result OF Out`. Field access `t.result` waits for the completed result and returns the raw `Result OF Out`. `thread::waitFor(t)` waits for that result to exist and returns it with normal `Result` auto-unwrapping behavior.
