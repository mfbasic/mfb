...
| `thread::transfer` | `FUNC transfer OF Res, Out(t AS Thread OF Res TO Out, res AS RES Res, timeoutMs AS Integer = 0) AS Nothing` | Resource plane: **move** a thread-sendable resource `res` to the thread. Consumes the sender binding; ownership returns to the sender on failure. `Res` must be a resource (`TYPE_THREAD_NOT_SENDABLE` otherwise). |
| `thread::accept` | `FUNC accept OF Res, Out(t AS Thread OF Res TO Out, timeoutMs AS Integer = 0) AS RES Res` | Resource plane: receive a transferred resource and bind it with `RES`. |
| `thread::transfer` | `FUNC transfer OF Res, Out(t AS ThreadWorker OF Res TO Out, res AS RES Res, timeoutMs AS Integer = 0) AS Nothing` | Worker-side resource transfer to the parent-visible plane. |
| `thread::accept` | `FUNC accept OF Res, Out(t AS ThreadWorker OF Res TO Out, timeoutMs AS Integer = 0) AS RES Res` | Worker-side receive of a transferred resource. |
