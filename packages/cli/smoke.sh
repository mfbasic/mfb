...
printf '%s\n' 'IMPORT cli' 'IMPORT io' '' 'FUNC main() AS Integer' '  LET options AS List OF cli::Option = [cli::Option[name := "port", alias := "p", required := TRUE, kind := cli::OptionKind.Integer], cli::Option[name := "enabled", alias := "b", required := FALSE, kind := cli::OptionKind.Bool], cli::Option[name := "title", alias := "t", required := FALSE, kind := cli::OptionKind.String], cli::Option[name := "quiet", alias := "", required := FALSE, kind := cli::OptionKind.Flag]]' '  cli::parse(options)' '  io::print(toString(cli::getInteger("port", 0)))' '  io::print(toString(cli::getBool("enabled", FALSE)))' '  cli::showUsage(["header"], options, ["footer"])' '  RETURN 0' 'END FUNC' > "$WORK/src/main.mfb"
"$MFB" build -q "$WORK"
BIN="$WORK/build/cli_smoke.out"
OUTPUT="$("$BIN" -p 3000 -b true)"
[[ "$OUTPUT" == *$'3000\nTRUE\nheader\nUsage: '* ]]
[[ "$OUTPUT" == *$'  -p, --port <integer> (required)'* ]]
[[ "$OUTPUT" == *$'  -b, --enabled <bool>'* ]]
[[ "$OUTPUT" == *$'  -t, --title <string>'* ]]
[[ "$OUTPUT" == *$'      --quiet'*$'\nfooter' ]]
[[ "$("$BIN" -p 3000 -b 0)" == $'3000\nFALSE' ]]
set +e
"$BIN" -p >/dev/null 2>&1
