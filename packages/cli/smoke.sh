...
printf '%s\n' 'IMPORT cli' 'IMPORT io' '' 'FUNC main() AS Integer' '  LET options AS List OF cli::Option = [cli::Option[name := "port", alias := "p", required := TRUE, kind := cli::OptionKind.Integer], cli::Option[name := "enabled", alias := "b", required := FALSE, kind := cli::OptionKind.Bool], cli::Option[name := "quiet", alias := "", required := FALSE, kind := cli::OptionKind.Flag]]' '  cli::parse(options)' '  io::print(toString(cli::getInteger("port", 0)))' '  io::print(toString(cli::getBool("enabled", FALSE)))' '  cli::showUsage(["header"], options, ["footer"])' '  RETURN 0' 'END FUNC' > "$WORK/src/main.mfb"
"$MFB" build -q "$WORK"
