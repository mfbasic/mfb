...
/// Every test belongs to a category directory — `tests/cli/`, `tests/runtime/`,
/// `tests/codegen/`, … — and none sits loose at the top of `tests/`.
///
/// This is the one rule here that is about organisation rather than about a test
/// silently not running, and it is enforced for the same reason: a convention
/// that lives only in a reviewer's memory decays. A file dropped at
/// `tests/foo.rs` WOULD run (Cargo auto-discovers it), so nothing else in this
/// file would object — the loss is that the tree goes back to being a flat pile.
#[test]
fn no_test_file_sits_directly_in_tests() {
    let loose: Vec<String> = std::fs::read_dir(repo_root().join("tests"))
        .expect("read tests/")
        .flatten()
        .map(|e| e.path())
    ...
    assert!(
        loose.is_empty(),
        "{} test file(s) sit directly in tests/ instead of in a category \
         directory: {}\nMove each into the directory that matches what it \
         checks, then add its [[test]] stanza (this file's other tests print \
         the TOML).",
        loose.len(),
        loose.join(", ")
    );
