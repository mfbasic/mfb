...
//!
//! It lives in this crate, not in `mfb`, because the registry client is one of
//! its callers and `mfb_repository` cannot depend on `mfb` (bug-489). `mfb`
//! re-exports it from `src/terminal_safe.rs`, so there is exactly one
