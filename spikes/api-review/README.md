...
| `bug-519-parse-normalizes` | does `datetime::parse` reject month 13? | no, and it does not "carry it into the result" either: `2026-13-45 25:70:99` silently becomes `2027-02-15T02:11:39Z`, while `datetime::date(2026,13,45)` raises 77050002 |
| `bug-521-toiso-nanos` | is `toIso` → `parseIso` the round trip its man page promises? | not for sub-millisecond values: 123456789 ns in, 123000000 ns out, 456789 ns lost |
