...
| `bug-518-withzone-doc` | does `withZone` move the instant, as its `zone` parameter row says? | no. instant 1782475200 before and after — the Description was right, the parameter row was wrong. **Fixed (bug-518):** the row now states the instant is preserved and names `datetime::civil(dt.date, dt.time, zone)` as the opposite operation |
...
| `bug-522-transfer-list-stale` | may a `tcp::Listener` cross a thread, as the package intro says and `thread::transfer` denies? | yes — it compiles, transfers and the worker returns. The `transfer` page's list is stale. **Fixed (bug-522):** `transfer` and `accept` now state the same six as the intro, all three also name the five that may NOT cross, and a test asserts every `sendable` bit against an explicit table |
| `bug-523-res-shapes-undocumented` | can a `RES` handle be a record field and a collection element? | yes to both, and no resource type page mentions either — every one of the 11 is a single sentence about scope-exit closing. **Fixed (bug-523):** each `types` page states the two uniform shapes once and derives thread-transferability from the `sendable` bit; `mfb man variable` gained a runnable example of each; two pages that claimed a record field was forbidden are corrected |
| `bug-524-process-close` | does `process::close` close its handle, like every other `close` in the language? | no. `isRunning` is still TRUE, `receive` and `pid` still work — so the member was renamed `process::closeInput` (bug-524, fixed) |
| `bug-526-tls-poll-res` | is the signature `mfb man tls poll` prints a declaration that compiles? | no. `List OF tls::Socket` is `TYPE_RESOURCE_REQUIRES_RES`; `mfb man tcp poll` prints the same overload correctly. **Fixed (bug-526):** the descriptor carries the `RES` node the way `tcp::poll` does, and a registry-wide pin now fails any rendered signature whose collection element is an unmarked resource |
