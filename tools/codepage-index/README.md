...
(`python3 scripts/audit_codepage_index.py`).

## Regenerating the compiler tables

`scripts/gen_codepage_tables.py` reads these files and writes
...
Re-fetch with `python3 scripts/fetch_codepage_index.py`, `git diff` these files to
