...
impl Platform {
    fn libc(&self) -> &'static str {
        match self.flavor {
            ...
            LinuxFlavor::Musl => "libc.musl-riscv64.so.1",
        }
    }
}
...
impl code::CodegenPlatform for Platform {
    ...
    fn emit_environ_pointer(
        ...
        // `environ` is an imported libc data global. On rv64 the `adrp`/`add`
        // pair lowers to `auipc`/`ld` (GOT), loading `&environ` from the GOT
        // slot; one further deref gives the live `char**`.
    ...
    fn emit_sync_file(
        ...
        // is a no-op here — kept for uniformity.
        Ok(())
    }
    ...
    fn o_nonblock(&self) -> &'static str {
        "2048" // O_NONBLOCK (0o4000 = 0x800) on Linux riscv64
    }
