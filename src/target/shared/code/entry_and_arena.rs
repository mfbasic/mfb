...
pub(crate) fn lower_program_entry(
    ...
    // plan-15 §4.5 compat shim: subscribe the main thread to the stdin broadcast
    // log at the current frontier (fill == 0 here), so a single-threaded program
    // sees the whole stdin stream and stays byte-identical. Runs after the
    // global/link initializers (so the log's lazy setup is single-threaded) and
    // before the entry call. Clobbers x0–x17, but the args materialization below
    // re-derives x0 from the frame.
    if subscribe_stdin {
