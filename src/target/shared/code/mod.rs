    ...
    let (frame, mut instructions, mut relocations, stack_slots) =
        if builtins::term::is_term_call(spec.call) {
            ...
            match spec.call {
                ...
                // `fs.*` is migrated onto the clean-room registry: every syscall
                // member lowers through the generic OS-seam dispatch, reaching
                // `fs::native::lower_fs_helper` by `platform.family()`.
                call if call.starts_with("fs.") => {
