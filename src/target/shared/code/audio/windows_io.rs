...
/// pass `inp`/`outp` in `%v9`+.
fn emit_s16_to_f32(inp: &str, outp: &str, tag: &str, ins: &mut Vec<CodeInstruction>) {
...
fn emit_f32_to_s16(inp: &str, outp: &str, tag: &str, ins: &mut Vec<CodeInstruction>) {
...
fn emit_write_fill(base_frame_off: usize, n_off: usize, ins: &mut Vec<CodeInstruction>) {
    let n = ins.len();
    ...
    emit_s16_to_f32("%v9", "%v10", "wfill", ins);
    ins.extend([
    ...
    emit_copy_bytes("%v12", "%v15", "%v14", &format!("wfill_dc_{n}"), ins);
...
fn lower_write(
    ...
    emit_write_fill(OFFSET_OFF, FRAMES_GOT_OFF, &mut ins);
...
fn emit_read_fill(got_off: usize, cf_off: usize, ins: &mut Vec<CodeInstruction>) {
    let n = ins.len();
    ...
    emit_f32_to_s16("%v9", "%v10", "rfill", ins);
    ins.extend([
    ...
    emit_copy_bytes("%v12", "%v15", "%v14", &format!("rfill_dc_{n}"), ins);
    ins.push(abi::label(&done));
}
