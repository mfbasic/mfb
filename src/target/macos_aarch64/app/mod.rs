...
//!
//! Implements the macOS app-mode runtime (see src/docs/spec/app/01_macos-runtime.md): emit the app-mode `_main`
...
/// `mfbDrawBox:` — the main-thread grid draw-box entry point for `term::drawBox`.
/// Like `mfbDrawLine:`, marshaled onto the main thread (bug-165); reads the six
/// box glyphs + two points the worker parked in the TermView state.
...
const TV_BOX_CTR_OFFSET: usize = 160; // u32 top-right corner
const TV_BOX_CBL_OFFSET: usize = 168; // u32 bottom-left corner
const TV_BOX_CBR_OFFSET: usize = 176; // u32 bottom-right corner
const TV_BOX_X1_OFFSET: usize = 184; // i64 point 1 column (raw)
