...
* **Resource linearity** (flow-sensitive) — use-after-move / double-close across branches (`TYPE_USE_AFTER_MOVE`, cross-branch `MaybeMoved` union; see `./mfb spec language memory-semantics` §14.9), the `RES`/`STATE` ownership axis (`TYPE_RESOURCE_REQUIRES_RES`, `TYPE_RES_REQUIRES_RESOURCE`, `TYPE_STATE_INVALID`, `TYPE_UNION_STATE_FORBIDDEN`), STATE-type agreement between a resource's owning binding and every parameter, return, or later binding that names it (`TYPE_STATE_MISMATCH` — an untagged payload re-typed through a disagreeing declaration is an ABI-level type confusion), opaque-`STATE` narrowing (`TYPE_STATE_OPAQUE_NARROWING` — a bare `RES` parameter's STATE is "some state or none" and cannot be re-declared concrete), and the ban on a thread/resource in a `Map` key.
...
* **Native `LINK` ABI** — C-type escape (`NATIVE_CPTR_ESCAPE`), ABI slot/param/CONST binding, `BIND STATE` well-formedness (`NATIVE_BIND_STATE_INVALID` — the named slot is an OUT CSTRUCT whose record is the resource's STATE type, on a function that returns that stateful resource), STATE-type agreement across a native resource's declarations (`TYPE_STATE_MISMATCH`), and result-marker consistency over the merged link table.
