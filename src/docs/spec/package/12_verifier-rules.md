...
* **Resource linearity** (flow-sensitive) — use-after-move / double-close across branches (`TYPE_USE_AFTER_MOVE`, cross-branch `MaybeMoved` union; see `./mfb spec language memory-semantics` §14.9), invalidation by a non-owner (`TYPE_RESOURCE_INVALIDATE_NOT_OWNER`), the `RES`/`STATE` ownership axis (`TYPE_RESOURCE_REQUIRES_RES`, `TYPE_RES_REQUIRES_RESOURCE`, `TYPE_STATE_INVALID`, `TYPE_UNION_STATE_FORBIDDEN`), STATE-type agreement between a resource's owning binding and every parameter, return, or later binding that names it (`TYPE_STATE_MISMATCH` — an untagged payload re-typed through a disagreeing declaration is an ABI-level type confusion), and collection-element ownership (`TYPE_RESOURCE_ELEMENT_NOT_OWNER`, thread/resource in a `Map` key).
