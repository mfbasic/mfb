...
The Binary Representation `version` is currently **`6`**. Version 2 added per-node source locations (`loc` on `Call`/`CallResult`/`Binary`/`Unary`/`For`) and a per-function source `file`, backing read-only `Error.source`/`ErrorLoc`. Version 3 extends spans to the full diagnostic vocabulary: every `IrOp` (statement line), every `IrMatchCase` (case-arm line), and the declaration nodes `IrFunction`/`IrParam`/`IrType`/`IrField`/`IrVariant`/`IrBinding` (declaration line) carry a trailing `loc` (`u32` line, `u32` column; column is `1` for statement/declaration spans). Version 4 adds the declaration-fidelity fields the relocated semantic rules need: a `Bind` op and a top-level `IrBinding` carry `explicit_type` (a `bool`: whether `type_` came from a source `AS T` annotation versus being inferred — only explicitly annotated bindings can mismatch their initializer), and an `IrType` and top-level `IrBinding` carry their declaring source `file` (a `str`, for cross-file visibility and declaration-site diagnostics). Concretely, the binding encoding appends `file` then `explicit_type` after the binding's `loc`; the type encoding appends `file` after the type's `loc`; a `Bind` op writes `explicit_type` immediately before its trailing `loc`. Version 4 also serializes each `IrFunction`'s `resource_owners` map (escape-analysis ownership decisions): the function encoding appends, after the function's `loc`, a `u32`-counted vector of `(name, ResOwner)` entries sorted by binding name, where `ResOwner` is a tag byte — `0` = `Local`, `1` = `Float` followed by an inline target-name string. A decoded package has no AST to re-run escape analysis on, so these decisions must ride the format for the package-path `RES`/ownership rules and resource codegen to stay correct. These fields let the IR-level semantic checker report at the same source line the source checker does and enforce every relocated rule on decoded package IR. Version 5 (plan-50-C/E) reshapes the native `LINK` trailer: an ABI slot's `isOut` bool becomes a three-valued `direction` byte so `INOUT` is expressible, `IrLinkExpr::Var` gains a name payload (it was a nameless "the native return"), and the `CSTRUCT` table and `BIND IN` bindings are added. Version 6 (plan-58-C) appends the `CBuffer` surface to each `LINK` function record: a `buffers` vector of `BUFFER <slot> SIZE <expr>` clauses and an optional `resultLength` (`RETURN … LENGTH <expr>`), plus three arithmetic `IrLinkExpr` opcodes (`Mul`/`Add`/`Sub`) so a buffer can be sized from an element count — see the native-bindings topic of this spec. The decoder rejects any payload whose first four bytes are not `MFBR`, or whose version is not exactly `6`. (This is independent of the MFPC container version, which is major `2`.) [[src/ir/binary.rs:BINARY_REPR_VERSION]] [[src/ir/binary.rs:encode_binding]] [[src/ir/binary.rs:encode_type]] [[src/ir/binary.rs:encode_function]] [[src/ir/binary.rs:decode_binary_repr]]
...
`IrValue` is encoded as a tree, one tag byte per kind: `Binary { op, left, right }`, `Call { target, args }`, `CallResult { … }`, `Checked { type, value }`, `ResultIsOk` / `ResultValue` / `ResultError`, `Constructor`, `MemberAccess`, `UnionWrap` / `UnionExtract`, literals, and identifiers. Value tags are **append-only**: `Checked` (bug-471, the inline-`TRAP` desugar's wrapper for a raising operator) took the next free tag the way `SetLiteral` did before it, so adding a kind does not bump `BINARY_REPR_VERSION` and every already-published package re-encodes byte-identically. There is no flattening into per-register temporaries. `CallResult` of a built-in is just an `IrValue::CallResult` node — there is no flat built-in dispatch, so the old "unknown function" emitter failure cannot occur, and an inline `TRAP` over a built-in serializes like any other expression.

In format v3 the IR is **fully typed**: every computed value node carries its result type as a canonical type-name string — `Call`/`CallResult` (callee result / success type), `Checked` (the success type of the `Result` it yields), `Binary`/`Unary` (operation result, written before the trailing `loc`), `MemberAccess` (member type), and `ResultValue` (extracted success type), in addition to the nodes that always carried one (`Const`, `Constructor`, `UnionWrap`/`UnionExtract`, `WithUpdate`, list/map literals, `Closure`, `Capture`, `LocalRef`, `FunctionRef`). `ResultIsOk` is implicitly `Boolean` and `ResultError` implicitly `Error`; `Local`/`Global` references resolve through the enclosing `Bind`/param/global declarations (one environment lookup, not inference). This — together with the v4 declaration-fidelity fields (`explicit_type`, `file`) — is what makes the package-path semantic verifier complete: a decoded package body is checkable without re-running type inference, and the typed-IR semantic verifier is the single source of truth for every semantic rule on both the source-lowered IR and decoded package IR (the front-end source checker retains only the source-*syntax* rules for constructs that lowering erases and therefore can never appear in a package — named arguments, `EXIT FUNC`/`EXIT SUB` flavors, inline-`TRAP` boundaries; see `./mfb spec architecture frontend`).
...
* ./mfb spec package binary-representation — the section table this `IR` payload sits in
* ./mfb spec architecture binary-representation — the structured node model encoded here
* ./mfb spec package functions — the function entries whose bodies this section carries
