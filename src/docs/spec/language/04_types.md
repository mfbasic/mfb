...
**Scalar.** A backtick scalar literal `` `x` `` is validated at **lex** time, not in this IR phase: it must contain exactly one Unicode scalar. An empty literal is `TYPE_SCALAR_LITERAL_EMPTY`, more than one scalar is `TYPE_SCALAR_LITERAL_TOO_MANY`, and an unterminated literal or an invalid escape is `TYPE_SCALAR_LITERAL_INVALID`; a `\u{...}` escape naming a surrogate or a value above `U+10FFFF` reuses the shared `MFB_LEX_INVALID_UNICODE_ESCAPE`. Because the lexer is the sole rejecter, every `Scalar` constant that reaches the IR already holds a valid code point and needs no range recheck. [[src/lexer.rs:lex_scalar]]
