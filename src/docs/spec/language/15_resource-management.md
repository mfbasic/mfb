...
A resource value may be passed to a function whose parameter is declared `RES` and either **explicitly names that concrete resource type** — such as `RES f AS File`, `RES s AS Socket`, or a `LINK`-declared resource — **or names a resource union of which that type is a variant** (a `File` value passed to a `RES s AS Stream` parameter, where `Stream` is the resource union below). A function returns a resource with an explicit `AS RES <Type>` return.
