...
The full keyword set is: `AS CASE CONTINUE DO ELSE ELSEIF FALSE FAIL EXIT FOR EACH FUNC IF IN IMPORT ISOLATED LET LAMBDA LOOP DIV MOD MATCH MUT NOTHING AND OR NOT NEXT XOR RETURN SUB TESTING THEN TRUE END ENUM EXPORT PUBLIC PROGRAM PRIVATE PROPAGATE RECOVER RES STEP TO TYPE TRAP UNTIL UNION WHEN WHILE WEND WITH`. `WEND` is reserved but is no longer part of any production: a `WHILE` block closes with `END WHILE`, and a source file using `WEND` is rejected with a diagnostic naming `END WHILE` as the replacement. A keyword token may still be accepted in a name position (e.g. a native `LINK` function named `step`); definition and call sites both canonicalize to the keyword's lowercase lexeme so they match consistently.
