...
`app`, `astrings`, `audio`, `bits`, `collections`, `crypto`, `csv`, `datetime`,
`encoding`, `errorCode`, `fs`, `http`, `io`, `json`, `math`, `money`, `net`, `os`,
`process`, `regex`, `strings`, `term`, `thread`, `tls`, `vector`. [[src/builtins/mod.rs:is_builtin_import]] A bare unqualified `find`, `get`, `append`, `print`,
… is a `SYMBOL_UNKNOWN_IDENTIFIER` error; a qualified `io::print` without
`IMPORT io` is a `SYMBOL_UNKNOWN_IMPORT` error. The `app` package is additionally
gated to `--app` builds: importing it in a plain console build is a compile-time
error (plan-62-A).
...
Error codes (`IMPORT errorCode`): `errorCode::ErrInvalidArgument`, `errorCode::ErrNotFound`, and the other constants listed in the built-in error-code registry. These are compile-time `Integer` constants, not callables. [[src/builtins/errorcode.rs]]
...
- A **producing call** yields a resource, message, connection, or bytes and has
  ...
  `List OF RES TlsSocket → TlsSocket` form), `net::read`/`readText`/`write`/`writeText`
