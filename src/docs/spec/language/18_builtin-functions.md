...
Process environment (`IMPORT os`): `os::getEnv`, `os::getEnvOr`, `os::hasEnv`, `os::setEnv`, `os::unsetEnv`, `os::environ` (`src/builtins/os.rs` `is_os_call`). `os::getEnv` raises `ErrNotFound` for an unset variable; `os::getEnvOr`/`os::hasEnv` are the non-raising alternatives; `os::environ` returns a `Map OF String TO String` snapshot of the live environment.
