...
> Implementation status: the current compiler does **not** support multiple `OUT` slots or `RETURN_OUT`. The parser's native-FUNC body (`parse_link_function`, `src/ast/items.rs`) accepts only `SYMBOL`, `ABI`, `CONST`, `SUCCESS_ON`, `ERROR_ON`, `RESULT`, and `FREE` — there is no `RETURN_OUT` clause — and the syntaxchecker rejects any `OUT` slot not named `return` with `NATIVE_ABI_UNBOUND_SLOT` (`src/syntaxcheck/`). A wrapper may therefore declare at most one `OUT` slot, and it must be the result marker named `return`. The example above is documentation of the deferred form, not a compilable binding.
