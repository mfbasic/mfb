...
The declared name may coincide with a **built-in resource type**'s bare name — `File`, `Socket`, `Listener`, `UdpSocket`, `TlsSocket`, `TlsListener`, `AudioInput`, `AudioOutput` — without shadowing it: built-in resources are package-qualified (`fs::File`, `net::Socket`, …), so a user `RESOURCE File` names a distinct, package-scoped type with its own registered close op, and the two never collide in resolution or in close-op dispatch. (The former `RESOURCE_SHADOWS_BUILTIN` rejection predates package-qualified built-ins and is retired.)
