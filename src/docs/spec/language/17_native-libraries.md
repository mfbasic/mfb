...
Native ABI types are separate from MFBASIC source types. The names below are the ones the marshaling backend acts on. The ABI-slot C type is *not* validated against a fixed list by the source checker; it is parsed as a free identifier and any name not handled below falls through to raw 64-bit passthrough. (A separate, narrower allow-list governs which C-ABI type names are rejected from a wrapper's MFBASIC-facing signature via `NATIVE_CPTR_ESCAPE`; it does **not** include `CBool`, `CByte`, or `CVoid`.) [[src/target/shared/code/link_thunk.rs:emit_return_passthrough]] [[src/ast/items.rs:parse_c_type_name]] [[src/syntaxcheck/helpers.rs:is_c_abi_type]]
