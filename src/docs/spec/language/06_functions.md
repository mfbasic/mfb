...
- **Isolated functions**: a project-visible top-level `FUNC` — `PUBLIC` (the default) or `EXPORT` — may be marked `ISOLATED` to declare that it can run as a thread entry point. `ISOLATED` is invalid on `SUB`, lambdas, closures, and local functions — a `PRIVATE` or non-`FUNC` declaration marked `ISOLATED` is rejected ("ISOLATED function … must be a project-visible FUNC declaration (PUBLIC — the default — or EXPORT, not PRIVATE)"). [[src/ir/verify/mod.rs]] `thread::start` further requires its entry point to be an exported `ISOLATED FUNC` from an *imported* package, so a `PUBLIC ISOLATED FUNC` compiles but is not yet nameable as an entry.
