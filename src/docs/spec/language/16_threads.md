...
`thread::sleep(Thread, ms)` blocks the *calling* thread for `ms` milliseconds and returns `Nothing`. It is unrelated to the queues — a plain wall-clock delay that reads nothing from the worker and does not observe cancellation. Following the `ms` convention, `0` returns immediately, a positive `ms` blocks for at least that long, and a negative `ms` is `ErrInvalidArgument`.
