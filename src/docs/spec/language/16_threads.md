...
thread::sleep OF Msg, Out(t AS Thread OF Msg TO Out, ms AS Integer) AS Nothing
thread::sleep OF Msg, Out(t AS ThreadWorker OF Msg TO Out, ms AS Integer) AS Nothing
...
`thread::sleep(t, ms)` blocks the *calling* thread for `ms` milliseconds and returns `Nothing`. Following the `ms` convention, `0` returns immediately, a positive `ms` blocks for at least that long, and a negative `ms` is `ErrInvalidArgument`. It accepts either handle side, which selects the behavior: on a parent `Thread` it is a plain, uninterruptible wall-clock delay that does not observe cancellation; on a `ThreadWorker` it is a cancellation point — a worker sleep in progress wakes early and fails with `ErrInterrupted` when the parent requests cancellation (`thread::cancel`, or dropping the parent handle), and because the deadline is absolute a parent `send` arriving mid-sleep does not shorten it. The worker form is one of the runtime-managed worker waits governed by the cancellation contract below.
