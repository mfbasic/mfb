...
Each thread has a bounded inbound queue and bounded outbound queue. `thread::start` rejects limits less than `1` with `ErrInvalidArgument`. `thread::send(Thread, ...)` sends a value to the worker inbound queue. `thread::receive(ThreadWorker, ...)` reads from that inbound queue and is valid only inside the running worker. `thread::send(ThreadWorker, ...)` sends to the parent-visible outbound queue. `thread::poll` waits up to `ms` milliseconds for an outbound message from the worker and returns `TRUE` when the next `thread::receive(Thread, ...)` can read without waiting. `thread::receive(Thread, ...)` reads the next outbound message; with no timeout it blocks until a message is available or the worker completes, and the timed form `thread::receive(Thread, 0)` instead fails at once with `ErrNotFound` when no message is available.

Timeouts differ between the writing side and the reading side. For `thread::send` and `thread::transfer`, `timeoutMs = 0` (the default) means do not wait, a positive timeout waits up to that many milliseconds for space, and a negative timeout is invalid. `thread::receive` and `thread::accept` instead have two forms: omitting `timeoutMs` blocks until a message/resource arrives, the queue closes, or the worker is cancelled; an explicit `timeoutMs` must be `>= 0` (`0` polls without waiting, a positive value waits up to that many milliseconds), and a negative explicit timeout is rejected with `ErrInvalidArgument`. Sending to a full queue or a timed read of an empty queue that expires fails with `ErrTimeout`.
