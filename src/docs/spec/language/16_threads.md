...
- A thread entry point must name an `ISOLATED FUNC` — that is the whole rule. It may be one declared in the current project, at any visibility (`PRIVATE`, `PUBLIC` or `EXPORT`) and in a project of any `kind`, named by its bare identifier (`thread::start(worker, …)`); or an `EXPORT ISOLATED FUNC` of an imported package, named through its import binding (`thread::start(pkg::worker, …)`). A package names its own entries by their bare identifier exactly as an executable does. Nothing about *how* the entry is reached matters: `ISOLATED` is the sole marker.
...
- Each started thread receives its own fresh instance of the project that *declares* the entry — an imported package, or the current project when the entry is a local `ISOLATED FUNC` — including a distinct worker arena. Its top-level bindings are initialized from their declarations, not zeroed: a `MUT` declared `= 7` reads 7 in the worker, not 0. Starting isolated functions from the same project more than once creates independent state for each thread, and the parent's own copy is never mutated by a worker.
- **How that per-project instance is provided: by name resolution, not by partitioning.** The worker's arena carries a fresh, re-initialized copy of the *whole program's* writable globals — the region is not split per project. What makes the instance per-project is that a worker can only *name* the top-level bindings of the project declaring its entry, plus those of that project's imports; the remaining slots exist but are unreachable by any spelling. The guarantee above is the contract; this is the mechanism that supplies it. Anything that observes globals non-lexically rather than by name — reflection, a debug dump, a cross-module registry — would see past the boundary, so such a facility must not be added without revisiting this rule. [[src/codegen/engine/builder/mod.rs:arena_global_slots]]
- **Initializer side effects re-run once per `thread::start`.** Because the worker re-initializes the declaring project's top-level bindings, any side effect in an initializer expression happens again on every start, not once per process. This is most visible when the entry lives in an executable, whose top level is where startup-flavored bindings usually sit.
