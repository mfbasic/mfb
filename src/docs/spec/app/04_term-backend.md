...
attributes without moving the shadow cursor. A single builtin, **`term::sync()`**, is the *only*
...
runtime-synthesized classes). Eight methods are added, then
...
never messages. Seventeen 8-byte fields = 136 bytes (`TV_STATE_SIZE`). [[src/target/macos_aarch64/app/term_view.rs:emit_term_init_helper]]
