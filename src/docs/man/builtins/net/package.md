...
| `77050008` | `ErrTimeout` | the single timeout error for every net wait (plan-73-C): `connectTcp`/`accept` when the connection/client does not arrive before the deadline (including a `0` immediate attempt), and `read`/`readText`/`write`/`writeText`/`sendTo`/`sendTextTo`/`receiveFrom`/`receiveTextFrom` when the socket's read/write timeout elapses (this replaced the retired `ErrReadTimeout`/`ErrWriteTimeout`) [[src/target/shared/code/error_constants.rs:ERR_TIMEOUT_CODE]] |
