...
| `77050008` | `ErrTimeout` | The destination queue is full and space did not become available before the deadline — immediately when `timeoutMs` is `0`. (Omitting `timeoutMs` blocks instead of raising this.) [[src/codegen/builtins/errorcode/mod.rs:ErrTimeout]] |
