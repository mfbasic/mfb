...
[[src/codegen/builtins/datetime/func_is_before.rs:__datetime_isBefore]]
...
| --- | --- | --- |
| `a` | `Instant` | The left operand, a point on the UTC timeline. Its `seconds` field is whole seconds since `1970-01-01T00:00:00Z` (possibly negative) and its `nanos` field is the sub-second remainder. The result is `TRUE` only when `a` falls strictly earlier than `b`. [[src/codegen/builtins/datetime/mod.rs:IS_BEFORE]] |
...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when `a` is strictly before `b`, and `FALSE` when `a` is equal to or after `b`. The `seconds` fields are compared first and the `nanos` fields break ties, so a `FALSE` result includes the case where `a` and `b` name the same instant. [[src/codegen/builtins/datetime/mod.rs:DATETIME]] |
