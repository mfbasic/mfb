...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when `a` is strictly before `b`, and `FALSE` when `a` is equal to or after `b`. The `seconds` fields are compared first and the `nanos` fields break ties, so a `FALSE` result includes the case where `a` and `b` name the same instant. [[src/builtins/datetime.rs:DATETIME]] |
