...
| --- | --- | --- |
| `77050003` | `ErrInvalidFormat` | `value` does not match `pattern`: a literal character or quoted literal is absent, a numeric token finds no digit, a month-name or AM/PM token is unrecognized, an offset token is malformed, or `pattern` contains a run of letters that is not a recognized token. [[src/codegen/builtins/datetime/package.mfb:__datetime_parseFields]] [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidFormat]] |
| `77050002` | `ErrInvalidArgument` | An offset token in `value` decodes to a magnitude of 24 hours (86400 seconds) or more, which is out of range for a fixed-offset zone. [[src/codegen/builtins/datetime/package.mfb:__datetime_fixedOffset1]] [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidArgument]] |
