...
| Type | Description |
| --- | --- |
| `Integer` | The number of whole nanoseconds from the Unix epoch to `at`: negative before the epoch, `0` at the epoch, positive after. The value is exact, capturing the complete sub-second precision of `at`. [[src/codegen/builtins/datetime/mod.rs:DATETIME]] |
