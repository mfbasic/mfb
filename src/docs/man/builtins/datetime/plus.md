...
| --- | --- | --- |
| `a` | `Duration` | The first span to add. Its `seconds` field is a whole-second count (which may be negative) and its `nanos` field is the sub-second remainder. [[src/codegen/builtins/datetime/mod.rs:PLUS]] |
