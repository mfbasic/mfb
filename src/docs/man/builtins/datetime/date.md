...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `month` is outside `1 .. 12`, or `day` is outside the valid range for the given month and year (for example `datetime::date(2026, 2, 29)`, since 2026 is not a leap year). [[src/builtins/datetime_package.mfb:__datetime_date]] [[src/builtins/errorcode.rs:ErrInvalidArgument]] |
