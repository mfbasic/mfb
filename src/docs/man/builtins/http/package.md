...
need `IMPORT tls` — the TLS branch stays sealed inside `http`. [[src/codegen/builtins/http/package.mfb:__http_pump]]
...
escapes. [[src/codegen/builtins/http/package.mfb:__http_read]]
...
threads. The same `Response` record is used by the server (below). [[src/codegen/builtins/http/package.mfb:Response]]
...
function. [[src/codegen/builtins/http/package.mfb:__http_parseResponse]]
...
capped at 64 MiB regardless. [[src/codegen/builtins/http/package.mfb:__http_readNet]]
...
followed. [[src/codegen/builtins/http/package.mfb:__http_buildRequest]]
...
connection. There are no threads, no async, and no keep-alive. [[src/codegen/builtins/http/package.mfb:__http_handleRequest]]
...
(`List OF Byte`) — all read with the ordinary `collections` accessors. [[src/codegen/builtins/http/package.mfb:__http_matchPath]]
...
`Connection: close` are supplied on emit. [[src/codegen/builtins/http/package.mfb:__http_buildResponse]]

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | raised by `read`/`write` when the method is empty or contains a space, when a caller header name/value or the URL-derived request-target/Host carries a control byte below `0x20` (in particular CR/LF — request-splitting is rejected, not framed), and by `route` when a `*`/`:name?` segment is not trailing [[src/codegen/builtins/http/package.mfb:__http_normalizeMethod]] [[src/codegen/builtins/http/package.mfb:__http_hasControlBytes]] |
| `77050003` | `ErrInvalidFormat` | raised by `read`/`write` on a malformed response status line, header block, or chunked framing; the server maps the same class of request-parse failure (malformed request line, non-text headers, bad multipart framing) to a `400` response [[src/codegen/builtins/http/package.mfb:__http_parseStatusLine]] |
| `77050010` | `ErrOverflow` | raised by `read`/`write` when the response exceeds the internal 64 MiB size cap; the server maps an oversize request to a `413` response [[src/codegen/builtins/http/package.mfb:__http_readNet]] |
