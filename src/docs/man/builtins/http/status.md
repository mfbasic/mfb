...
[[src/codegen/builtins/http/package.mfb:__http_responseWith]] [[src/codegen/builtins/http/func_status.rs:status]]
...
[[src/codegen/builtins/http/package.mfb:__http_reasonPhrase]]
...
- The header name is stored lowercased, as `content-type`. `http::withHeader`
  ...
  both are emitted. [[src/codegen/builtins/http/package.mfb:__http_withHeader]]
  [[src/codegen/builtins/http/package.mfb:__http_serializeHead]]
...
when a handler traps. [[src/codegen/builtins/http/package.mfb:__http_dispatch]]
[[src/codegen/builtins/http/package.mfb:__http_buildResponse]]
...
| --- | --- | --- |
| `code` | `Integer` | The HTTP status code stored in the response and written to the status line. Not range-checked; any `Integer` is accepted. [[src/codegen/builtins/http/mod.rs:aliases]] |
| `body` | `String` | The response body. Encoded to bytes as UTF-8. Any string is accepted, including the empty string. [[src/codegen/builtins/http/mod.rs:aliases]] |

## Return value

| Type | Description |
| --- | --- |
| `Response` | A new response with `status` `code`, `reason` `""`, `httpVersion` `"1.1"`, `headers` containing only `content-type` → `"text/plain; charset=utf-8"`, `body` the UTF-8 bytes of `body`, and `ok` `TRUE` when `code` is in `200..299` and `FALSE` otherwise. [[src/codegen/builtins/http/package.mfb:__http_status]] |
