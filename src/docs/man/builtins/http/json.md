...
[[src/codegen/builtins/http/package.mfb:__http_json]]
[[src/codegen/builtins/http/package.mfb:__http_responseWith]] [[src/codegen/builtins/http/func_json.rs:json]]
...
- `reason` is `""`, not `"OK"`. That is deliberate — when the response is
  ...
  anything. [[src/codegen/builtins/http/package.mfb:__http_serializeHead]]
...
- The header name is stored lowercased, as `content-type`. `http::withHeader`
  ...
  entry and both are emitted. [[src/codegen/builtins/http/package.mfb:__http_withHeader]]
...
dropped. [[src/codegen/builtins/http/package.mfb:__http_serializeHead]]
...
| --- | --- | --- |
| `body` | `String` | The JSON document, sent verbatim. Encoded to bytes as UTF-8. Any string is accepted, including the empty string and text that is not valid JSON. [[src/codegen/builtins/http/mod.rs:aliases]] |

## Return value

| Type | Description |
| --- | --- |
| `Response` | A new response with `status` `200`, `reason` `""`, `httpVersion` `"1.1"`, `headers` containing only `content-type` → `"application/json"`, `body` the UTF-8 bytes of `body`, and `ok` `TRUE`. [[src/codegen/builtins/http/package.mfb:__http_json]] |
