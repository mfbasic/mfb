...
| Type | Description |
| --- | --- |
| `Integer` | `value` shifted left by `count` bits, with vacated low bits zero and bits above bit 63 discarded. Equal to `value` when `count` is `0`. [[src/codegen/builtins/bits/mod.rs:register]] |
