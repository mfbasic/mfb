...
| `77050010` | `ErrOverflow` | `abs` of the minimum representable `Integer` or `Fixed` value, and any `Integer` computation whose intermediate or result exceeds the `Integer` range, as in the scalar `math::` functions [[src/builtins/errorcode.rs:ErrOverflow]] |
