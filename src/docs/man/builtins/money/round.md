...
time to record it. [[src/codegen/builtins/money/mod.rs:MONEY]]
...
[[src/codegen/builtins/money/func_round.rs:lower_money_round]]
...
`Integer`. [[src/codegen/builtins/money/func_round.rs:lower_money_round]]
...
| --- | --- | --- |
| `value` | `Money` | The amount to settle. Any `Money` is accepted, including zero and negative amounts. [[src/codegen/builtins/money/func_round.rs:register]] |
| `decimals` | `Integer` | The number of fractional decimal places to keep. Must be `0` through `5` inclusive; `5` is the identity and `0` settles to whole currency units. [[src/codegen/builtins/money/func_round.rs:lower_money_round]] |

## Return value

| Type | Description |
| --- | --- |
| `Money` | `value` settled to `decimals` places under the current rounding mode, still a `Money` carrying five decimal places (the places below `decimals` are zero). [[src/codegen/builtins/money/mod.rs:MONEY]] |
