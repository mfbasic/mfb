...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when some entry of `prefixes` is a prefix of `value`, `FALSE` otherwise. An empty list yields `FALSE`; a list containing `""` always yields `TRUE`. [[src/builtins/strings.rs:STRINGS]] |
