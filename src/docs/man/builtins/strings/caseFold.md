...
| --- | --- | --- |
| `value` | `String` | The string to fold. Any `String` is accepted, including the empty string. [[src/codegen/registry/mod.rs:call_param_names]] |

## Return value

| Type | Description |
| --- | --- |
| `String` | A new `String` holding the case-folded form of `value`. The empty string yields `""`; a string with no cased scalars yields an equal string. May be longer than `value`. [[src/codegen/builtins/strings/mod.rs:register]] |
