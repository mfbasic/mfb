...
produces the rewritten text. [[src/codegen/builtins/regex/mod.rs:register]]
...
| Type | Description |
| --- | --- |
| `String` | A new `String` with every non-overlapping match replaced by the expansion of `replacement`. Equal to `value` when `pattern` matches nothing. [[src/codegen/builtins/regex/mod.rs:register]] |
