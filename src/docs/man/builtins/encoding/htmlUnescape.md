...
and `;`: [[src/codegen/builtins/encoding/func_html_unescape.rs:__encoding_htmlUnescape]]

- A **hexadecimal numeric** reference `&#x…;` or `&#X…;` (for example
  `&#xE9;`), where the digits after `#x`/`#X` are parsed as base 16.
  [[src/codegen/builtins/encoding/package.mfb:__encoding_parseHex]]
- A **decimal numeric** reference `&#…;` (for example `&#233;`), where the
  digits after `#` are parsed as base 10.
  [[src/codegen/builtins/encoding/package.mfb:__encoding_parseDecimal]]
- A **named** reference `&…;` (for example `&eacute;`), looked up in the
  built-in entity table. [[src/codegen/builtins/encoding/package.mfb:__encoding_htmlEntity]]
...
not screened out. [[src/codegen/builtins/encoding/package.mfb:__encoding_fromCodepoint]]
...
| Type | Description |
| --- | --- |
| `String` | A copy of `text` with each named and numeric character reference replaced by its character. The empty string for empty input. [[src/codegen/builtins/encoding/func_html_unescape.rs:register]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050003` | `ErrInvalidFormat` | A `&` reference is not closed by a `;`; a numeric reference has empty or non-digit digits; a named reference is unknown; or a numeric reference resolves to a value above `1114111`. [[src/codegen/builtins/encoding/func_html_unescape.rs:__encoding_htmlUnescape]] [[src/codegen/builtins/encoding/package.mfb:__encoding_fromCodepoint]] [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidFormat]] |
