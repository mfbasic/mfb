...
Assert that two `String` values are not equal, checking the operand type too

## Synopsis

```
expectNString(actual AS String, expected AS String)
```
...
`expectNString` is the `String`-typed inequality assertion and the exact mirror of
`expectString`. It evaluates `actual` and `expected` — in that order, exactly once
...
printable — is a compile error here, so the assertion pins the type and the
difference in one line. [[src/builtins/testing.rs:expect_operand_type]]

The check is the ordinary `String` `=`, negated: an exact content comparison with
no case folding, no Unicode normalization, and no whitespace trimming. Two
strings that render identically but differ in normalization form therefore
satisfy `expectNString`, which is worth keeping in mind when the assertion is
meant to prove a transformation changed the text.
[[src/testing/desugar.rs:expand_eq]]

`expectNString` is a statement-level assertion: it produces `Nothing` and cannot
...
| --- | --- | --- |
| `actual` | `String` | The text produced by the code under test. Must be exactly `String`. Evaluated first, exactly once. This is the value rendered in the failure detail. |
| `expected` | `String` | The text `actual` must differ from, compared exactly. Must be exactly `String`. |

## Return value

| Type | Description |
| --- | --- |
| `Nothing` | `expectNString` is a statement-level assertion and yields no value. [[src/syntaxcheck/inference.rs:check_expect_call]] |
...
Assert that a case transformation actually changed the text:
