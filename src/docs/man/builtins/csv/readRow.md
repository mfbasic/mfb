...
next call. [[src/codegen/builtins/csv/func_read_row.rs:__csv_next]]
...
`csv::parseStream`. [[src/codegen/builtins/csv/func_read_row.rs:__csv_next]]
...
| Type | Description |
| --- | --- |
| `CsvRow` | `{ fields AS List OF String, reader AS CsvReader, done AS Boolean }`. When `done` is `TRUE` there was no more input and `fields` is empty. [[src/codegen/builtins/csv/package.mfb:CsvRow]] |
