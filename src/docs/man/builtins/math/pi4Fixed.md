...
`math::pi4Fixed` is the closest Q32.32 `Fixed` to `pi / 4`, which is irrational and has no exact finite representation. Its decimal shorthand is `0.7853981633974483`; the stored `Fixed` is that value rounded to the nearest representable Q32.32 step, so it carries less precision than the `Float` form `math::pi4`. [[src/builtins/math.rs:constant_value]]
...
| Type | Description |
| --- | --- |
| `Fixed` | The `Fixed` nearest to `pi / 4`, approximately `0.7853981633974483`. The same value on every reference. [[src/builtins/math.rs:constant_value]] |
