...
`math::piFixed` is the closest Q32.32 `Fixed` to `pi`, which is irrational and has no exact finite representation. Its decimal shorthand is `3.141592653589793`; the stored `Fixed` is that value rounded to the nearest representable Q32.32 step, so it carries less precision than the `Float` form `math::pi`. [[src/codegen/builtins/math/mod.rs:constant_value]]
...
| Type | Description |
| --- | --- |
| `Fixed` | The `Fixed` nearest to `pi`, approximately `3.141592653589793`. The same value on every reference. [[src/codegen/builtins/math/mod.rs:constant_value]] |
