...
| Type | Description |
| --- | --- |
| Same as the argument | The cosine of `value`, in the argument's own type; within `[-1, 1]` for an argument in the accurate range. `cos(0)` is one. The array form returns a new `List OF Float` of the same length. [[src/builtins/math.rs:MATH]] |
...
`math::cos` takes exactly one argument. [[src/builtins/math.rs:MATH]] It must be
