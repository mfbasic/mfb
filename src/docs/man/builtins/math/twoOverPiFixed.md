...
`math::twoOverPiFixed` is the closest Q32.32 `Fixed` to `2 / pi`, which is irrational and has no exact finite representation. Its decimal shorthand is `0.6366197723675814`; the stored `Fixed` is that value rounded to the nearest representable Q32.32 step, so it carries less precision than the `Float` form `math::twoOverPi`. [[src/codegen/builtins/math/mod.rs:constant_value]]
...
| Type | Description |
| --- | --- |
| `Fixed` | The `Fixed` nearest to `2 / pi`, approximately `0.6366197723675814`. The same value on every reference. [[src/codegen/builtins/math/mod.rs:constant_value]] |
