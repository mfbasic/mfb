...
Base-10 (common) logarithm of a `Float` or `Fixed`.

## Synopsis

```
math::log10(value AS Float) AS Float
math::log10(value AS Fixed) AS Fixed
math::log10(values AS List OF Float) AS List OF Float
math::log10(values AS List OF Fixed) AS List OF Fixed
```
...
`math::log10` returns the base-10 logarithm of `value` — the power to which ten
must be raised to obtain `value`. `log10(1)` is zero, `log10(10)` is one, and
`log10(100)` is two.
...
A positive argument always succeeds. `math::log10` has no overflow or infinity
failure path — the base-10 logarithm of the largest representable `Float` is only
about `308`. [[src/target/shared/code/builder_simd_float_math.rs:FloatKernel]]

The `Fixed` overload computes the natural logarithm by normalise-and-series and
then scales it from base `e` to base 10.
[[src/target/shared/code/builder_fixed_math.rs:emit_fixed_log]]

`math::log10` accepts `Float` or `Fixed` **only**. `Integer` is not accepted and
neither is `Money`: the transcendental functions do not take part in the `Money`
dimension, so `math::log10(2)` and `math::log10(2.00m)` are both compile-time errors.
...
overloads share that one kernel, so `math::log10(x)` and the corresponding element
of `math::log10` over a one-element list are bit-identical. Across macOS,
...
Deterministic Q32.32 normalise-and-series scaled to base 10, rounded to the nearest representable `Fixed`. Fails with `ErrInvalidArgument` when `value` is zero or negative.

**`math::log10(values AS List OF Float) AS List OF Float`**
**`math::log10(values AS List OF Fixed) AS List OF Fixed`**

Applies `math::log10` to every element and returns a new list of the same element
...
| --- | --- | --- |
| `value` | `Float` or `Fixed` | The number whose base-10 logarithm is taken. Must be strictly positive. This parameter has no alternate name. [[src/codegen/builtins/math/mod.rs:call_param_names]] |
...
| Type | Description |
| --- | --- |
| Same as the argument | The base-10 logarithm of `value`, in the argument's own type. `1` returns zero. The array forms return a new list of the same element type and length. [[src/codegen/builtins/math/mod.rs:MATH]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050012` | `ErrFloatDomain` | The `Float` overload only: `value` is zero or negative, so it has no real logarithm. [[src/codegen/builtins/errorcode/mod.rs:ErrFloatDomain]] |
| `77050002` | `ErrInvalidArgument` | The `Fixed` overload only: `value` is zero or negative. [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidArgument]] |

## Type checking

`math::log10` takes exactly one argument. [[src/codegen/builtins/math/mod.rs:MATH]] It must be
...
SUB main()
  LET a AS Float = math::log10(1000.0)
  LET b AS Fixed = math::log10(1000.0F)
  io::print(toString(a))
  io::print(toString(b))
END SUB
```
...
SUB main()
  LET samples AS List OF Float = [1000.0, 1000.0]
  LET results AS List OF Float = math::log10(samples)
