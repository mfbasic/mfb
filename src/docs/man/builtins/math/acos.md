...
| Type | Description |
| --- | --- |
| Same as the argument | The arc cosine of `value` in radians, in the argument's own type, within `[0, pi]`. The array form returns a new `List OF Float` of the same length. [[src/builtins/math.rs:MATH]] |
...
`math::acos` takes exactly one argument. [[src/builtins/math.rs:MATH]] It must be
