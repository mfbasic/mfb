...
`math::pi2Fixed` is the closest Q32.32 `Fixed` to `pi / 2`, which is irrational and has no exact finite representation. Its decimal shorthand is `1.5707963267948966`; the stored `Fixed` is that value rounded to the nearest representable Q32.32 step, so it carries less precision than the `Float` form `math::pi2`. [[src/builtins/math.rs:constant_value]]
...
| Type | Description |
| --- | --- |
| `Fixed` | The `Fixed` nearest to `pi / 2`, approximately `1.5707963267948966`. The same value on every reference. [[src/builtins/math.rs:constant_value]] |
