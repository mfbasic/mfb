...
| Type | Description |
| --- | --- |
| Same as the argument | The arc sine of `value` in radians, in the argument's own type, within `[-pi/2, pi/2]`. The array form returns a new `List OF Float` of the same length. [[src/builtins/math.rs:MATH]] |
...
`math::asin` takes exactly one argument. [[src/builtins/math.rs:MATH]] It must be
