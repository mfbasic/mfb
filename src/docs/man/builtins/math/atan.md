...
| Type | Description |
| --- | --- |
| Same as the argument | The arc tangent of `value` in radians, in the argument's own type, within `(-pi/2, pi/2)`; exactly `+/-pi/2` for an infinite `Float`. The array form returns a new `List OF Float` of the same length. [[src/builtins/math.rs:MATH]] |
...
`math::atan` takes exactly one argument. [[src/builtins/math.rs:MATH]] It must be
