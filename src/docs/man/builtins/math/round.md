...
| Type | Description |
| --- | --- |
| `Integer` | The integer nearest to `value`, with an exact half resolved away from zero. Whole-number inputs return their exact integer value. For a `Money` argument, the dimensionless count of whole currency units. [[src/codegen/builtins/math/mod.rs:MATH]] |
...
`math::round` takes exactly one argument. [[src/codegen/builtins/math/mod.rs:MATH]] It must
