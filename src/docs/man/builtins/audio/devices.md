...
[[src/codegen/builtins/audio/mod.rs:register]]
...
because ALSA hints do not distinguish a system default. [[src/codegen/builtins/audio/native/alsa.rs:lower_devices]]
...
rather than returning an empty list. [[src/codegen/builtins/audio/native/macos.rs:lower_devices]]
...
when ALSA reports no PCM hints; it does not raise. [[src/codegen/builtins/audio/native/alsa.rs:lower_devices]]

## Parameters

(none) [[src/codegen/builtins/audio/mod.rs:register]]

## Return value

| Type | Description |
| --- | --- |
| `List OF AudioDevice` | Every audio device the host reports, in the order the operating system returns them. On macOS the list is always nonempty on success — an empty enumeration raises `ErrAudioUnavailable` instead; on Linux the list may be empty when ALSA reports no PCM hints. [[src/codegen/builtins/audio/mod.rs:register]][[src/codegen/builtins/audio/native/alsa.rs:lower_devices]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050017` | `ErrAudioUnavailable` | No audio device is present (macOS reports an empty device list), or, on Linux, `libasound.so.2` could not be resolved or the ALSA hint enumeration failed. [[src/codegen/builtins/audio/native/macos.rs:lower_devices]][[src/codegen/builtins/audio/native/alsa.rs:lower_devices]] |
| `77050018` | `ErrAudioDevice` | The Core Audio device-enumeration API failed while querying the device list, a device UID, or a device name (macOS only). [[src/codegen/builtins/audio/native/macos.rs:lower_devices]] |
| `77010001` | `ErrOutOfMemory` | Allocation of the returned list, a device record, or a device name/id string failed. [[src/codegen/builtins/audio/native/macos.rs:lower_devices]][[src/codegen/builtins/audio/native/alsa.rs:lower_devices]] |
