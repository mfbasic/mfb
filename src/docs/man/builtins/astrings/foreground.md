...
Construct a foreground-color `Attribute`.

## Synopsis

```
astrings::foreground(r AS Byte, g AS Byte, b AS Byte) AS Attribute
```
...
`foreground` returns an `Attribute` wrapping the `AttrNumber` with `kind`
...
`term` and `astrings` imported), the color is emitted as a truecolor foreground.
...
| Type | Description |
| --- | --- |
| `Attribute` | The foreground attribute (`AttrNumber[AttrTypeNumber.Foreground, 0xRRGGBB]`). |
...
SUB main()
  ...
  LET styled AS AttributedString = astrings::addAttribute(a, 0, 4, astrings::foreground(255, 128, 0))
END SUB
```

## See also

- `mfb man astrings background`
