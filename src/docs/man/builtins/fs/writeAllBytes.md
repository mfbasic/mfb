...
[[src/codegen/builtins/fs/native/io.rs:lower_fs_write_all_bytes_helper]]
...
| --- | --- | --- |
| `file` | `File` | An open `File` resource to write to, positioned at the point where the bytes should be written. Must not have been closed and must have been opened in a mode that permits writing (`"write"`, `"readWrite"`, or `"append"`). [[src/codegen/builtins/fs/mod.rs:register]] |
| `bytes` | `List OF Byte` | The bytes to write, in order, taken verbatim from the list's data region. An empty list writes nothing. [[src/codegen/builtins/fs/mod.rs:register]] |

## Return value

| Type | Description |
| --- | --- |
| `Nothing` | Returns nothing on success, after every byte of `bytes` has been written to `file`. [[src/codegen/builtins/fs/mod.rs:register]] |
