...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `base` or `child` is empty or contains an embedded NUL byte, so it cannot be turned into a valid NUL-terminated host path. [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidArgument]] |
| `77010001` | `ErrOutOfMemory` | An internal NUL-terminated copy of an argument or a `realpath` resolution buffer cannot be allocated. [[src/codegen/builtins/errorcode/mod.rs:ErrOutOfMemory]] |
