...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `path` is empty or contains an embedded NUL byte, so it cannot be turned into a valid NUL-terminated host path. [[src/builtins/errorcode.rs:ErrInvalidArgument]] |
...
| `77020002` | `ErrOutput` | Writing, flushing, or closing the temporary file fails, or creating the temporary file or renaming it fails for any other host reason. [[src/builtins/errorcode.rs:ErrWriteFailed]] |
