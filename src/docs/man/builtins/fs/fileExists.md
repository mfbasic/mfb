...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when `path` exists and resolves to a regular file; `FALSE` otherwise (missing path, directory, or any other non-regular entry). [[src/builtins/fs.rs:FS]] |
