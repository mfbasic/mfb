...
error instead of repeating the release. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]]
...
flag and does nothing. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]]
...
| --- | --- | --- |
| `file` | `File` | The open `File` resource to close, as returned by `fs::open`, `fs::openFile`, `fs::openFileNoFollow`, `fs::openWithin`, or `fs::createTempFile`. Must not already be closed or moved. [[src/codegen/builtins/fs/mod.rs:register]] |

## Return value

| Type | Description |
| --- | --- |
| `Nothing` | Returns nothing on success. After a successful return the descriptor is released and `file` is marked closed and must not be used again. [[src/codegen/builtins/fs/mod.rs:register]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77030004` | `ErrResourceClosed` | `file` has already been closed, whether by an earlier `fs::close` on the same value or by a prior scope-drop. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]] |
| `77030009` | `ErrResourceMoved` | `file` was moved to another thread by `thread::transfer` and no longer belongs to this thread, so it cannot be closed here. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]] |
| `77030006` | `ErrCloseFailed` | The host operating system reports a failure while releasing the descriptor. The handle is still marked closed and must not be reused. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]] |
| `77020002` | `ErrOutput` | The mandatory pre-close drain of the handle's buffered output fails — for example the disk is full or the descriptor is no longer writable. The descriptor is still released. [[src/codegen/builtins/fs/native/io.rs:lower_fs_close_helper]] |
