...
| `value` | `String` | The text to write, taken verbatim as the `String`'s UTF-8 bytes, in order. An empty `String` truncates the file to zero length. [[src/codegen/builtins/fs/mod.rs:register]] |
...
| `77020002` | `ErrOutput` | The file cannot be opened for any other host reason, including `path` referring to a directory or another non-writable target, and when writing, flushing, or closing the file fails partway through. [[src/codegen/builtins/fs/native/atomic.rs:ErrWriteFailed]] |
