...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `path` is empty or contains an embedded NUL byte, so it cannot be turned into a valid NUL-terminated host path. [[src/target/shared/code/fs_helpers_atomic.rs:lower_fs_atomic_write_helper]] |
...
| `77020002` | `ErrOutput` | Writing, flushing, or closing the temporary file fails, or creating the temporary file or renaming it fails for any other host reason. [[src/target/shared/code/fs_helpers_atomic.rs:lower_fs_atomic_write_helper]] |
