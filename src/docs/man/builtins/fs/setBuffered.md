...
| --- | --- | --- |
| `file` | `File` | An open `File` resource whose buffering mode is being changed. [[src/codegen/builtins/fs/mod.rs:register]] |
| `enabled` | `Boolean` | `TRUE` to enable output buffering for this handle; `FALSE` to drain any pending output and disable it. [[src/codegen/builtins/fs/mod.rs:register]] |

## Return value

| Type | Description |
| --- | --- |
| `Nothing` | Returns nothing. The call is made for its side effect of changing the handle's buffering flag (and, when disabling, draining any pending output). [[src/codegen/builtins/fs/mod.rs:register]] |
