...
produced on each platform. [[src/target/shared/code/fs_helpers.rs:emit_fs_path_errno_error_mapping]]
...
| `77030001` | `ErrPathNotFound` | A parent component of `path` does not exist (host `ENOENT`). [[src/target/shared/code/fs_helpers.rs:emit_fs_path_errno_error_mapping]] |
| `77030003` | `ErrAccessDenied` | The host denies permission to create the directory (host `EACCES`). [[src/target/shared/code/fs_helpers.rs:emit_fs_path_errno_error_mapping]] |
| `77050005` | `ErrAlreadyExists` | An entry already exists at `path`, including an existing directory (host `EEXIST`). [[src/target/shared/code/fs_helpers.rs:emit_fs_path_errno_error_mapping]] |
