...
| --- | --- | --- |
| `77030004` | `ErrResourceClosed` | `file` has already been closed. [[src/target/shared/code/fs/io.rs:ERR_RESOURCE_CLOSED_CODE]] |
| `77020003` | `ErrEof` | `file` is already at end of input, so no bytes remain to form a line. [[src/target/shared/code/fs/io.rs:ERR_EOF_CODE]] |
| `77020001` | `ErrRead` | Repositioning the file or reading its bytes fails, including when `file` was not opened for reading and when the host read fails partway through. [[src/target/shared/code/fs/io.rs:ERR_READ_CODE]] |
| `77020004` | `ErrEncoding` | The bytes of the line are not valid UTF-8. [[src/target/shared/code/fs/io.rs:ERR_ENCODING_CODE]] |
