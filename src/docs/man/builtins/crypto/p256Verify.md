...
| --- | --- | --- |
| `publicKey` | `List OF Byte` | The P-256 public key as the 65-byte SEC1 uncompressed point `0x04 \|\| X \|\| Y` (the `publicKey` field of a `crypto::generateP256` key pair). Any other length, or bytes that do not decode to a valid curve point, is an error, not a `FALSE` verdict. [[src/codegen/builtins/crypto/native/openssl.rs:verify]] |
...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` if `signature` is a valid P-256/SHA-256 signature of `message` under `publicKey`; `FALSE` if it is not (including an unparsable signature). A `FALSE` result is a normal outcome, not an error. [[src/codegen/builtins/crypto/mod.rs:CRYPTO]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `publicKey` is not a well-formed 65-byte P-256 SEC1 point — either not exactly 65 bytes, or bytes that do not decode to a valid curve point. A valid-length key with a non-matching (or unparsable) signature returns `FALSE` instead. [[src/codegen/builtins/crypto/native/openssl.rs:verify]] [[src/codegen/builtins/crypto/native/macos.rs:verify]] |
