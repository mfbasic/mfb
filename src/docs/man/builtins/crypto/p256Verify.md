...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` if `signature` is a valid P-256/SHA-256 signature of `message` under `publicKey`; `FALSE` if it is not (including an unparsable signature). A `FALSE` result is a normal outcome, not an error. [[src/builtins/crypto.rs:CRYPTO]] |
