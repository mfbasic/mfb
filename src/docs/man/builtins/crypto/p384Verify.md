...
| --- | --- | --- |
| `publicKey` | `List OF Byte` | The P-384 public key as the 97-byte SEC1 uncompressed point `0x04 \|\| X \|\| Y` (the `publicKey` field of a `crypto::generateP384` key pair). Any other length, or bytes that do not decode to a valid curve point, is an error, not a `FALSE` verdict. [[src/target/shared/code/crypto_ec/openssl.rs:verify]] |
...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` if `signature` is a valid P-384/SHA-384 signature of `message` under `publicKey`; `FALSE` if it is not (including an unparsable signature). A `FALSE` result is a normal outcome, not an error. [[src/builtins/crypto.rs:call_return_type_name]] |
