...
[[src/builtins/crypto_ecdsa.mfb:__crypto_generateP384]]
...
- `privateKey` — 145 bytes, the wire form `0x04 || X || Y || K`: the SEC1
  ...
  `crypto::p384Sign` consumes. [[src/builtins/crypto_ecdsa.mfb:__crypto_generateP384]]
...
| Type | Description |
| --- | --- |
| `crypto::KeyPair` | A record whose `privateKey` is the 145-byte `0x04 \|\| X \|\| Y \|\| K` form and whose `publicKey` is the 97-byte `0x04 \|\| X \|\| Y` SEC1 uncompressed point. [[src/builtins/crypto_hash.mfb:KeyPair]] |
...
| `77010001` | `ErrOutOfMemory` | An arena allocation for the key bytes (or for a byte list built while slicing out the public key) fails. [[src/target/shared/code/crypto_ec/macos.rs:generate]] [[src/target/shared/code/crypto_ec/openssl.rs:generate]] [[src/builtins/crypto_hash.mfb:__crypto_truncate]] |
