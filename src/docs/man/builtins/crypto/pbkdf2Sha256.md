...
Derive a key from a password with PBKDF2 (RFC 8018) over HMAC-SHA-256.

## Synopsis

```
crypto::pbkdf2Sha256(password AS List OF Byte, salt AS List OF Byte, iterations AS Integer, length AS Integer) AS List OF Byte
crypto::pbkdf2Sha256(password AS String, salt AS List OF Byte, iterations AS Integer, length AS Integer) AS List OF Byte
```
...
`crypto::pbkdf2Sha256` is the PBKDF2 password-based key-derivation function of
RFC 8018 instantiated over HMAC-SHA-256. It stretches a low-entropy `password`
...
[[src/builtins/crypto_package.mfb:__crypto_pbkdf2Sha256_bytes]]
...
[[src/builtins/crypto_package.mfb:__crypto_pbkdf2Block256]]
...
`crypto::hkdfSha256` for already-high-entropy keying material.
...
| `iterations` | `Integer` | The PBKDF2 iteration count (work factor). Must be at least 1. Larger values are more resistant to brute force and slower. [[src/builtins/crypto_package.mfb:__crypto_pbkdf2Sha256_bytes]] |
| `length` | `Integer` | Number of derived key bytes to produce. Must be at least 1. [[src/builtins/crypto_package.mfb:__crypto_pbkdf2Sha256_bytes]] |
...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `iterations` is less than 1, or `length` is less than 1. [[src/builtins/crypto_package.mfb:__crypto_pbkdf2Sha256_bytes]] |
...
SUB main()
  ...
  LET key AS List OF Byte = crypto::pbkdf2Sha256("correct horse", salt, 100000, 32)
END SUB
```
...
SUB main()
  ...
  LET salt AS List OF Byte = crypto::randomBytes(16)
  LET key AS List OF Byte = crypto::pbkdf2Sha256(passwordBytes, salt, 100000, 32)
END SUB
```

## See also

- `mfb man crypto pbkdf2Sha512`
- `mfb man crypto hkdfSha256`
