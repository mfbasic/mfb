...
[[src/codegen/builtins/crypto/package.mfb:__crypto_generateP521]]
...
- `privateKey` — 199 bytes, the wire form `0x04 || X || Y || K`: the SEC1
  ...
  `crypto::p521Sign` consumes. [[src/codegen/builtins/crypto/package.mfb:__crypto_generateP521]]
- `publicKey` — 133 bytes, the wire form `0x04 || X || Y`: the leading SEC1
  uncompressed public point, sliced from the private bytes.
  [[src/codegen/builtins/crypto/package.mfb:__crypto_truncate]]
...
OpenSSL/pyca. [[src/codegen/builtins/crypto/native/mod.rs:ec_call]]
...
| Type | Description |
| --- | --- |
| `crypto::KeyPair` | A record whose `privateKey` is the 199-byte `0x04 \|\| X \|\| Y \|\| K` form and whose `publicKey` is the 133-byte `0x04 \|\| X \|\| Y` SEC1 uncompressed point. [[src/codegen/builtins/crypto/package.mfb:KeyPair]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050000` | `ErrUnknown` | The platform key API fails to load or key generation fails (e.g. `SecKeyCreateRandomKey` / `EC_KEY_generate_key` returns an error). [[src/codegen/builtins/crypto/native/macos.rs:generate]] [[src/codegen/builtins/crypto/native/openssl.rs:generate]] |
| `77010001` | `ErrOutOfMemory` | An arena allocation for the key bytes (or for the byte list built while slicing out the public key) fails. [[src/codegen/builtins/crypto/native/macos.rs:generate]] [[src/codegen/builtins/crypto/native/openssl.rs:generate]] [[src/codegen/builtins/crypto/package.mfb:__crypto_truncate]] |
