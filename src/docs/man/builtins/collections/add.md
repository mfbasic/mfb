...
variadic. [[src/codegen/builtins/collections/mod.rs:register]]
...
| --- | --- | --- |
| `value` | `Set OF T` | The set to insert into; left unchanged. Also accepted under the name `set`. Must be a set type; passing a `List` or a scalar resolves no overload and is a compile-time error. [[src/codegen/registry/mod.rs:call_param_names]] [[src/codegen/registry/mod.rs:resolve_call]] |
| `item` | `T` | The element to insert. Also accepted under the name `element`. Its type must be exactly the element type `T`. [[src/codegen/registry/mod.rs:call_param_names]] [[src/codegen/registry/mod.rs:resolve_call]] |

## Return value

| Type | Description |
| --- | --- |
| `Set OF T` | A new set with `item` present, of the same type as `value`. Adding an element already in `value` returns a set equal to `value`. [[src/codegen/registry/mod.rs:resolve_call]] |
