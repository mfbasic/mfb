...
| Type | Description |
| --- | --- |
| `List OF T` | For the list overload: a new list of the same type and the same length as `value`, differing only at `index`. [[src/codegen/registry/mod.rs:resolve_call]] |
| `Map OF K TO V` | For the map overload: a new map of the same type as `value` in which the key maps to `item`. Its size is unchanged when the key was already present and one larger when it was not. [[src/codegen/registry/mod.rs:resolve_call]] |
