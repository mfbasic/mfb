...
element has been tested and none matched. [[src/codegen/builtins/collections/func_any.rs:__collections_any]]
...
| --- | --- | --- |
| `value` | `List OF T` | The list to scan, in index order starting at `0`. An empty list is accepted and yields `FALSE`. Not modified. [[src/codegen/builtins/collections/func_any.rs:__collections_any]] |
| `predicate` | `FUNC(T) AS Boolean` | Test applied to each element. Called with one element at a time; the scan stops at the first call that returns `TRUE`. An error it raises propagates to the caller. [[src/codegen/builtins/collections/func_any.rs:__collections_any]] |

## Return value

| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when `predicate` returns `TRUE` for at least one element of `value`; `FALSE` when it returns `FALSE` for every element, including when `value` is empty. [[src/codegen/builtins/collections/func_any.rs:__collections_any]] |
...
`Boolean`. [[src/codegen/builtins/collections/func_any.rs:__collections_any]]
