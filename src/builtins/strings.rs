use crate::bytecode::{
    ...
    OPCODE_STRING_LOWER, OPCODE_STRING_NORMALIZE_NFC, OPCODE_STRING_REGEX_FIND,
    OPCODE_STRING_REGEX_MATCH, OPCODE_STRING_REGEX_REPLACE, OPCODE_STRING_SPLIT,
...
const REGEX_MATCH: &str = "strings.regexMatch";
const REGEX_FIND: &str = "strings.regexFind";
const REGEX_REPLACE: &str = "strings.regexReplace";
...
pub(crate) fn call_return_type_name(name: &str) -> Option<&'static str> {
    match name {
        ...
        | REGEX_REPLACE => Some("String"),
        GRAPHEMES | SPLIT => Some("List OF String"),
        STARTS_WITH | ENDS_WITH | CONTAINS | REGEX_MATCH => Some("Boolean"),
        BYTE_LEN | REGEX_FIND => Some("Integer"),
        _ => None,
    }
}
...
pub(crate) fn resolve_call<'a>(name: &str, arg_types: &'a [String]) -> Option<ResolvedCall<'a>> {
    let return_type = match name {
        ...
        REGEX_REPLACE if exact(arg_types, &["String", "String", "String"]) => {
            Cow::Borrowed("String")
        }
        _ => return None,
    };
...
pub(crate) fn expected_arguments(name: &str) -> Option<&'static str> {
    match name {
        ...
        STARTS_WITH | ENDS_WITH | CONTAINS | SPLIT | REGEX_MATCH => Some("String, String"),
        JOIN => Some("List OF String, String"),
        REGEX_FIND => Some("String, String, Integer"),
        REGEX_REPLACE => Some("String, String, String"),
        _ => None,
    }
}
...
pub(crate) fn arity(name: &str) -> Option<(usize, usize)> {
    match name {
        TRIM | TRIM_START | TRIM_END | UPPER | LOWER | CASE_FOLD | NORMALIZE_NFC | GRAPHEMES
        | BYTE_LEN => Some((1, 1)),
        STARTS_WITH | ENDS_WITH | CONTAINS | SPLIT | JOIN | REGEX_MATCH => Some((2, 2)),
        REGEX_FIND => Some((2, 3)),
        REGEX_REPLACE => Some((3, 3)),
        _ => None,
    }
}
...
fn opcode_for(name: &str) -> Result<u16, String> {
    match name {
        ...
        REGEX_MATCH => Ok(OPCODE_STRING_REGEX_MATCH),
        REGEX_FIND => Ok(OPCODE_STRING_REGEX_FIND),
        REGEX_REPLACE => Ok(OPCODE_STRING_REGEX_REPLACE),
