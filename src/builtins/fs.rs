use crate::bytecode::{
    BuiltinCallLowerer, ValueSlot, OPCODE_FS_APPEND_TEXT, OPCODE_FS_CANONICAL_PATH,
    OPCODE_FS_CLOSE, OPCODE_FS_CREATE_DIRECTORIES, OPCODE_FS_CREATE_DIRECTORY,
    OPCODE_FS_CREATE_TEMP_FILE, OPCODE_FS_CURRENT_DIRECTORY, OPCODE_FS_DELETE_DIRECTORY,
    OPCODE_FS_DELETE_FILE, OPCODE_FS_DIRECTORY_EXISTS, OPCODE_FS_EOF, OPCODE_FS_EXISTS,
...
pub(crate) fn expected_arguments(name: &str) -> Option<&'static str> {
    match name {
        ...
        CREATE_TEMP_FILE => Some("String, String, String"),
        READ_LINE | READ_ALL | CLOSE | EOF => Some(FILE_TYPE),
        WRITE_ALL => Some("File, String"),
        IS_WITHIN => Some("String, String"),
        PATH_JOIN => Some("List OF String"),
        CURRENT_DIRECTORY => Some("no arguments"),
        _ => None,
    }
}
...
pub(crate) fn lower_bytecode_call(
    ...
    match name {
        ...
        CREATE_TEMP_FILE if lowered.len() == 1 => {
            lowered.push(lowerer.push_string_const("mfb-")?);
            lowered.push(lowerer.push_string_const(".tmp")?);
        }
        CREATE_TEMP_FILE if lowered.len() == 2 => {
            lowered.push(lowerer.push_string_const(".tmp")?);
        }
        _ => {}
    }
