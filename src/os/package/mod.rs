...
mod tests {
    ...
    fn wraps_mfbc_payload_in_unsigned_mfp_container() {
        ...
        assert_eq!(&package[0..8], &MFP_MAGIC);
        assert_eq!(read_u16(&package, 8), CONTAINER_MAJOR);
        assert_eq!(read_u16(&package, 10), CONTAINER_MINOR);
        assert_eq!(read_u16(&package, 12), BYTECODE_MAJOR);
        assert_eq!(read_u16(&package, 14), BYTECODE_MINOR);
        assert_eq!(read_u32(&package, 16), 0);
        assert_eq!(read_u16(&package, 20), SIGNATURE_UNSIGNED);
        assert_eq!(read_u32(&package, 22), 0);

        let mut offset = 26;
        assert_eq!(read_string(&package, &mut offset), "shape");
        assert_eq!(read_string(&package, &mut offset), "1.2.3");
        assert_eq!(read_string(&package, &mut offset), "Ada");
        assert_eq!(
            read_string(&package, &mut offset),
            "https://example.invalid/shape"
        );
        assert_eq!(read_u64(&package, offset), payload.len() as u64);
        offset += 8;
        assert_eq!(&package[offset..], payload);
    }

    #[test]
    fn rejects_non_mfbc_payload() {
        let metadata = BytecodeMetadata::new("shape".to_string(), "1.0.0".to_string());
        let err = build_package_bytes(&metadata, b"nope").expect_err("invalid payload");
        assert!(err.contains("MFBC"));
    }
