...
pub(crate) fn module_uses_type_name(module: &NirModule) -> bool {
    module
        .functions
        .iter()
...
pub(crate) fn module_uses_migrated(module: &NirModule, bare: &str) -> bool {
...
pub(crate) fn module_may_record_cleanup_failure(module: &NirModule) -> bool {
    module
        .functions
        .iter()
...
pub(crate) fn module_field_types(module: &NirModule) -> FieldTypes {
    ...
    for type_ in &module.types {
        match type_.kind.as_str() {
            ...
            "enum" => {
                fields.insert_enum(
                    ParameterType::named(&type_.name),
                    type_
                        .members
                        .iter()
...
pub(crate) fn module_may_emit_float_numeric_error(module: &NirModule) -> bool {
    if module_uses_any_call(
        module,
        &[
            "math.sqrt",
...
pub(crate) fn module_uses_unicode_runtime_tables(module: &NirModule) -> bool {
    let fields = module_field_types(module);
    module.functions.iter().any(|function| {
