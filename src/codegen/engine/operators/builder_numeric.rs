//! Lowering for the numeric operators and their deterministic `Integer`/`Fixed`
...
impl CodeBuilder<'_> {
    ...
    pub(crate) fn lower_numeric_unary_negation(
        &mut self,
        operand: ValueResult,
    ) -> Result<ValueResult, String> {
        let register = self.allocate_register()?;
    ...
    pub(crate) fn guard_lower_bound(&self, condition: &NirValue) -> Option<(String, i64)> {
        let NirValue::Binary {
            op, left, right, ..
        } = condition
        else {
    ...
    pub(crate) fn operand_as_double(&mut self, value: &ValueResult) -> Result<String, String> {
        if Self::float_is_dnative(value) {
    ...
    pub(crate) fn emit_overflow_if_flags_set(&mut self) -> Result<(), String> {
    ...
    pub(crate) fn emit_byte_upper_bound_check(
        &mut self,
    ...
    pub(crate) fn emit_nonzero_or_invalid(
        &mut self,
        value: impl Into<Operand>,
    ) -> Result<(), String> {
    ...
    pub(crate) fn emit_integer_division_overflow_check(
        &mut self,
        left: impl Into<Operand>,
        right: impl Into<Operand>,
    ) -> Result<(), String> {
    ...
    pub(crate) fn emit_fixed_multiply(
        &mut self,
        dst: impl Into<Operand>,
        left: impl Into<Operand>,
        right: impl Into<Operand>,
    ...
    pub(crate) fn emit_fixed_divide(
        &mut self,
        dst: impl Into<Operand>,
        left: impl Into<Operand>,
        right: impl Into<Operand>,
    ...
    pub(crate) fn emit_abs_i64(&mut self, register: impl Into<Operand>) -> Result<(), String> {
    ...
    pub(crate) fn emit_neg_i64(&mut self, register: impl Into<Operand>) -> Result<(), String> {
        let register = register.into();
        self.emit(abi::subtract_registers(
            register.clone(),
            abi::ZERO,
    ...
    pub(crate) fn load_numeric_as_double(
        &mut self,
        dst: impl Into<Operand>,
        value: &ValueResult,
    ) -> Result<(), String> {
    ...
    pub(crate) fn load_numeric_as_fixed(
        &mut self,
        dst: impl Into<Operand>,
        value: &ValueResult,
    ) -> Result<(), String> {
