...
#[derive(Default)]
pub(crate) struct U32Hasher(u64);
...
pub(crate) type U32Map<V> = HashMap<u32, V, BuildHasherDefault<U32Hasher>>;
pub(crate) type U32Set = std::collections::HashSet<u32, BuildHasherDefault<U32Hasher>>;
...
pub(crate) struct ClassModel {
    ...
    pub(crate) parse_vreg: fn(&str) -> Option<u32>,
    pub(crate) physical_index: fn(&str) -> Option<u32>,
    ...
    pub(crate) caller_saved: PhysMask,
}
...
pub(crate) fn caller_saved_mask(
...
pub(crate) fn call_clobber_mask(instruction: &CodeInstruction, model: &ClassModel) -> PhysMask {
...
pub(crate) fn int_physical_index_non_aarch64(name: &str) -> Option<u32> {
    int_concrete_physical_index(name)
}
...
pub(crate) fn riscv_int_index(name: &str) -> Option<u32> {
    RISCV_GPRS
        .iter()
        .position(|&reg| reg == name)
        .map(|i| i as u32)
...
/// (plan-00-H) to `0..=15`.
pub(crate) fn fp_physical_index(name: &str) -> Option<u32> {
...
pub(crate) fn riscv_fp_index(name: &str) -> Option<u32> {
    RISCV_FPRS
        .iter()
        .position(|&reg| reg == name)
        .map(|i| i as u32)
...
pub(crate) struct Effect {
    pub(crate) defs: Vec<RegRef>,
    pub(crate) uses: Vec<RegRef>,
...
pub(crate) struct Liveness {
    ...
    pub(crate) vreg_interval: U32Map<(usize, usize)>,
    ...
    pub(crate) phys_busy_at: Vec<PhysMask>,
    ...
    pub(crate) call_clobber: Vec<(usize, PhysMask)>,
}

/// Occupancy bitset over physical registers `x0`–`x30` (31 < 64 bits).
pub(crate) type PhysMask = u64;

pub(crate) fn physical_busy(bits: PhysMask, index: u32) -> bool {
    bits & (1u64 << index) != 0
}
