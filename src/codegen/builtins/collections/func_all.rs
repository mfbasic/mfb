...
may catch it. `all` itself defines no error of its own. A lambda passed to `all` cannot capture an outer `MUT` binding; use
`collections::forEach` when the scan needs to update one. `all` has no side
