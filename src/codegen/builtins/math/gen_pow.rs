...
use crate::codegen::builtins::math::FloatInfinityError;
...
impl CodeBuilder<'_> {
    ...
    pub(crate) fn lower_pow_array(
        ...
        let result_base =
            self.emit_alloc_result_list(&count, &COLLECTION_TYPE_FLOAT.to_string(), "pow_arr")?;
        ...
        self.emit_collection_data_pointer_for(&ldata, &left_ptr, &ParameterType::Float);
        self.emit(abi::store_u64(&ldata, abi::stack_pointer(), ldata_slot));
        let rdata = self.allocate_register();
        self.emit_collection_data_pointer_for(&rdata, &right_ptr, &ParameterType::Float);
        self.emit(abi::store_u64(&rdata, abi::stack_pointer(), rdata_slot));
        let odata = self.allocate_register();
        self.emit_collection_data_pointer_for(&odata, &result_base, &ParameterType::Float);
