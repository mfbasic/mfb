...
is resolved for that moment, and the canonical `datetime::DateTime` naming the resulting
...
underlying `datetime::Instant` generally differs from `dt`'s `datetime::Instant` truncated to whole
...
SUB main()
  ...
  LET midnight AS datetime::DateTime = datetime::startOfDay(dt)
END SUB
```
