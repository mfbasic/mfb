...
const INTRO: &str = r#"The day of the week of a `datetime::DateTime`'s civil date."#;
const DESC: &str = r#"`datetime::weekday` returns the day of the week on which `dt`'s civil date falls,
as a value of the `datetime::Weekday` enum (`Monday`, `Tuesday`, `Wednesday`, `Thursday`,
...
starting on Monday, matching the ordering of the `datetime::Weekday` enum, so
...
SUB main()
  ...
  LET w AS datetime::Weekday = datetime::weekday(dt)
END SUB
```

Branch on whether a `datetime::DateTime` falls on the weekend:
...
SUB main()
  ...
  LET w AS datetime::Weekday = datetime::weekday(dt)
  IF w = datetime::Weekday.Saturday OR w = datetime::Weekday.Sunday THEN
    io::print("weekend")
  END IF
END SUB
```"#;
