//! `strings::trimChars` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.trimChars` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.trimChars", args)?
        .ok_or_else(|| "strings.trimChars: no native lowering for these arguments".to_string())
}
