//! `strings::toBytes` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.toBytes` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.toBytes", args)?
        .ok_or_else(|| "strings.toBytes: no native lowering for these arguments".to_string())
}
