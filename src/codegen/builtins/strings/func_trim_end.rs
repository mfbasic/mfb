//! `strings::trimEnd` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.trimEnd` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.trimEnd", args)?
        .ok_or_else(|| "strings.trimEnd: no native lowering for these arguments".to_string())
}
