//! `strings::contains` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.contains` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.contains", args)?
        .ok_or_else(|| "strings.contains: no native lowering for these arguments".to_string())
}
