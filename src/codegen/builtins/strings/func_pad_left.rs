//! `strings::padLeft` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.padLeft` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.padLeft", args)?
        .ok_or_else(|| "strings.padLeft: no native lowering for these arguments".to_string())
}
