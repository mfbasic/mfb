//! `strings::padRight` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.padRight` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.padRight", args)?
        .ok_or_else(|| "strings.padRight: no native lowering for these arguments".to_string())
}
