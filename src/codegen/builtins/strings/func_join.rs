//! `strings::join` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.join` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.join", args)?
        .ok_or_else(|| "strings.join: no native lowering for these arguments".to_string())
}
