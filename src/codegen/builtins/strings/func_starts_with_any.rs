//! `strings::startsWithAny` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.startsWithAny` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.startsWithAny", args)?
        .ok_or_else(|| "strings.startsWithAny: no native lowering for these arguments".to_string())
}
