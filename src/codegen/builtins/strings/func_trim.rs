//! `strings::trim` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.trim` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.trim", args)?
        .ok_or_else(|| "strings.trim: no native lowering for these arguments".to_string())
}
