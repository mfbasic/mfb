//! `strings::caseFold` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.caseFold` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.caseFold", args)?
        .ok_or_else(|| "strings.caseFold: no native lowering for these arguments".to_string())
}
