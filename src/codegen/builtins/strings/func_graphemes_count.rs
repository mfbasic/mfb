//! `strings::graphemesCount` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.graphemesCount` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.graphemesCount", args)?
        .ok_or_else(|| "strings.graphemesCount: no native lowering for these arguments".to_string())
}
