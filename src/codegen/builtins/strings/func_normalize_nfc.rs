//! `strings::normalizeNfc` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.normalizeNfc` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.normalizeNfc", args)?
        .ok_or_else(|| "strings.normalizeNfc: no native lowering for these arguments".to_string())
}
