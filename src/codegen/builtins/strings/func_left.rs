//! `strings::left` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.left` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.left", args)?
        .ok_or_else(|| "strings.left: no native lowering for these arguments".to_string())
}
