//! `strings.padLeftToWidth` — descriptor + source-backed lowering (bug-528).
//!
//! The body is the shared `__strings_padLeftToWidth` in `helper_pad_to_width.rs`
...
const INTRO: &str = r#"Pad a string on the left to a given terminal column width."#;

const DESC: &str = r#"`strings::padLeftToWidth` returns a new `String` in which copies of `padChar` are
prepended to `value` until the result fills as much of `columns` terminal columns
as whole copies of `padChar` allow. Width is measured by
`strings::displayWidth`, so this is the member that lines a table up on screen.

`strings::padLeft` measures the same argument in **Unicode scalar values**, which
is the right unit for a fixed-length record field or a zero-padded number and the
wrong one for a column: `strings::padLeft("日本", 4, "-")` is four scalars and six
columns, and three such rows padded to the same scalar count do not line up.
...
`value` to six columns with a two-column `padChar` prepends two copies and yields
...
is added and the result equals `value`. `padLeftToWidth` never truncates.
...
const EX: &str = r#"Right-align a column of mixed-script labels — the rows line up on screen,
which the scalar-counted `strings::padLeft` cannot do:
...
FUNC main() AS Integer
  FOR EACH label IN ["ascii", "日本語", "café"]
    io::print("|" & strings::padLeftToWidth(label, 10) & "|")
  NEXT
  RETURN 0
END FUNC
```
...
FUNC main() AS Integer
  LET padded AS String = strings::padLeftToWidth("x", 6, "😀")
  io::print(toString(strings::displayWidth(padded)))
  io::print(strings::padLeftToWidth("日本語", 4))
  RETURN 0
END FUNC
```"#;
    ...
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            params: vec![
                ...
                Parameter {
                    name: "padChar",
                    desc: "Optional. The fill character prepended toward `columns`; defaults to a single space. Must be exactly one Unicode scalar value occupying at least one column.",
                    aliases: &[],
                    ty: ParameterType::String,
            ...
            body: Body::Rewrite("__strings_padLeftToWidth"),
        }],
    });
}
