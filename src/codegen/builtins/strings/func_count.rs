//! `strings::count` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.count` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.count", args)?
        .ok_or_else(|| "strings.count: no native lowering for these arguments".to_string())
}
