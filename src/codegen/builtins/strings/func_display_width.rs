...
/// Target-generic native lowering for `strings.displayWidth` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.displayWidth", args)?
        .ok_or_else(|| "strings.displayWidth: no native lowering for these arguments".to_string())
}
