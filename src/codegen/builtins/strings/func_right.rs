//! `strings::right` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.right` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.right", args)?
        .ok_or_else(|| "strings.right: no native lowering for these arguments".to_string())
}
