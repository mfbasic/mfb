//! `strings::split` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.split` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.split", args)?
        .ok_or_else(|| "strings.split: no native lowering for these arguments".to_string())
}
