//! `strings::stripSuffix` — descriptor + native-lowering wrapper.
//!
...
/// Target-generic native lowering for `strings.stripSuffix` (registry `Body::Native`
...
pub(crate) fn lower(builder: &mut CodeBuilder, args: &[NirValue]) -> Result<ValueResult, String> {
    builder
        .lower_strings_package_call("strings.stripSuffix", args)?
        .ok_or_else(|| "strings.stripSuffix: no native lowering for these arguments".to_string())
}
