...
//! seven `Body::Rewrite` members.
//!
//! plan-118-B: the predicates used to depend on a 4,099-arm `__strings_genCat`
//! IF-chain appended here at registration — the SAME generated table `regex`
//! embeds, renamed, and compiled a second time at another 1,057,783 machine
//! instructions. They now call `strings::genCat`, which looks the scalar up in a
//! rodata run table shared with `regex`, so the table is neither duplicated nor
//! compiled at all.
//!
//! Why this is still a gated helper chunk and not seven `Body::mfb` bodies on the
//! member descriptors: a `Body::Mfb` body renders into `get_mfb` for EVERY
//! `IMPORT strings` program, and the `WhenUsed` gate exists so the seam is
//! compiled only when a program references its members. A gated chunk is its own
//! synthetic file (`<builtin-strings>`) and injected FUNCs are file-local, so the
//! seam and the predicates must stay together in one chunk. A second gate — same
...
REM classification predicates. The general categories come from strings::genCat,
REM an internal native member reading the shared rodata run table (plan-118-B);
REM this chunk used to carry a whole renamed copy of the regex package's
REM generated 4099-arm category table so the two never collided.
...
' digit, or Unicode space), skipping the lookup and its String compare on the hot
' path. cp >= 0x80 keeps the genCat path.
FUNC __strings_isLetter(sc AS Scalar) AS Boolean
  LET cp AS Integer = toInt(sc)
  IF cp < 128 THEN
    RETURN (cp >= 65 AND cp <= 90) OR (cp >= 97 AND cp <= 122)
  END IF
  LET cat AS String = strings::genCat(cp)
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    // plan-118-B: the chunk is now just `SEAM`. It used to be `SEAM` concatenated
    // with a whole renamed copy of the generated category table, leaked to
    // `'static` because the concatenation happens at registry build time.
