...
pub(crate) fn register(r: &mut Registry) {
    ...
    pkg.add_constant(constant("ErrUnknown", "77050000", "Unclassified standard-package failure.", "_mfb_str_error_unknown"))
        ...
        .add_constant(constant("ErrBadFontFile", "77050022", "File is not a font this build can read: it must be TrueType outlines (sfnt `0x00010000` or `true`, alone or as a face of a collection), not CFF/OpenType-PostScript or WOFF, and its `head.unitsPerEm` must be within 16..16384.", "_mfb_str_error_bad_font_file"))
