//! `term::getUnderline` — abi_function member (native terminal I/O).
//!
//! Registers its own [`lower_get_underline`] `Body::abi_function`
...
const INTRO: &str = r#"Report whether the underline attribute is currently set"#;

const DESC: &str = r#"`term::getUnderline` returns `TRUE` when subsequently drawn text will be
...
END SUB
```"#;
/// `abi_function` body for `term::get_underline` — delegates to the shared family-generic
...
/// layer).
pub(crate) fn lower_get_underline(
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            ...
            errors: vec![],
            body: Body::abi_function(lower_get_underline),
        }],
    });
}
