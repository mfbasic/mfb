...
use crate::codegen::builtins::vector::{COS_COEFFS, EXP_COEFFS, LOG_COEFFS, SIN_COEFFS};
...
pub(crate) const MATH_CONST_POOL_SYMBOL: &str = "_mfb_math_const_pool";
...
pub(crate) fn math_const_pool_words() -> Vec<u64> {
...
pub(crate) fn math_const_pool_data_value() -> String {
    use std::fmt::Write;
...
impl CodeBuilder<'_> {
    ...
    pub(crate) fn lower_simd_float_unary(
        &mut self,
        kernel: FloatKernel,
        input: ValueResult,
        text: String,
    ...
    /// `exp`/`sin`/`cos`.
    pub(crate) fn lower_simd_float_scalar(
        &mut self,
        kernel: FloatKernel,
    ...
    fn emit_atan_core(&mut self, k: &KernelRegs) {
        ...
        self.broadcast_f64(abi::VEC_SCRATCH[3], ATAN_LO[0]);
        ...
        self.broadcast_f64(abi::VEC_SCRATCH[3], ATAN_LO[1]);
        ...
        self.broadcast_f64(abi::VEC_SCRATCH[3], ATAN_LO[2]);
    ...
    fn emit_sin_cos_body_scalar(&mut self, want_cos: bool, k: &KernelRegs) {
        ...
        // bit0 set.
        if !want_cos {
            self.emit_cos_r_into(abi::VEC_SCRATCH[0], k);
        } else {
            self.emit_sin_r_into(abi::VEC_SCRATCH[0], k);
        }
    ...
    fn emit_compensated_horner(
        ...
        for i in (0..coeffs.len() - 1).rev() {
            ...
            self.emit_twoprod(abi::VEC_SCRATCH[7], &k.v28, hi, var);
    ...
    /// spilled them).
    pub(crate) fn lower_simd_float_binary(
        &mut self,
    ...
    pub(crate) fn lower_simd_float_binary_scalar(
        &mut self,
        kernel: FloatBinaryKernel,
...
#[derive(Clone, Copy, PartialEq, Eq)]
pub(crate) enum FloatBinaryKernel {
    /// `atan2(y, x)`.
    Atan2,
}
