...
impl CodeBuilder<'_> {
    ...
    pub(crate) fn lower_simd_sqrt_fixed(
        &mut self,
        input: ValueResult,
        text: String,
        ...
        self.emit_collection_data_pointer_for(&in_data, &in_ptr, &ParameterType::Fixed);
        let out_data = self.allocate_register();
        self.emit_collection_data_pointer_for(&out_data, &result_base, &ParameterType::Fixed);
    ...
    pub(crate) fn lower_simd_log_fixed(
        &mut self,
        input: ValueResult,
        base10: bool,
        text: String,
        ...
        let result_base =
            self.emit_alloc_result_list(&count, &COLLECTION_TYPE_FIXED.to_string(), "simd_fxlog")?;
