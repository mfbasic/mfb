//! `vector::max` — descriptor entry + the per-type `__vector_max_*`
...
const INTRO: &str = r#"Component-wise maximum of two vectors"#;

const DESC: &str = r#"`vector::max` returns a new vector whose every component is the larger of the two
...
vector is generally not equal to either input: `max(vector::Float2[2.0, 3.0], vector::Float2[4.0, 1.0])`
...
const EX: &str = r#"The component-wise maximum of two 2D vectors:
...
r#"FUNC __vector_max_float2(a AS Float2, b AS Float2) AS Float2
  LET cx AS Float = math::max(a.x, b.x)
  LET cy AS Float = math::max(a.y, b.y)
  RETURN Float2[cx, cy]
END FUNC"#;
#[rustfmt::skip]
const BODY_FLOAT3: &str =
r#"FUNC __vector_max_float3(a AS Float3, b AS Float3) AS Float3
  LET cx AS Float = math::max(a.x, b.x)
  LET cy AS Float = math::max(a.y, b.y)
  LET cz AS Float = math::max(a.z, b.z)
  RETURN Float3[cx, cy, cz]
END FUNC"#;
#[rustfmt::skip]
const BODY_FLOAT4: &str =
r#"FUNC __vector_max_float4(a AS Float4, b AS Float4) AS Float4
  LET cx AS Float = math::max(a.x, b.x)
  LET cy AS Float = math::max(a.y, b.y)
  LET cz AS Float = math::max(a.z, b.z)
  LET cw AS Float = math::max(a.w, b.w)
  RETURN Float4[cx, cy, cz, cw]
END FUNC"#;
#[rustfmt::skip]
const BODY_FIXED2: &str =
r#"FUNC __vector_max_fixed2(a AS Fixed2, b AS Fixed2) AS Fixed2
  LET cx AS Fixed = math::max(a.x, b.x)
  LET cy AS Fixed = math::max(a.y, b.y)
  RETURN Fixed2[cx, cy]
END FUNC"#;
#[rustfmt::skip]
const BODY_FIXED3: &str =
r#"FUNC __vector_max_fixed3(a AS Fixed3, b AS Fixed3) AS Fixed3
  LET cx AS Fixed = math::max(a.x, b.x)
  LET cy AS Fixed = math::max(a.y, b.y)
  LET cz AS Fixed = math::max(a.z, b.z)
  RETURN Fixed3[cx, cy, cz]
END FUNC"#;
#[rustfmt::skip]
const BODY_FIXED4: &str =
r#"FUNC __vector_max_fixed4(a AS Fixed4, b AS Fixed4) AS Fixed4
  LET cx AS Fixed = math::max(a.x, b.x)
  LET cy AS Fixed = math::max(a.y, b.y)
  LET cz AS Fixed = math::max(a.z, b.z)
  LET cw AS Fixed = math::max(a.w, b.w)
  RETURN Fixed4[cx, cy, cz, cw]
END FUNC"#;
#[rustfmt::skip]
const BODY_INTEGER2: &str =
r#"FUNC __vector_max_integer2(a AS Integer2, b AS Integer2) AS Integer2
  LET cx AS Integer = math::max(a.x, b.x)
  LET cy AS Integer = math::max(a.y, b.y)
  RETURN Integer2[cx, cy]
END FUNC"#;
#[rustfmt::skip]
const BODY_INTEGER3: &str =
r#"FUNC __vector_max_integer3(a AS Integer3, b AS Integer3) AS Integer3
  LET cx AS Integer = math::max(a.x, b.x)
  LET cy AS Integer = math::max(a.y, b.y)
  LET cz AS Integer = math::max(a.z, b.z)
  RETURN Integer3[cx, cy, cz]
END FUNC"#;
#[rustfmt::skip]
const BODY_INTEGER4: &str =
r#"FUNC __vector_max_integer4(a AS Integer4, b AS Integer4) AS Integer4
  LET cx AS Integer = math::max(a.x, b.x)
  LET cy AS Integer = math::max(a.y, b.y)
  LET cz AS Integer = math::max(a.z, b.z)
  LET cw AS Integer = math::max(a.w, b.w)
  RETURN Integer4[cx, cy, cz, cw]
END FUNC"#;

/// The `__vector_max_<type>` body for one applicable vector type.
fn body(ty: &str) -> &'static str {
    match ty {
        ...
        other => unreachable!("vector::max has no {other} overload"),
    }
}
