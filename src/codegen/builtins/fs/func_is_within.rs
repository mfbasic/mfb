...
/// `abi_function` body for `fs::isWithin` — calls its per-member `lower_fs_*_helper` emitter and finalizes
...
pub(crate) fn lower_fs_is_within(
    ...
    let (instructions, relocations, stack_size) = super::gen_canonical::lower_fs_is_within_helper(
        &symbol,
