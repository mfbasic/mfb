...
/// `abi_function` body for `fs::readAll` — calls its per-member `lower_fs_*_helper` emitter and finalizes
/// (crypto/io's clean-room shape).
pub(crate) fn lower_fs_read_all(
    builder: &mut CodeBuilder,
    _args: &[ValueResult],
    ctx: &AbiCtx,
) -> Result<ValueResult, String> {
    let symbol = builder.current_symbol.clone();
    let (instructions, relocations, stack_size) = super::gen_read_write::lower_fs_read_all_helper(
        &symbol,
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            ...
            errors: vec![],
            body: Body::abi_function(lower_fs_read_all),
        }],
    });
}
