...
/// `abi_function` body for `fs::setCurrentDirectory` — calls its per-member `lower_fs_*_helper` emitter and finalizes
/// (crypto/io's clean-room shape).
pub(crate) fn lower_fs_set_current_directory(
    ...
    let (instructions, relocations, stack_size) =
        super::gen_directory::lower_fs_path_operation_helper(
            ...
            ctx.platform,
            crate::codegen::engine::types::FsPathOperation::Chdir,
        )?;
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            ...
            errors: vec![],
            body: Body::abi_function(lower_fs_set_current_directory),
        }],
    });
}
