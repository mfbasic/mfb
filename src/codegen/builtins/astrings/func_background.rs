...
const INTRO: &str = r#"Construct a background-color `Attribute`."#;

const DESC: &str = r#"`background` returns an `Attribute` wrapping the `AttrNumber` with `kind`
...
r#"FUNC __astrings_background(r AS Byte, g AS Byte, b AS Byte) AS Attribute
  RETURN AttrNumber[AttrTypeNumber.Background, __astrings_packColor(r, g, b)]
END FUNC"#;
