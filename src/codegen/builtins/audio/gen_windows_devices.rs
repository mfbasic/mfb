...
fn emit_string_from_wstr(
    ...
    let v15 = vregs.next();
    ins.extend([
        abi::store_u64(wptr, abi::stack_pointer(), CSTR_OFF), // save wide ptr
        abi::move_immediate(&v10, "Integer", "0"),           // len (units)
        abi::compare_immediate(wptr, "0"),
        abi::branch_eq(&len_done),
        abi::label(&len_loop),
        abi::load_u16(&v11, wptr, 0),
        abi::compare_immediate(&v11, "0"),
        abi::branch_eq(&len_done),
        abi::add_immediate(wptr, wptr, 2),
...
pub(crate) fn lower_devices(
    ...
    com_call(SLOT_ENUM_ENDPOINTS, 4, &mut ins, &mut vregs);
    ins.extend([
        abi::compare_immediate(abi::return_register(), "0"),
        abi::branch_lt(&unavailable),
        // coll->GetCount(&count)
    ...
    com_call(SLOT_COLL_GET_COUNT, 2, &mut ins, &mut vregs);
    ins.extend([
        abi::compare_immediate(abi::return_register(), "0"),
        abi::branch_lt(&unavailable),
    ...
    com_call(SLOT_COLL_ITEM, 3, &mut ins, &mut vregs);
    ins.extend([
        abi::compare_immediate(abi::return_register(), "0"),
        abi::branch_lt(&unavailable),
        // dev->GetId(&pId)
    ...
    com_call(SLOT_DEV_GET_ID, 2, &mut ins, &mut vregs);
    // id = string(pId)
    ins.push(abi::load_u64(&v9, abi::stack_pointer(), D_IDRAW));
    emit_string_from_wstr(symbol, "id", DEVID_OFF, &alloc_fail, &mut ins, &mut rel, &mut vregs, v9.as_str());
    ...
    com_call(SLOT_DEV_OPEN_PROPSTORE, 3, &mut ins, &mut vregs);
    ins.extend([
        abi::compare_immediate(abi::return_register(), "0"),
    ...
    com_call(SLOT_PROPS_GET_VALUE, 3, &mut ins, &mut vregs);
    ins.extend([
        abi::compare_immediate(abi::return_register(), "0"),
        abi::branch_lt(&name_from_id),
    ...
    emit_string_from_wstr(symbol, "nm", NAME_OFF, &alloc_fail, &mut ins, &mut rel, &mut vregs, v9.as_str());
