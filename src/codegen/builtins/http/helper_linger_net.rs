//! `__http_lingerNet` — shared private helper for the `http` package.
//!
...
SUB __http_lingerNet(RES sock AS tcp::Socket)
  tcp::setReadTimeout(sock, 500)
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    pkg.add_helper(RegistryHelper::always("http_lingerNet", BODY));
}
