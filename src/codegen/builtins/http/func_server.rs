...
const INTRO: &str = r#"Bind a plaintext HTTP/1.1 listening socket and return the `tcp::Listener` that drives the accept loop."#;
...
port from the host, which can be read back with `tcp::localAddress`.
...
#[rustfmt::skip]
const BODY: &str =
r#"FUNC __http_server(port AS Integer, host AS String, backlog AS Integer) AS RES tcp::Listener
...
pub(crate) fn register(pkg: &mut RegistryPackage) {
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            params: vec![
                super::req("port", "The local TCP port to bind. Only the low 16 bits are used (values outside `0..65535` truncate modulo 65536). `0` requests a host-assigned ephemeral port, readable with `tcp::localAddress`.", &[], ParameterType::Integer),
