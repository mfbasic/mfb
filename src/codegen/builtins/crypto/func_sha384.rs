//! `crypto::sha384` — descriptor entry + authored docs.
//!
...
FIPS 180-4, and returns it as a fixed 48-byte (384-bit) `List OF Byte`. It is one
of the four `crypto` hashes — `sha224`, `sha256`, `sha384`, and `sha512` — and is
the 384-bit member of the SHA-2 family (a truncated SHA-512 with distinct initial
values).

The digest is a deterministic function of the input alone: the same message
always produces the same 48 bytes, with no keying, salting, or randomness. The
...
output is **byte-identical on every target** (macOS/Linux, aarch64/x86-64/riscv64)
and uses no platform crypto library. A digest is raw binary, not text; stringify
it with `encoding::hexEncode` or `encoding::base64Encode` to display or store it.

`sha384` is a general-purpose digest and message-integrity primitive. It is
...
SUB main()
  ...
  LET digest AS List OF Byte = crypto::sha384(raw)
...
SUB main()
  io::print(encoding::hexEncode(crypto::sha384("hello")))
END SUB
```"#;
    ...
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![
            Implementation {
                ...
                return_type: bytes(),
                errors: vec![],
                body: Body::Rewrite("__crypto_sha384_bytes"),
            },
            Implementation {
                params: vec![Parameter {
                    name: "data",
                ...
                return_type: bytes(),
                errors: vec![],
                body: Body::Rewrite("__crypto_sha384_text"),
            },
        ],
    });
}
