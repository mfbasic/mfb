//! `crypto::sha256` — descriptor entry + authored docs.
//!
...
const INTRO: &str = r#"Compute the SHA-256 cryptographic hash (FIPS 180-4) of a message."#;
const DESC: &str = r#"`crypto::sha256` computes the SHA-256 message digest of `data`, as specified by
FIPS 180-4, and returns it as a fixed 32-byte (256-bit) `List OF Byte`. It is one
of the four `crypto` hashes — `sha224`, `sha256`, `sha384`, and `sha512` — and is
the 256-bit member of the SHA-2 family.

The digest is a deterministic function of the input alone: the same message
always produces the same 32 bytes, with no keying, salting, or randomness. The
...
`sha256` is a general-purpose digest and message-integrity primitive. It is
...
SUB main()
  io::print(encoding::hexEncode(crypto::sha256("hello")))
END SUB
```"#;
    ...
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![
            Implementation {
                ...
                return_type: bytes(),
                errors: vec![],
                body: Body::Rewrite("__crypto_sha256_bytes"),
            },
            Implementation {
                params: vec![Parameter {
                    name: "data",
                ...
                return_type: bytes(),
                errors: vec![],
                body: Body::Rewrite("__crypto_sha256_text"),
            },
        ],
    });
}
