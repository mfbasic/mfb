...
//! message reaches the same hash-generic HMAC core as raw bytes (a `strings::toBytes`
//! then the bytes path). A `String` and a `List OF Byte` are not ABI-interchangeable,
//! so the two overloads must
