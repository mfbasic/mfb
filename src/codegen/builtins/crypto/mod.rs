...
mod tests {
    ...
    fn crypto_registered_on_the_clean_room_registry() {
        ...
        // 21 members: the unified clean-room `generate(Certificate)` replaced the
        ...
        // The unified hash-generic `hmac`/`hkdf`/`pbkdf2(Hash, …)` add three members
        // alongside the (not-yet-retired) per-digest `hmacSha*`/`hkdfSha*`/`pbkdf2Sha*`.
