...
mod tests {
    ...
    fn crypto_registered_on_the_clean_room_registry() {
        ...
        // 24 members: the unified clean-room `generate`/`sign`/`verify`/`hash`
        // replaced the per-type generate/sign/verify/sha members; `seal`/`open` over
        // `SymmetricCipher` were added (each one member with two overloads); the
        // hash-generic `hmac`/`hkdf`/`pbkdf2(Hash, …)` add three more alongside the
        // (not-yet-retired) per-digest `hmacSha*`/`hkdfSha*`/`pbkdf2Sha*`; and `convert`
        // (Ed25519↔X25519 key conversion) adds one.
