//! `crypto::p521Sign` — descriptor entry + authored docs.
//!
//! A NATIVE member: ECDSA signing on the NIST P-521 curve. Its `Body::native`
//! OS-seam slots point at [`super::native::lower_crypto_ec`], the shared elliptic-curve
//! lowering. Docs migrated from `src/docs/man/builtins/crypto/p521Sign.md`.

use super::{bytes, Body, DefaultValue, Implementation, Parameter, RegistryFunction};

const INTRO: &str = r#"ECDSA-sign a message with a NIST P-521 private key (FIPS 186)."#;
const DESC: &str = r#"`crypto::p521Sign` produces an ECDSA signature over `message` using `privateKey`
    ...
    pkg.add_function(RegistryFunction {
        ...
        implementations: vec![Implementation {
            params: vec![
                Parameter {
                    name: "privateKey",
                    desc: "The P-521 private key bytes.",
                    aliases: &[],
                    ty: bytes(),
