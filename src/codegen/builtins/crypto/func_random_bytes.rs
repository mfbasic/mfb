...
**Range and boundaries.** `count` must satisfy `0 <= count <= 16777216`
(16 MiB). A `count` of `0` returns an empty list; a negative `count` or one
above the 16 MiB cap raises `ErrInvalidArgument` and produces nothing. The check
happens before any work is done.
...
the deliberate contrast with `math::rand`, a fast, seedable random sequence that
is **not** cryptographically secure and must never be used for keys, tokens, or
nonces. Once the returned list is built the working copy of the entropy is
wiped, so nothing later in the program can read the generated bytes back out of
it. When you later compare secret material derived from these bytes (a MAC, a
