...
//! aad)`: RFC 9180 HPKE single-shot base-mode `Seal` — the recipient's Ed25519
//! public key is mapped to the suite's KEM curve (`__crypto_hpkeRecipientPub`), a
//! fresh ephemeral KEM key pair is drawn from `crypto::randomBytes`, and
...
#[rustfmt::skip]
const BODY: &str =
r#"' RFC 9180 HPKE base-mode Seal to an Ed25519 recipient: enc || ct (ct includes the tag).
