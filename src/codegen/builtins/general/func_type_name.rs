...
const INTRO: &str = "The name of a value's runtime type.";
...
It is a diagnostic tool. Use it in a log line, a test message, or while working
out why a value is not the type you expected. It is **not** a way to branch on
type: MFBASIC decides types at compile time, so comparing `typeName` against a
string to choose a code path expresses a decision the language has already made,
and a `MATCH` on a `UNION` says the same thing in a form the compiler checks.
