...
14. An inline `TRAP` may not be attached directly to a call to an **inline-lowered built-in** — the string/collection members (`strings::find`, `mid`, `replace`, `collections::get`, `append`, …), the `bits::*` operations, and `len`/`toString`/`typeName`. These have their machine code spliced in at the call site and own no callable symbol, so the trap form cannot be compiled; it is rejected as `TYPE_INLINE_TRAP_ON_INLINED_BUILTIN`. To handle such a call's error, move it into a `FUNC`/`SUB` and attach the inline `TRAP` to that call instead. (Conversion built-ins like `toInt`, helper-backed built-ins like `fs::*`, and user `FUNC`/`SUB` calls all support inline `TRAP` normally.)
