...
[[src/target/shared/code/mod.rs:lower_program_entry]]
...
entry exits rather than returns. [[src/target/shared/code/mod.rs:lower_program_entry]]
...
[[src/target/shared/code/mod.rs:ARENA_FREE_LIST_HEAD_OFFSET]]
...
state slots) * 8`, rounded up to 16. [[src/target/shared/code/mod.rs:ENTRY_ARGC_OFFSET]]
...
[[src/target/shared/code/mod.rs:TERM_STATE_SLOTS]]
...
arenas are never freed by the entry. [[src/target/shared/code/mod.rs:MAIN_ARENA_GLOBAL_SYMBOL]]
...
[[src/target/shared/code/mod.rs:SIGNAL_HANDLER_SYMBOL]]
...
for normal-exit cleanup. [[src/target/shared/code/mod.rs:lower_signal_handler]]
...
`x0 = x19` and the seed in `x1`. [[src/target/shared/code/mod.rs:RNG_SEED_SYMBOL]]
...
[[src/target/shared/code/mod.rs:ARENA_START_TIME_OFFSET]]
...
1. **Native `LINK` init.** If the program has `LINK` bindings, call the link-init
   ...
   path, so a load failure aborts before `main`. [[src/target/shared/code/mod.rs:lower_program_entry]]
2. **Global initializer.** If present, call it. A `RESULT_PROGRAM_EXIT_TAG`
   ...
   non-`RESULT_OK_TAG` jumps to the error path. [[src/target/shared/code/mod.rs:RESULT_PROGRAM_EXIT_TAG]]
3. **Argument list.** If the language entry accepts args, save argc/argv to their
   ...
   pointer is loaded into `x0` as the entry's argument. [[src/target/shared/code/mod.rs:emit_entry_args_list_materialization]]
...
[[src/target/shared/code/mod.rs:ENTRY_ERROR_PREFIX_SYMBOL]]
...
reloads the exit code, and calls the platform process-exit. [[src/target/shared/code/mod.rs:SHUTDOWN_SYMBOL]]
...
[[src/target/shared/code/mod.rs:lower_shutdown]]
