...
Kind `1` (`FUNCTION_BINARY_REPR`) is the **only** kind the current compiler emits — every lowered function, including ones that wrap imported or native targets, is written as kind `1`. Other kind numbers (imported / native wrapper / built-in reference / package initializer) are not produced by the current encoder.
