...
an existing file (`write_new_file`).[[src/main.rs:write_new_file]]

## `build` Flags

`parse_build_options` parses the flags.[[src/main.rs:parse_build_options]] The
...
against the repository's signing key/fingerprint.[[src/main.rs:load_build_signing_info]]
...
macOS or Linux target`).[[src/main.rs:build_project]] A duplicate `-app` yields
...
any stage failure exits `1`.[[src/main.rs:build_project]] Build-mode and
...
optional `[location]` (file or project dir, default `.`).[[src/main.rs:run_fmt_command]]
...
optional `[location]` (default `.`).[[src/main.rs:run_doc_command]] It renders
...
`run_pkg_command` matches the subcommand by name.[[src/main.rs:run_pkg_command]]
...
`doc.html`).[[src/main.rs:run_pkg_doc]] Each subcommand's arity error and the
...
`auth`.[[src/main.rs:run_repo_command]] `register` prints `Registered owner <o>
...
`verify_packages` prints one line per declared dependency.[[src/main.rs:verify_packages]]
...
expected version always matches.[[src/main.rs:package_dependency_status]] A
...
`package_verify_line`:[[src/main.rs:package_verify_line]]
...
in order.[[src/main.rs:print_package_info]] Every empty string renders as
...
`1`→`Ed25519`, other→`unknown (n)`.[[src/main.rs:signature_type_name]] Export
kinds are mapped by `package_export_kind_name`: `FUNC`, `SUB`, `TYPE`, `UNION`,
`ENUM`.[[src/main.rs:package_export_kind_name]] The `.mfp` header format
...
`--width=N`, then up to two positionals (topic, subtopic).[[src/main.rs:show_spec]]
...
rule);[[src/main.rs:print_spec_all]] with two it renders that exact topic page.
...
classic **80** fallback (also used when stdout is piped).[[src/main.rs:detect_terminal_width]]
`parse_spec_width` clamps `--width` to `[20, 1000]` and rejects non-numeric
values.[[src/main.rs:parse_spec_width]] Color defaults to whether stdout is a
...
cell is escaped (`escape_spec_cell`).[[src/main.rs:print_spec_listing]]
...
page; an unknown package/function or more than two args exits `2`.[[src/main.rs:show_man]]
The `man` listing heading is `TOPICS`/`topic` for the `types` package and
`FUNCTIONS`/`function` otherwise.[[src/main.rs:man_entry_heading]]
