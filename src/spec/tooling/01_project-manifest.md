...
`kind` is a hard error. [[src/main.rs:validate_kind]]
...
defaults rather than erroring. [[src/main.rs:validate_project_manifest]] [[src/main.rs:package_metadata]]
...
package metadata; the camelCase forms are canonical. [[src/main.rs:package_metadata]]
...
error reported as `Source entry #N must be an object.`. [[src/main.rs:validate_sources]] [[src/main.rs:validate_source_pattern_field]]
...
the declared `version`. [[src/main.rs:project_package_dependency]] [[src/main.rs:package_dependencies]] [[src/main.rs:installed_package_files]]
...
rules — each failure emits `PROJECT_ENTRY_INVALID`: [[src/main.rs:validate_entry_point]]
...
`ir::EntryPoint { name, returns, accepts_args }`. [[src/main.rs:validate_entry_point]]

## Loading and Parse Errors

`validate_project_manifest` loads the file and walks the schema in order,
emitting these structural diagnostics before any per-field check: [[src/main.rs:validate_project_manifest]]
...
failed. [[src/main.rs:validate_project_manifest]]
...
| --- | --- | --- | --- |
| `2-200-0001` | `PROJECT_JSON_MISSING` | error | `project.json` does not exist [[src/main.rs:validate_project_manifest]] |
...
| `2-200-0005` | `PROJECT_JSON_REQUIRED_FIELD` | error | a required field (`name`/`version`/`mfb`/`sources`/`kind`/source `root`) is missing [[src/main.rs:validate_required_string]] |
| `2-200-0006` | `PROJECT_JSON_FIELD_TYPE` | error | a field has the wrong type (non-string scalar, non-array `sources`/`include`/`exclude`, non-object source entry) [[src/main.rs:validate_optional_string]] |
| `2-200-0007` | `PROJECT_JSON_EMPTY_FIELD` | error | a required string is empty after trim [[src/main.rs:validate_required_string]] |
| `2-200-0008` | `PROJECT_JSON_EMPTY_SOURCES` | error | `sources` is present but an empty array [[src/main.rs:validate_sources]] |
| `2-200-0009` | `PROJECT_JSON_UNKNOWN_KIND` | warn | `kind` is a string other than `executable`/`package` (non-fatal) [[src/main.rs:validate_kind]] |
| `2-200-0010` | `PROJECT_JSON_VALID` | info | manifest passed validation |
| `2-200-0011` | `PROJECT_ENTRY_INVALID` | error | executable entry-point resolution failed [[src/main.rs:validate_entry_point]] |
...
the field is absent). [[src/main.rs:validate_sources]]
