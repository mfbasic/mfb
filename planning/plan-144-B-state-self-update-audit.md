...
| What | Count | Command |
|---|---|---|
| Rows | §0 total, as measured by plan-144-A Phase 1 | `grep -c '^\| F[1-5]'` over §1 |
...
- [ ] Appendix B.2 of the findings. For each of the 11 `try_inplace_state_*`
...
- [ ] Layer 1 eligibility: record, with a citation, which field types
      `try_inplace_state_scalar_assign` accepts. Check each of plan-144-A's inlined
      classifications against it.
- [ ] Record the whole-record replace path and the bug-644 free condition, and
      ...
      line.
- [ ] Check whether any `StateAssign` reaches `try_inplace_self_update`
      (`self_update.rs:234`), and cite the answer.
- [ ] Record the parameter path: how `RES h` in a callee gets its STATE pointer,
...
- [ ] Build one generated `--ncode` probe in `/tmp/plan-144-probes/state/` with one
...
- [ ] Fill every cell with `y`, `n (<first declining gate or path>)`, or `n/a`
      with a reason. A `y` names every gate it passed. When the reading and the
      dump disagree, record both, and the dump wins.
- [ ] Write §2b: the five non-expressible shapes, each with the probe source and
...
- [ ] §3.1: one paragraph stating when a record-field or `STATE` field
      self-update is in place.
- [ ] §3.2: findings, most consequential first. Each one names the deciding path
...
- [ ] §3.3: the counts, per site and per family, for both tables. They are
      computed by a `summary.py` kept in the appendix, as plan-141 C.7 did, and
      the total must equal rows × (7 + 8).
- [ ] §3.4: contradictions with `.ai/collections.md`, `mfb man variable`,
      plan-141's findings, and the STATE comments in `builder_control.rs`, for the
      fix plan to correct.
- [ ] §3.5: things observed that are not verdicts, such as leaks on the
