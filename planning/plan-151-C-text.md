# plan-151-C: Unicode and attributed text

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-B; complete and verify it before starting this letter.

Populate reviewed per-overload costs for strings, astrings. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-B-collections.md](plan-151-B-collections.md)
...
Files: `src/codegen/builtins/strings/`, `src/codegen/builtins/astrings/`, plus helpers reached from their registrations. Evidence population:
60 literal `RegistryFunction {` occurrences in this group, split
strings=42, astrings=18. These are EDITING-SCOPE counts,
...
for pkg in ['strings', 'astrings']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=strings,astrings MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/unicode/02_strings-model.md
- src/docs/spec/stdlib/15_astrings.md
...
Consult benchmark/mfb/src/string.mfb, strbuild.mfb and width.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase C1 — Review and populate the group

- [ ] Audit each member descriptor and called text/Unicode helpers. Separate byte length, scalar count, grapheme count and display width; define only dimensions that actually control work.
- [ ] Review search, repeated matching, slicing, normalization, case expansion, replacement and concatenation for repeated scans and result copying; include output length. Do not call a character-index lookup constant without tracing how that index reaches a byte position.
- [ ] For attributed text, include text payload and span/run count, overlap/splitting, rendering and result size. Keep internal-only readSpans/writeSpans/scalarLen outside public coverage, while auditing any costs they contribute to public calls.
...
Check: `MFB_COST_PACKAGES=strings,astrings MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase C2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: strings find; strings normalizeNfc; strings displayWidth.
...
- `MFB_COST_PACKAGES=strings,astrings MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope strings astrings` →
