...
- [x] `tests/runtime/rt_compress_bounds.rs` (`common::run_bounded_with_rss`): a 64 MiB+1 zero
      ...
      output ratio ≤ 4.4.
      (`a_decompression_bomb_is_refused_at_the_limit_in_bounded_memory`: a 64 MiB + 1 zero zlib bomb under the
      default `maxBytes` prints `raised 77050027` with peak RSS under 320 MiB — the ceiling derived in the module
      doc from bug-621's rule: last capacity ≤ 1.5 × 64 MiB, worst-case resident geometric sum 3 × 96 MiB, plus 32 MiB;
      with bug-621 open it would be ≈1.2 GiB. `decode_time_is_linear_in_output_size`: 2 MiB vs 8 MiB of text-and-noise
      content, five interleaved in-process rounds, 4n/n median ≤ 4.4. `[[test]] rt_compress_bounds` in `Cargo.toml`.
      `cargo test --test rt_compress_bounds` → `2 passed; 0 failed`.)
- [x] rt-behavior fixture `compress-decode-valid` (small streams generated by Python zlib,
      ...
      (`compress-decode-valid` — raw, zlib, gzip, stored, two members, trailing padding, `ignoreChecksum`, output of
      exactly `maxBytes`; `build.log` read by eye: the 210-byte text three times, `crc32 matches: TRUE`, `stored block`,
      `two members: 420`, `trailing padding ignored: 210`, `ignoreChecksum: 210`, `exact limit: 210`. rt-error:
      `compress-zlib-decode-bad-checksum-invalid` → `7-705-0003` "incorrect data check";
      `compress-zlib-decode-preset-dictionary-invalid` → `7-705-0003` "preset dictionary is not supported";
      `compress-inflate-too-large-invalid` → `7-705-0027`; `compress-inflate-max-bytes-negative-invalid` → `7-705-0002`.
      Streams embedded as hex with the generating command in each source's header comment. `scripts/sync-goldens.sh
      target/release/mfb 'compress-*'` → `synced 22 golden file(s) across 8 test(s)`.)
...
hostile input.
  ...
  (2026-09-14: oracle `decode-raw decode-zlib decode-gzip mutate` → `1059 case(s), 0 failure(s)`, exit 0;
...
- **A builtin call cannot skip a defaulted parameter before a later named one** — `mfb spec language
  ...
  `TYPE_CALL_ARGUMENT_MISMATCH` "(List OF Byte, Boolean)". **(b) was a misreading, corrected the same day:** the
  `TRUE` build (`/tmp/p137bindtrue`) reports BOTH — the located `TYPE_CALL_ARITY_MISMATCH` first, then
  `TYPE_CALL_ARGUMENT_MISMATCH` over the gap-squeezed argument list (`ir::shape` `CalleeParams::Builtin` calls
  `check_builtin_call` on the bound slots after the omission diagnostic); the earlier excerpt showed only the later
