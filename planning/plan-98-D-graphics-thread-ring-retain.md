# plan-98-D: Graphics thread, scene ring, resize handshake, fence-gated retain/release
...
ring, the resize/swapchain-recreation handshake, and — the crown jewel — the
three-thread fence-gated retain/release protocol for resources. It renders **still on
the software backend**: the point is to get threading correct before GPU complexity
lands (design "Get threading right *before* GPU complexity lands"). After D, canvas mode
runs a graphics thread that repaints on vsync/resize/damage with **zero language-program
involvement**, and the retain/release protocol is proven against the exact race the
design names: publish → immediate `image::destroy` → graphics thread mid-record.
...
- plan-98-A invariant 4 (the normative refcount sequence + fence-gated retirement).
...
| B's refcount table uses increment-then-recheck | plan-98-B Phase 4 acceptance met | NOT MET |
...
- **Redraw triggers** are wired: (1) new scene published, (2) resize, (3) OS damage,
  (4) swapchain out-of-date (E/F). **Time is explicitly not a trigger** — a static scene
  costs nothing. Triggers 2–4 repaint with zero worker involvement.
- **Fence-gated retain/release protocol** is implemented and proven: `present()` retains
  (increment-then-recheck, from B); a scene is retired and its refs decremented only when
  the **frame's completion fence signals** (not when it leaves `live`); `image::destroy`
  is safe at any instant.
...
- No change to the scene model bytes, resource table shape, or non-canvas codegen.
...
- **B provides:** the resource table with the increment-then-recheck retain. D adds the
  concurrent releaser (the graphics thread, at fence signal) — which is exactly why B had
  to get the retain sequence right without the releaser present.
...
| Redraw triggers to wire | 4 (present, resize, OS damage, swapchain-stale) | design "Redraw Triggers" |
| Distinct fence/retire races to test | UNMEASURED | enumerate in Phase 1's paper protocol |
...
- UNVERIFIED until Phase 1: the exact set of retire/free orderings that must hold when
  publish, `image::destroy`, and graphics-thread record interleave. The paper protocol
  enumerates them; the tests then drive each deterministically.

## 3. Design Overview

Four layered pieces; the protocol is written before any of them.

0. **Paper protocol (`.ai/` doc) first.** Author the three-thread retain/release ordering
   and the resize handshake as a normative doc (design "Open Items" 1 & 2) before code.
...
4. **Fence-gated retain/release.** Each in-flight frame records which scene (hence which
   resources) it references; on the frame's completion fence, that scene is retired and
   its resource refs decremented (increment-then-recheck retain from B on the publish
   side). `image::destroy` marks dead + bumps generation; free at refcount zero. This is
   the blast-radius piece — land last, behind the race tests.

**Where correctness risk concentrates:** piece 4, precisely at the interleaving the
design names — *scene published, worker immediately calls `image::destroy`, graphics
thread mid-record*. The paper protocol must show: the published scene holds a ref
(retain at publish), so destroy only marks-dead + bumps-gen and cannot free; the
mid-record frame holds the scene until its fence; retirement (and the decrement that could
reach zero and free) happens only at fence signal, on the graphics thread. Tests drive
each ordering deterministically with injected delays/fences.
...
- *Retire on `live` swap.* Rejected (invariant 4): a command buffer/render from the
  swapped-out scene may still be in flight; retire must be fence-gated.
...
- **Changes:** a third OS thread per canvas program; the scene handoff moves from a single
  live slot (B) to a triple-buffer ring; resource retirement becomes fence-gated. All
  internal — no user-visible API change (`present()` semantics are unchanged; it just no
  longer renders inline).
- **Unchanged:** the `DrawItem`/scene bytes, the resource handle shape, `Mode`/gate
...
- [ ] Author `.ai/canvas-threading.md`: the three-thread model; the scene ring state
      ...
      resize handshake; and the retain/release protocol with the increment-then-recheck
      retain and fence-gated retirement spelled out as an ordered sequence.
- [ ] Enumerate the race orderings that must hold (publish vs destroy vs record vs fence),
      each with the invariant that protects it. This list becomes Phase 4's test matrix.
...
- [ ] Tests: a present signals exactly one repaint; a synthetic OS-damage signal repaints
      ...
      assert via a repaint counter over a fixed wall-clock window.

Acceptance: the graphics thread renders C's golden scene off the worker thread; static
scene = zero repaints; damage repaints without the worker. Golden still byte-exact.
Commit: —
...
### Phase 4 — Fence-gated retain/release protocol (largest blast radius last)

- [ ] Each in-flight frame records the scene (and its resource refs) it references; on the
      frame completion fence, retire that scene and decrement its resource refs (retain at
      publish uses B's increment-then-recheck).
- [ ] `image::destroy` mid-frame: mark dead + bump generation; free only at refcount zero;
      a live/in-flight scene keeps it alive.
- [ ] Implement the exact design race deterministically: publish scene → worker calls
      `image::destroy` → graphics thread mid-record — assert no tear, no use-after-free,
      stale-generation access rejected, free happens exactly once at fence.
- [ ] Tests: the full race matrix from Phase 1, each driven deterministically with injected
      fences/delays; a stress test interleaving present/destroy/resize over many frames
...
publish→destroy→mid-record sequence never frees a referenced resource and always frees at
fence; no use-after-free under the stress/sanitizer run. Full `cargo test` green; golden
byte-exact.
Commit: —
...
- Tests: repaint-trigger tests, ring lock-freedom + scene-skip tests, resize-without-worker
  tests, and the deterministic retain/release race matrix + stress run.
- Coverage check: ring/retire logic in the `--bin mfb` denominator via in-process unit
  tests; the graphics-thread loop is exercised by the headless subprocess (uncaptured) —
  cover the ring state machine and the retire/refcount transitions with in-process tests.
- Runtime proof: a headless `--app` program presents, destroys a referenced image
  immediately, resizes, and idles — no crash, correct final frame, resource freed exactly
  once (observable via refcount/free counters exposed to the harness).
...
- **Fence abstraction for the software backend** — recommended: model a software "frame
  fence" as a completion flag the blit sets, so E/F swap in a real GPU fence with the same
  retirement code. (§Design 4)
...
D is where the design's two named highest-risk areas land: the three-thread retain/release
protocol and the resize/swapchain handshake. The discipline is: paper first
(`.ai/canvas-threading.md`), then threading, then the ring, then the fence-gated protocol
behind a deterministic race matrix — all while rendering on C's software backend so
threading correctness is isolated from GPU complexity. With A–D done, canvas mode is a
complete, shippable, GPU-free product; E/F swap the renderer behind an unchanged
thread/ring/retirement boundary.
