...
|---|---|---|
| No open peer work edits the scripts this plan deletes or moves | `git status --short scripts tools tests/gate` → empty | MET (2026-09-12: only `planning/todo.md` modified) |
| Release binary builds at HEAD | `cargo build --release` → exit 0 | MET (`target/release/mfb` present, 11:29 today) |
...
- [ ] `scripts/check-icmp-permission.sh`: change the four `CASE PingStatus.*` arms to
      `CASE net::PingStatus.*`. Re-run the extraction build from §2 → it must compile.
- [ ] Run `bash scripts/check-icmp-permission.sh target/release/mfb` on macOS. Expect `PASS` or a
      stated `SKIP`; record which, with its output line, in this plan.
- [ ] Run `bash scripts/check-net-harness-selftest.sh target/release/mfb`. It must pass all four
...
- [ ] `scripts/test-canvas-vulkan.sh`: set `ROOT` from the script's own location and `cd "$ROOT"`
...
- [ ] `git rm scripts/bug387-gate.sh scripts/typemodel-debug-sweep.sh scripts/ncode-determinism.sh scripts/snap-term.py scripts/tls-secure-transport-version-probe.c scripts/tls-wrap-adoption-probe-macos.c scripts/tls-wrap-adoption-probe-macos-server.c scripts/tls-wrap-adoption-probe-openssl.c`
- [ ] `tests/gate/gate_lock_covers_every_writer.rs`: remove the `bug387-gate.sh` and
      `ncode-determinism.sh` rows from `CLASSIFICATION`, and update the module doc and the comment
      that names `bug387-gate.sh`.
- [ ] Update `SCAN_FLOOR` and its comment. Justification under AGENTS.md's four questions:
...
- [ ] `.gitignore`: remove the `snap-term.py` venv/artifact block (lines 56–58). Keep the
      `snap-macos.py` block.
- [ ] `scripts/README.md`: remove the `bug387-gate.sh` and `ncode-determinism.sh` entries. Make the
      `ncode-determinism-alltargets.sh` entry self-contained, since it currently says "Same
      determinism check as above".
- [ ] `.ai/testing-gates.md`: remove the `bug387-gate.sh` paragraph. Keep the `exe-oracle.sh` text
      until B merges it.
- [ ] Re-run `git grep -n -e bug387-gate -e typemodel-debug-sweep -e 'ncode-determinism\.sh' -e snap-term -e tls-wrap-adoption -e tls-secure-transport -- ':!planning/completed' ':!bugs/completed' ':!planning/plan-131-*'`
      → 0 hits. (`ncode-determinism\.sh` does not match the `-alltargets` name.)
...
- [ ] `tools/codepage-index/README.md`: change the three hyphenated script names to their
...
- [ ] `scripts/fetch_codepage_index.py` docstring: `gen-codepage-tables.py` → `gen_codepage_tables.py`.
- [ ] `tools/math-kernels/capture.sh`, `.github/workflows/coverage.yml:37`: `gen_regex_unicode.py`
      → `gen_unicode_gencat_table.py`.
- [ ] `.gitattributes`: replace the stale entries with the four current generated files
...
- [ ] `.github/workflows/coverage.yml:213`: change the comment to say the ICMP-denied check is
      run by hand (`scripts/check-net-harness-selftest.sh`), not in CI. Whether to wire it into
      CI is an Open Decision in sub-plan E.
- [ ] `scripts/net_blackhole_server.py` docstring: name `check-tcp-connect-timeout.sh`.
- [ ] `scripts/linux-artifact-baseline.sh` header: remove the "zero Linux goldens / uses uname"
      claims, and state what it covers that `artifact-gate.sh` does not (linked `.out`, fixtures
      with no goldens).
- [ ] `.ai/testing-gates.md`: correct the `regen-ncodesum.sh` scope claim (all of `tests/`). Remove
...
- [ ] `.ai/resources-packages.md`: repoint `scripts/check-man-examples.py` to
      `scripts/man-run-examples.sh`, or remove it (read the sentence first).
- [ ] `scripts/test-accept-selftest.sh`: delete section 5 (`classify_argv`) and its summary count.
      Run `bash scripts/test-accept-selftest.sh` → exit 0.
- [ ] `scripts/man-run-examples.sh`: add `--test` to the usage string.
- [ ] Remove the untracked local `scripts/__pycache__/` (it is gitignored, so no commit).
