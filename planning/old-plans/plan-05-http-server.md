# plan-05 — HTTP Server (routing, single-threaded) — overview

Last updated: 2026-06-25
Overall Effort: x-large

**Split plan** (by effort into three small/medium sub-plans; see §F.6). This document holds the
...
Split by effort into three small/medium sub-plans; each holds its phases, tasks, and acceptance.
The design sections above (§F.1–§F.5) and the reuse inventory here are the shared source of truth
all three reference.

| Doc | Effort | Phases | Depends on |
|---|---|---|---|
| [plan-05-A](plan-05-A-prims-shim.md) — TLS prims, net/http shim, compiler wiring | medium | S0 tls server prims · S1 net helpers · S2 http shim · S4 compiler wiring | — |
| [plan-05-B](plan-05-B-server-impl.md) — `.mfb` server implementation | large | S3 routing + multipart + accept/parse/dispatch/respond lifecycle | A |
| [plan-05-C](plan-05-C-docs-tests.md) — man pages, docs, tests | medium | S5 man pages · S6 user docs · S7 golden tests | A, B |
