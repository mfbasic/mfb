...
- **H** → [plan-86-H-vector-inline.md](plan-86-H-vector-inline.md) — **DONE (H1+H2).** **H1 (normalize inline,
  ...
  + a synthetic Float local for the single `len` — no new machinery needed. **H2 (length/distance inline for
  Integer/Fixed)** = correct but MARGINAL (vector int 55.7→55.3 within noise; the software isqrt dominates and
  `length(normalize(a))` keeps FUNC). Both bit-identical (verified vs no-H1 oracle), full artifact-gate 0 diffs.
