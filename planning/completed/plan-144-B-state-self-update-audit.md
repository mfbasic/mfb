...
| What | Count | Command |
|---|---|---|
| Rows | 326 (§0 total, measured by plan-144-A Phase 1) | `grep -c '^\| F[1-5]'` over §1 → 326 |
...
- [x] Create the §2 table
      ...
      — `fill_state.py` over `rows.json` (the same `rows.py` census) → 326 rows.
- [x] Build one generated `--ncode` probe in `/tmp/plan-144-probes/state/` with one
      ...
      — `gen_state.py` (findings C.10) → `2337 called functions, 102 excluded`, and
...
- [x] Fill every cell with `y`, `n (<first declining gate or path>)`, or `n/a`
      ...
      — `fill_state.py` → `rows 326 cells 2608 disagreements 0`. The `y` paths are
...
- [x] §3.2: findings, most consequential first. Each one names the deciding path
      ...
      **record vs `STATE` diff**: every row whose S4 and T2 verdicts differ, with
      ...
      and 13 are `STATE` `n/a`. Finding 2 is the `String` gap: 46 rows, 598 cells,
      with forms shrink 16, same-len 4, grow 8, rewrite 12 and not-derived 4.
- [x] §3.3: the counts, per site and per family, for both tables. They are
      ...
      — findings §3.3 and C.15: §1 = 10 y / 2,006 n / 266 n/a; §2 = 438 y /
      1,808 n / 362 n/a; 4,890 cells.
...
- **B2 — `json::Json` cannot be a `STATE` field.** §2 "What is already known" said
  ...
  §3.4 item 6.
- **B3 — the row count is 326, not "the §0 total as measured".** Following
  plan-144-A Correction A1, §0 has 326 rows, and the §3.3 total is
  326 × 15 = 4,890.
