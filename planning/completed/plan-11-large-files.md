...
Last updated: 2026-06-27 (re-inventoried).
...
**Status: complete (2026-06-27).** Every target file in the inventory below has
been converted from a single `.rs` file into a `foo/` module directory (or, for
the in-place `builder_*`/`code/` siblings, split across new sibling files), as a
pure code-relocation refactor verified by `cargo build` + the full test suite
(179 tests across the binary and all integration crates) after each step. The
`code/mod.rs` hub dropped from 17,583 → 2,621 lines; the largest file in the
tree is now `ir/lower.rs` at ~3,200 (a single cohesive lowering pass the plan
designated as acceptably large). Remaining files over ~1,000 lines are exactly
the ones this plan marked keep-as-is or anticipated would stay large as a
cohesive unit (lowering passes, dispatch matches, the `RULES` data table,
`tls/macos.rs`). Known follow-up (out of this plan's pure-relocation scope): the
`[[src/foo.rs:symbol]]` maintainer citations in `src/spec/**` still point at the
pre-split paths; they are stripped at render time and not validated, so they have
no functional impact, but a future pass should remap them to the new sub-files.

Historical note (pre-work): as of the 2026-06-27 re-inventory, no file had been
split yet. All line counts below were refreshed against the current tree, and the
inventory included nine large files that did not exist (or were under threshold)
when the plan was first written: the app-mode codegen backends, TLS codegen, the
OS linkers, the diagnostics rule table, and several `builder_*` modules. Per-file
line ranges in the split tables are the original planning-time snapshots and
drifted as files grew — they were re-measured before cutting seams.
...
7. `src/target/shared/code/mod.rs` + `builder_misc.rs` + `net.rs` + `tls.rs` +
   `builder_collection_updates.rs` + `builder_collection_layout.rs`
8. `src/target/macos_aarch64/app.rs` + `src/target/linux_aarch64/gtk.rs` (app-mode codegen)
9. `src/os/macos/link.rs` + `src/os/linux/link.rs` (OS linkers)
10. `src/arch/aarch64/encode.rs`
11. `src/audit/collect.rs`
12. `src/resolver.rs`, `src/monomorph.rs`, `src/rules.rs`,
    `src/target/shared/code/{builder_control,builder_math,link_thunk}.rs` (lighter splits)

Files left as-is (already cohesive; revisit only if they grow):
`validate.rs` (1,605), `builder_strings.rs` (1,405), `builder_values.rs`
(1,375), `builder_numeric.rs` (1,266), `builder_collection_queries.rs` (1,108).

(`builder_collection_layout.rs` was on this list at 1,112 lines; it has since
grown to 1,908 and now warrants a split — see below.)
...
| Lines | File | Plan |
|------:|------|------|
| 17,583 | `src/target/shared/code/mod.rs` | split → `code/` submodules |
| 7,839 | `src/typecheck.rs` | split → `typecheck/` |
| 6,419 | `src/ir.rs` | split → `ir/` |
| 6,108 | `src/ast.rs` | split → `ast/` |
| 4,309 | `src/binary_repr.rs` | split → `binary_repr/` |
| 3,861 | `src/target/macos_aarch64/app.rs` | split → `app/` |
| 3,474 | `src/target/shared/code/builder_misc.rs` | split → 4 files |
| 3,458 | `src/main.rs` | split → `cli/` + `manifest/` |
| 3,257 | `src/target/shared/code/tls.rs` | split → `tls/` |
| 2,962 | `src/target/shared/code/builder_collection_updates.rs` | split → mutate/query |
| 2,570 | `src/target/shared/code/builder_strings_package.rs` | split (was optional) |
| 2,524 | `src/target/shared/code/net.rs` | split → 2–3 files |
| 2,411 | `src/target/shared/runtime.rs` | split → `runtime/` (per category) |
| 2,191 | `src/target/linux_aarch64/gtk.rs` | split → `gtk/` |
| 2,168 | `src/target/shared/plan.rs` | split → `plan/` |
| 2,027 | `src/resolver.rs` | light split → `resolver/` |
| 2,016 | `src/monomorph.rs` | light split → `monomorph/` |
| 1,908 | `src/target/shared/code/builder_collection_layout.rs` | split (was keep-as-is) |
| 1,840 | `src/os/macos/link.rs` | split → `link/` (Mach-O) |
| 1,689 | `src/target/shared/nir.rs` | split → `nir/` |
| 1,677 | `src/arch/aarch64/encode.rs` | split → `encode/` |
| 1,605 | `src/target/shared/validate.rs` | **keep as-is** |
| 1,434 | `src/target/shared/code/builder_control.rs` | split → light split |
| 1,405 | `src/target/shared/code/builder_strings.rs` | **keep as-is** |
| 1,375 | `src/target/shared/code/builder_values.rs` | **keep as-is** |
| 1,266 | `src/target/shared/code/builder_numeric.rs` | **keep as-is** |
| 1,171 | `src/rules.rs` | light split → (data table) |
| 1,110 | `src/os/linux/link.rs` | split → `link/` (ELF) |
| 1,108 | `src/target/shared/code/builder_collection_queries.rs` | **keep as-is** |
| 1,070 | `src/audit/collect.rs` | split → `collect/` |
| 1,069 | `src/target/shared/code/builder_math.rs` | split → keep-as-is / light split |
| 1,009 | `src/target/shared/code/link_thunk.rs` | split → keep-as-is / light split |

---

# Per-file split plans

## `src/ir.rs` → `src/ir/` (6,419 lines)
...
## `src/ast.rs` → `src/ast/` (6,108 lines)
...
## `src/typecheck.rs` → `src/typecheck/` (7,839 lines)
...
## `src/binary_repr.rs` → `src/binary_repr/` (4,309 lines)
...
## `src/main.rs` → thin `main.rs` + `src/cli/` + `src/manifest/` (3,458 lines)
...
## `src/target/shared/plan.rs` → `plan/` (2,168 lines)
...
## `src/target/shared/runtime.rs` → `runtime/` (2,411 lines)
...
## `src/target/shared/nir.rs` → `nir/` (1,689 lines)
...
## `src/target/shared/code/mod.rs` → `code/` submodules (17,583 lines)
...
### `code/builder_misc.rs` → 4 files (3,474 lines)
...
### `code/net.rs` → 2–3 files (2,524 lines)

Grew ~880 lines since planning; the original two-way cut still holds, but
the IO half is now large enough to warrant its own poll/timeout sibling.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `net_helpers.rs` | ~500 | Shared emitters + TCP connect/listen establishment. |
| `net_io.rs` | ~1,100 | Socket IO: accept/read/write + DNS lookup. |
| `net_poll.rs` | ~900 | Poll/timeout/select machinery (split out if `net_io.rs` stays >1,500). |

### `code/builder_collection_updates.rs` → 2 files (2,962 lines)
...
### `code/builder_strings_package.rs` → 2 files (2,570 lines)

No longer optional — it has grown ~1,130 lines past the planning snapshot.
Extract the dominant `lower_strings_package_call` dispatch and its per-builtin
emitters into `builder_strings_builtins.rs`, leaving shared helpers in a
thinner `builder_strings_package.rs`.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `builder_strings_package.rs` | ~700 | Dispatch entry + shared string/format helpers. |
| `builder_strings_builtins.rs` | ~1,870 | Per-builtin lowering (`lower_strings_*`). |

### `code/builder_collection_layout.rs` → 2 files (1,908 lines)

Was on the keep-as-is list at 1,112 lines; now mixes flat/pointer payload
analysis with comparison/match-branch emission. Split along that line:

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `builder_collection_layout.rs` | ~900 | Payload-type analysis (`is_pointer_collection_payload_type`, `collection_payload_types`, `type_is_flat_inner`). |
| `builder_collection_compare.rs` | ~1,000 | Comparable-value match-branch emission (`emit_comparable_values_match_branch_*`). |

### `code/tls.rs` → `tls/` (3,257 lines)

TLS codegen: an OpenSSL dlopen/dlsym path plus a large macOS
Network.framework submodule (`mod macos`, ~1,600 lines). The submodule is the
obvious first cut.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `tls/mod.rs` | ~300 | Shared emit helpers (`emit_data_address`/`emit_alloc`/`emit_cstring`/`emit_dlsym`), `frame`, public surface. |
| `tls/openssl.rs` | ~1,300 | OpenSSL dlopen/dlsym + socket-timeout TLS path. |
| `tls/macos.rs` | ~1,650 | The `mod macos` Network.framework backend (move out of the inline `mod`). |

## `src/target/macos_aarch64/app.rs` → `app/` (3,861 lines)

macOS app-mode codegen (new since the original plan). Emits the
NSApplication bootstrap, the worker shim, and a hand-written `TermView`
backend (draw/init/clear/scroll/write/keydown) plus the `app::`/`term::` IO
emitters. A shared `Asm` helper threads through everything.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `app/mod.rs` | ~150 | Public entry, the `Asm` helper + small shared emitters (`hex_cstring`, `emit_double_immediate`). |
| `app/bootstrap.rs` | ~800 | `emit_main_bootstrap`, worker shim, append/finish/launch/terminate delegate helpers. |
| `app/term_view.rs` | ~1,500 | TermView: draw_rect, init/clear/scroll/write_string, key_down, is_flipped, color-from-packed, state stores. |
| `app/app_io.rs` | ~900 | `emit_app_*` IO ops (set_color/set_attr/move_to/clear/cursor/terminal_size). |

## `src/target/linux_aarch64/gtk.rs` → `gtk/` (2,191 lines)

Linux GTK4 app-mode codegen (new). Same shape as the macOS backend:
bootstrap + signal handlers, a Cairo `TermView` draw path, and `app::term::`
IO emitters, over a shared `Asm` helper.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `gtk/mod.rs` | ~250 | `lib_for`, the `Asm` helper, public entry. |
| `gtk/bootstrap.rs` | ~700 | libc-start trampoline, main bootstrap, activate/key-pressed/window-closed handlers, worker shim, append/finish helpers. |
| `gtk/term_draw.rs` | ~750 | Cairo draw/scroll/init/write + cell-geometry helpers + idle show/hide/redraw. |
| `gtk/app_io.rs` | ~500 | `emit_app_term_*` IO ops (terminal_size/set_color/set_attr/...). |

## `src/os/macos/link.rs` → `link/` (1,840 lines)

Mach-O executable encoder/linker (new). Relocation patching + import stubs,
the Mach-O encoder, segment/section/load-command emitters, and the
`.app` Info.plist.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `link/mod.rs` | ~250 | Public `link`/`write_executable_file`, relocation patching, import-stub append, `app_info_plist`/`plist_escape`. |
| `link/macho.rs` | ~700 | `encode_mach_o`/`encode_unsigned_mach_o`, `MachOLayout`, layout/offset computation. |
| `link/commands.rs` | ~700 | Segment/section/load-command emitters (text/data_const/data/sign, symtab/dysymtab/dylinker/dylib/dyld_info/uuid). |

## `src/os/linux/link.rs` → `link/` (1,110 lines)

ELF executable encoder/linker (new). Mirrors the Mach-O side: relocations +
stubs, static/dynamic ELF encoders, the `DynamicPayload` builder, and a test
block.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `link/mod.rs` | ~300 | Public entry, relocation patching, import-stub append, `symbol_vmaddr`, byte/align util. |
| `link/elf.rs` | ~650 | `encode_static_elf`/`encode_dynamic_elf`, section/program headers, signing section, `DynamicPayload`. |
| `link/tests.rs` | ~240 | The `#[cfg(test)]` ELF golden tests. |

## `src/arch/aarch64/encode.rs` → `encode/` (1,677 lines)
...
## `src/audit/collect.rs` → `collect/` (1,070 lines)
...
### `src/resolver.rs` → `resolver/` (2,027 lines)
...
### `src/monomorph.rs` → `monomorph/` (2,016 lines)
...
### `src/rules.rs` → `rules/` (1,171 lines)

Almost entirely a static `Rule` table (~1,080 lines of data) with a thin
display layer. Move the data out so the logic is readable.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `rules/mod.rs` | ~120 | `Severity`, `Rule`, `show_diagnostic`/`show_general_diagnostic`, `rule_for`. |
| `rules/table.rs` | ~1,050 | The static `RULES` array. |

### `code/builder_control.rs` → 2 files (1,434 lines)

New since the original plan. The control-flow op lowering
(`lower_ops_inner`) and the in-place mutate-assign optimizations are
separable.

| New file | Lines | Responsibility |
|----------|------:|----------------|
| `builder_control.rs` | ~700 | `lower_ops_inner` + the `nir_*_context` diagnostics helpers. |
| `builder_inplace_assign.rs` | ~730 | `try_inplace_{append,set,prepend,concat}_assign`, `lower_string_self_append_one`. |

### `code/builder_math.rs` (1,069 lines) — keep-as-is for now

New, just over threshold; a single cohesive `impl CodeBuilder` block of
`lower_math_*` emitters. If it grows, split scalar (`lower_math_*`) vs array
(`lower_math_*_array`) into `builder_math_array.rs`.

### `code/link_thunk.rs` (1,009 lines) — keep-as-is for now

New, just over threshold; cohesive LINK-thunk/initializer codegen. Revisit
only if it grows — the natural seam is `lower_link_initializer`/
`lower_link_thunk` vs the `ReturnMarshal`/`emit_*` marshalling helpers.
