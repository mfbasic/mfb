...
| **plan-116-I complete and archived** — `Picture` holds a `RES canvas::Image`, `Text` a `RES canvas::Font`, and `ImageRef`/`FontRef` are gone | `ls planning/completed/plan-116-I-*` → one match; `grep -n 'ImageRef\|FontRef' src/codegen/builtins/canvas/mod.rs` → no type declarations | **MET** (2026-09-05). Both commands pass: `ls planning/completed/plan-116-I-*` → one match, and `ImageRef`/`FontRef` survive only as prose and as the test that asserts neither resolves. The shared box-2228 row that was the last thing holding the archive move came back **green** (**J16**/**I7**), so plan-116-I is complete *and* filed. |
...
- [x] Re-run every row of §Prerequisites and every row of §2's measured table; update
      ...
      **Final state: four of five rows MET.** `plan-116-I complete and archived` went MET
      on 2026-09-05, when the shared box-2228 row came back green and the letter was
      filed. The one row still NOT MET is the **sampler** (**J11**) — a cross-plan
      dependency on plan-98-E/G that this letter explicitly does not need, and whose only
      effect was to make two pixel-level acceptance clauses undiscriminating, which
      §Phases restates as resource-state assertions instead.
...
**MET.** Every number below is from the tree **merged with `main`**, not from this branch
in isolation.
...
| box 2228 RELEASE, scoped | **GREEN** (2026-09-05): `rc=0`, **14 targets ok, 0 FAILED**, on the tree merged with `main`. `--bin mfb` **3824 passed**; `rt_canvas_group_ownership` **11 passed** (so **R17b** ran there too); `rt_canvas_golden` 19, `rt_canvas_rasteriser` 15, `cli_canvas_package` 60, `rt_canvas_font` 19, `rt_canvas_present_deep_copy` 8, and the rest. Decomposed per **J16**/**I7** to the targets whose behaviour can differ by platform — the whole row took ~3h on that one core even scoped. |
