# plan-148-B: System fonts on macOS (CoreText) and the public members

Last updated: 2026-09-21
Effort: large (3h–1d)
Depends on: plan-148-A

Prerequisites: see plan-148-A. Additionally: plan-148-A is archived
(`ls planning/plan-148-A-* 2>/dev/null` → nothing; `ls planning/completed/plan-148-A-*`
...
members on top of it (moved here from plan-148-E — see Corrections):
...
References: plan-148-A; `src/codegen/builtins/audio/gen_macos_shared.rs`
...
- **Non-goal was false: every canvas app, not just a caller, stops building for Linux
  ...
  is a feature-branch-only state (nothing merges before plan-148-E); the Linux and
...
- **The public members moved here from plan-148-E.** `systemFaces` is `internal_only`,
  ...
  replaced by the public `canvas::loadFont(path, face)` overload (plan-148-A
