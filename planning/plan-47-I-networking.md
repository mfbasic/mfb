...
**I2 depends on 47-E** (the lifted socket seam) and I1.
Feature-wide precondition: master §Prerequisites.
Produces: a `net_symbol` chokepoint (I1); the Winsock implementations and `net.*` in
`runtime_calls` (I2). Consumed by 47-J.
...
- `src/target/shared/code/net/{mod,io,poll}.rs` — the shared lowering holding all 37
  symbol literals (§2.1).
- `planning/plan-47-H-threads.md` §Phase 1 — **the technique this sub-plan clones**, at
  38% the scale.
- `planning/plan-47-E-raise-the-posix-seam.md` §3.1 — the socket seam split (keep 6
  portable constants, lift 4 non-portable ones). I2 is its consumer.

## Prerequisites

| Must be true | Command | Status 2026-07-20 |
|---|---|---|
| *(I1 only)* Byte-identity goldens for all four targets | `find tests -path '*/golden/*' -name '*.ncode*' \| while read f; do b="${f##*/}"; b="${b%.*}"; echo "${b##*.}"; done \| sort -u` | **NOT MET — `linux-riscv64` has 0** |
| *(I2)* plan-47-E has landed | `rg -n 'fn emit_classify_socket_error' src/` | **NOT MET** |
| *(I2)* plan-47-D has landed | `ls src/target/win_x86_64/code.rs` | **NOT MET** |
| *(I2)* The Win11 box answers, with outbound network | `ssh -p 2230 test@127.0.0.1 true` | **UNVERIFIED — run it** |
...
- **I1 adds no Windows behavior.** A Windows arm in I1 destroys its byte-identity proof.
- **Do not lift the 6 portable socket constants.** 47-E §3.1 keeps `SOL_SOCKET`,
...
| — portable to Winsock (47-E keeps) | 6 | 47-E §3.1 |
| — not portable (47-E lifts) | 4 | `o_nonblock`, `eagain`, `einprogress`, `emsgsize` |
...
semantic work; the chokepoint (I1) is what makes that visible.
...
| Winsock defines `SOL_SOCKET`/`SO_*` compatibly | **CONFIRMED** | Winsock2 header contract; hence 47-E keeps them |
| `O_NONBLOCK` has no Winsock constant | **CONFIRMED** | non-blocking is `ioctlsocket(FIONBIO)`, a call |
| Winsock errors do not use `errno` | **CONFIRMED** | `WSAGetLastError()`; hence 47-E's `emit_classify_socket_error` |
...
**I1 — the chokepoint (inert, blocks on nothing).** One `net_symbol(intent)` function
maps an intent to a symbol name; all 37 literals route through it. Nothing else changes.
Proof: 0-diff goldens on all four targets. Lands before 47-B alongside F1 and E1.

**I2 — the Winsock arms.** Fill `net_symbol`'s Windows arm (3 renames), consume 47-E's
...
- **(a) In the program entry (47-D's floor), unconditionally.** Simple and always
...
else is a rename or a seam 47-E already built. **I2 Phase 1 is a spike on `WSAStartup`
...
or worse, as success. 47-E's `emit_classify_socket_error` is the fix; I2's job is to make
...
| set non-blocking | `fcntl` | *(handled by 47-E's `emit_set_nonblocking` → `ioctlsocket`)* |
...
- **Changed (shared, I1):** 37 literals route through one chokepoint. Byte-identical for
...
### I2 Phase 3 — the async paths (largest blast radius last)
...
- Tests: I1's proof is byte-identity. I2's is runtime round-trips on the Win11 box.
- Coverage check: I1 edits shared lowering compiled by every backend, and
  `linux-riscv64` has zero goldens (master §Prerequisites row 3), so its 0-diff is
  vacuous. Seed them before I1.
...
1. **`WSAStartup` placement** (§3.2) — recommended (b), conditional on `net.*` being
   advertised, so `hello.exe` pays nothing. Settle it in I2 Phase 1 with a spike;
...
2. **`WSAPoll`'s known behavior gap.** `WSAPoll` historically did not report failed
   ...
   non-blocking-connect test in I2 Phase 3 specifically, and if it bites, use
...
- 2026-07-20 — **The master's classification of G was wrong.** It listed G with D as a
  ...
  38% the scale, and is split into an inert I1 that blocks on nothing.
- 2026-07-20 — **The literal count overstates the semantic work.** Of 15 distinct
  ...
  argument for I1 landing early regardless of when I2 does.
...
spurious success rather than an obvious break — which is why I2 Phase 2 ends with an
...
What is left untouched: the `net::` language surface, TLS (47-J), the 6 portable socket
constants 47-E deliberately keeps, and every other backend's networking.
