...
- [x] Add the UTF-8 validation-and-copy emitter in
...
- [x] Refactor `src/codegen/builtins/os/func_args.rs:lower_args` to call the
  shared primitive for every index after zero while preserving its empty-list
  and argument-order behavior.
- [x] Add valid and error runtime fixtures under `tests/rt-behavior/os/` and
...
- [x] Update the `os::args` registry prose and `src/docs/spec/stdlib/14_os.md`
...
- [x] Add `src/codegen/builtins/os/func_prog.rs` with registry descriptor,
  fallibility declaration, documentation, and a lowering built on the shared
  index-zero path.
- [x] Register `func_prog` in `src/codegen/builtins/os/mod.rs`; update the
...
- [x] Add `tests/rt-behavior/os/func_os_prog_valid/` proving an invocation name
...
- The repository already has a shared, full UTF-8 validator at
  `src/codegen/string/validate/validate_utf8.rs:emit_call_validate_utf8`.
  Phase 1 reuses it after the existing NUL-length scan rather than duplicating
  the scanner in `os/gen_shared.rs`; the `os` lowering now requests the helper
  explicitly. Measured by `rg -n 'emit_call_validate_utf8' src/codegen`.
- The raw-byte proof is a Unix Rust host-runner regression in
  `tests/cli/cli_entry_args.rs`, because fixture goldens cannot encode invalid
  argv bytes or a dynamic `argv[0]`. It covers stray continuation, truncation,
