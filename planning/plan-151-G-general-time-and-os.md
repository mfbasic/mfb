# plan-151-G: Time, operating-system queries and intrinsic functions

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-F; complete and verify it before starting this letter.

Populate reviewed per-overload costs for datetime, os, general, testing. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-F-big-and-crypto.md](plan-151-F-big-and-crypto.md)
...
Files: `src/codegen/builtins/datetime/`, `src/codegen/builtins/os/`, `src/codegen/builtins/general/`, `src/codegen/builtins/testing/`, plus helpers reached from their registrations. Evidence population:
68 literal `RegistryFunction {` occurrences in this group, split
datetime=44, os=20, general=2, testing=2. These are EDITING-SCOPE counts,
...
for pkg in ['datetime', 'os', 'general', 'testing']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=datetime,os,general,testing MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/stdlib/14_os.md
- src/docs/spec/language/18_builtin-functions.md
- src/docs/spec/language/22_test-framework.md
...
Consult benchmark/mfb/src/datetimeb.mfb, convert.mfb and scalarbench.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase G1 — Review and populate the group

- [ ] Update general::member and testing member factories to receive per-member metadata; include all unqualified globals even though top-level man --all excludes these packages.
- [ ] For datetime, distinguish calendar arithmetic, text formatting/parsing, timezone data lookups and external clock queries; define input/output lengths and any search tables that affect scaling.
- [ ] For os, separate environment/process state queries, external provider work and deliberate sleep. For sleep, document negative/zero/positive ms, main versus worker cancellation, and that a requested duration is not a guaranteed return deadline.
- [ ] For general conversions and testing assertions, account for concrete overload/intrinsic forms, printable payloads, formatting on failure and deferred expression handling. Distinguish evaluation of an assertion operand from the assertion helper work rather than classifying it as an ordinary callback parameter.
...
Check: `MFB_COST_PACKAGES=datetime,os,general,testing MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase G2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: os sleep; general toString; testing expectEqual.
...
- `MFB_COST_PACKAGES=datetime,os,general,testing MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope datetime os general testing` →
