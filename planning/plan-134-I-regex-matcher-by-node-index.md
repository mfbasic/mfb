...
- [ ] `__regex_Op`, `__regex_Leaf`, the new `__regex_Program` and `__regex_Choice` records
      (`mod.rs`); `__regex_compile` flattens (a new `__regex_flatten` helper).
- [ ] `__regex_run` by op index with the integer continuation; `__regex_tryAt`,
      `__regex_searchFrom` and the simple-leaf helper take the flat program.
- [ ] Remove the matcher-only records and helpers the census names.
- [ ] Tests: `rt_regex_bounds` passes; every `rt_regex_*` suite passes; the plan-134 value tests
