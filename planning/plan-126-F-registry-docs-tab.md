...
- **Post-merge acceptance.** `main` had advanced well past the fork (e66e594a4); it was merged into `worktree-P-126` cleanly (auto-merged `.ai/build-tooling.md` and `Cargo.toml`, which still lists `[".", "repository", "wire"]`), and the diff of `src/docs/spec`, `.ai` and `planning` from e66e594a4 to `main` adds no `[[…]]` citation into any file this plan moved symbols out of. `cargo fmt --all` then reformatted two files merged from main (import order in `src/codegen/builtins/tls/gen_schannel_tests.rs`, one joined line in `src/optimizer/opt1/globals.rs`; formatting only), committed as 34ec08cfc; the second `repository/Cargo.toml` pass changed nothing. `rustup run 1.96.0 cargo test --no-fail-fast` on the merged tree → exit 0: **164 test binaries, 5,449 passed, 0 failed, 6 ignored**, 0 cargo `error` lines (log `/tmp/p126-postmerge-fulltest.log`). `scripts/artifact-gate.sh target/release/mfb all` on the merged tree, with a freshly built release binary at load average ~8 on 12 cores → **1,431 tests, 1,597 builds, 2,009 goldens checked, 0 diffs** (log `/tmp/p126-postmerge-gate.log`).
