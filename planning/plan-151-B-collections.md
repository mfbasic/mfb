# plan-151-B: Collection access, mutation, copying and callbacks

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-A; complete and verify it before starting this letter.

Populate reviewed per-overload costs for collections. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-A-cost-schema-and-rendering.md](plan-151-A-cost-schema-and-rendering.md)
...
Files: `src/codegen/builtins/collections/`, plus helpers reached from their registrations. Evidence population:
49 literal `RegistryFunction {` occurrences in this group, split
collections=49. These are EDITING-SCOPE counts,
...
Before annotating, run `MFB_COST_PACKAGES=collections MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/language/12_collections.md
- src/docs/spec/memory/05_collections.md
...
Consult benchmark/mfb/src/list.mfb (`test_lf_append`, `test_lf_append_batch`, `test_lf_get`, `test_lf_any`, `test_lf_sortBy`), mapmatrix.mfb, setops.mfb, listchurn.mfb and pipeline.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase B1 — Review and populate the group

- [ ] Read each `func_*.rs:register` and its source BODY or native lowering; follow `gen_list.rs`, `gen_map.rs`, `gen_set.rs`, `gen_mutate.rs`, `gen_slice.rs`, `gen_memory.rs`, and `gen_flow.rs` where called. Follow optimizer-selected paths rather than assuming the descriptor body is the only path.
- [ ] For get/getOr/contains/hasKey/find/set/removeKey: separate List/Map/Set overloads, eligible hash keys versus scans, query-key size, comparison cost, collisions, absent keys, selected-value copying and eager default preparation inside the call. Caller-side evaluation of a default expression remains outside the cost boundary.
- [ ] For append/prepend/insert/set/removeAt/add/remove: distinguish single-item and bulk forms, same-binding update versus separate result, growth versus spare capacity, fixed versus variable-sized payloads, record/STATE fields and target/optimization conditions. Trace helpers and optimizer dispatch before claiming amortized behavior; retain a conservative general case.
- [ ] For HOFs and sort/sortBy: account for input preparation, output/intermediate copies, callback counts, comparison/key-extraction work, early exits and partial-result cleanup on failure. Inspect sort_fast_path/sort_by_fast_path alongside source BODY; never infer complexity solely from the name of the sorting algorithm.
- [ ] For flatten/zip/chunks/window/groupBy/merge and set algebra: define input sizes and total payload/output dimensions independently. Document output-sensitive and nested costs; do not use element count alone for arbitrary-sized values.
...
Check: `MFB_COST_PACKAGES=collections MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase B2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: collections append; collections get; collections any; collections sortBy.
...
- `MFB_COST_PACKAGES=collections MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope collections` →
