# plan-151-E: Fixed-width numeric and vector operations

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-D; complete and verify it before starting this letter.

Populate reviewed per-overload costs for math, bits, vector, money, color. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-D-parsing-and-codecs.md](plan-151-D-parsing-and-codecs.md)
...
Files: `src/codegen/builtins/math/`, `src/codegen/builtins/bits/`, `src/codegen/builtins/vector/`, `src/codegen/builtins/money/`, `src/codegen/builtins/color/`, plus helpers reached from their registrations. Evidence population:
73 literal `RegistryFunction {` occurrences in this group, split
math=6, bits=17, vector=19, money=3, color=28. These are EDITING-SCOPE counts,
...
for pkg in ['math', 'bits', 'vector', 'money', 'color']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=math,bits,vector,money,color MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/stdlib/07_math-rng.md
- src/docs/spec/stdlib/09_vector.md
- src/docs/spec/stdlib/12_bits.md
- src/docs/spec/stdlib/13_money.md
- src/docs/spec/stdlib/18_color.md
...
Consult benchmark/mfb/src/math.mfb, vector.mfb, bits.mfb and mathpipe.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase E1 — Review and populate the group

- [ ] Thread per-member cost arguments through math::overload and vector::implementations/imp; do not stamp every generated overload with the same cost merely because one helper builds them.
- [ ] Distinguish fixed-width scalar work, fixed-dimensional vectors, list/array overloads, variable iteration counts, random generation/rejection loops and formatting/conversion work. Finite machine width does not justify hiding a user-controlled work factor.
- [ ] Trace rounding, checked-domain failures, software kernels and platform/provider paths. State any numerical-iteration bound and initialization work as evidenced; crypto constant-time language is not relevant to Big-O labels here.
- [ ] Move existing performance-only prose into Cost where duplicated; preserve semantic guarantees such as numerical accuracy and rounding in Description.
...
Check: `MFB_COST_PACKAGES=math,bits,vector,money,color MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase E2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: math sqrt; bits band; money round.
...
- `MFB_COST_PACKAGES=math,bits,vector,money,color MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope math bits vector money color` →
