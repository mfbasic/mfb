# plan-151-H: Files, standard streams and child processes

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-G; complete and verify it before starting this letter.

Populate reviewed per-overload costs for fs, io, process. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-G-general-time-and-os.md](plan-151-G-general-time-and-os.md)
...
Files: `src/codegen/builtins/fs/`, `src/codegen/builtins/io/`, `src/codegen/builtins/process/`, plus helpers reached from their registrations. Evidence population:
72 literal `RegistryFunction {` occurrences in this group, split
fs=43, io=15, process=14. These are EDITING-SCOPE counts,
...
for pkg in ['fs', 'io', 'process']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=fs,io,process MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/app/03_console-io.md
...
Consult benchmark/mfb/src/iobench.mfb and main.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase H1 — Review and populate the group

- [ ] Audit payload preparation separately from filesystem/device/process work. Identify buffering, whole-file reads, encoding, directory traversal, output growth and synchronous cleanup; input bytes alone may not bound provider latency.
- [ ] For each waiting operation, state whether progress depends on a device, peer or child, whether a timeout exists, its exact scope and retry behavior. Explicitly distinguish no API deadline from provider-controlled waits.
- [ ] For resource-producing calls and spawn, describe retained handle/process/buffer state; launch cost does not include the entire child runtime. Closing/flushing/waiting must account for their own provider work rather than inherit the open cost.
...
Check: `MFB_COST_PACKAGES=fs,io,process MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase H2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: fs readText; io print; process spawn.
...
- `MFB_COST_PACKAGES=fs,io,process MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope fs io process` →
