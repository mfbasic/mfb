# plan-151-K: Presentation, drawing, terminal and audio costs

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-J; complete and verify it before starting this letter.

Populate reviewed per-overload costs for canvas, audio, app, term. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-J-http-and-threads.md](plan-151-J-http-and-threads.md)
...
Files: `src/codegen/builtins/canvas/`, `src/codegen/builtins/audio/`, `src/codegen/builtins/app/`, `src/codegen/builtins/term/`, plus helpers reached from their registrations. Evidence population:
99 literal `RegistryFunction {` occurrences in this group, split
canvas=60, audio=11, app=2, term=26. These are EDITING-SCOPE counts,
...
for pkg in ['canvas', 'audio', 'app', 'term']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=canvas,audio,app,term MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/app/05_presentation-mode.md
...
Consult Existing canvas/audio/terminal runtime fixtures; no new benchmark program in this feature. Existing measurements can challenge a claim but cannot prove an
...
### Phase K1 — Review and populate the group

- [ ] Audit every currently registered public function; do not copy the old built binary canvas census as the source denominator. Treat source changes since A as new rows requiring review.
- [ ] Separate building scene values from submission, presentation completion and deferred graphics work. Define pixels, draw items, points, glyphs, text bytes and image payload as applicable; include persistent scenes/textures in retained-space prose without exposing backend mechanics in man.
- [ ] For audio, distinguish buffer preparation from device waits, sample/frame/channel units, queue capacity, streaming and closure. A buffer duration is not a bound on all processing latency.
- [ ] For app/term, inspect window/terminal-provider calls and output volume, scene or mode transitions, clearing and text conversion. State platform conditions when the implementation differs; do not infer constant latency from a fixed argument count.
...
Check: `MFB_COST_PACKAGES=canvas,audio,app,term MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase K2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: app setMode; term clear.
...
- `MFB_COST_PACKAGES=canvas,audio,app,term MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope canvas audio app term` →
