...
The **Imp** column is what is **implemented today**, audited 2026-08-24 against the tree (not a wishlist — every mark below cites its code): **Y** = implemented; **~** = partially implemented, with the gap named in the row; blank = not implemented. It is deliberately sparse — **12** rows are `Y` and **11** are `~` out of 189 (`awk -F'|' '/^\| [YN~]/ {gsub(/^ +| +$/,"",$3); if($3=="Y") y++; else if($3=="~") p++; tot++} END {print y, p, tot}' planning/optimizations.md`). Re-audit it with a command, never from memory.
...
| Y   | ~   | Dead-code elimination (DCE)                    | 2 | Opt1 + Opt2 | Remove unused code; simple tree-level DCE in Opt1, precise SSA-based DCE in Opt2. **~ Partial, both seams LANDED at Level 2 (first `-O2` row — the dial now accepts 0–3; default stays `-O1`). Opt1: `src/optimizer/opt1/dce.rs` + the `opt1/plans/reads` name census — removes unused scalar binds/evals whose initializers are provably pure AND trap-free (leaves, comparisons, Boolean logic; arithmetic can raise `ErrOverflow` and an unused Float bind is itself the §4.1 observation boundary, so both stay), fixpoint for chains, scope-blind census so shadowing is conservative. Opt2: `src/optimizer/opt2/dce.rs` + `opt2/plans/mark` — def-use "assume dead, prove live" over the selected stream using the allocator's own effect model (`regalloc::class_models`/`effect`), sweeping the pure ALU whitelist with unused vreg dsts (e.g. the constant folder's stranded `mov_imm` feeders); flag-setters are never candidates, so checked arithmetic survives structurally. The named gap: "precise SSA-based" precision waits on Plan2 SSA — the non-SSA marking keeps every def of a used vreg.** |
| Y   | ~   | Dead-store elimination                         | 2 | Opt2 | Remove stores overwritten before read; needs dataflow/alias info. **~ Partial: LANDED at Level 2 as the block-local `sp`-slot subset — `src/optimizer/opt2/dse.rs`, in the store-to-load-forwarding safety mold (pending-store set; only provably memory-free ALU/compares pass through; any load-overlap, partial overwrite, call, label, branch, or unmodeled op clears state, so mis-modeling can only lose an elimination). Runs before DCE so the stranded value feeders get swept. The named gap: the general dataflow/alias form (cross-block, non-`sp` bases, memory-SSA) waits on Plan2 alias infrastructure.** |
| Y   | ~   | Unreachable code elimination                   | 2 | Opt1 + Opt2 | Prune statically-dead branches in Opt1; prune unreachable CFG blocks in Opt2. **~ Partial: LANDED at Level 2. Opt1: `src/optimizer/opt1/uce.rs` truncates post-terminal statements (after RETURN/FAIL/EXIT PROGRAM/EXIT/CONTINUE, an IF whose both arms terminate, an ELSE-bearing MATCH whose cases all terminate), keeping a function's final RETURN; no trap gate — unreachable code never executes. Runs before tree-DCE so names read only from unreachable code become collectible. Opt2: `src/optimizer/opt2/uce.rs` prunes CFG blocks unreachable from entry, with an over-approximated root set (any label referenced by ANY instruction field keeps its block), labels included. The named gap: the "statically-dead branches" half (constant `IF` conditions) is the branch-simplification row's product and lands with it.** |
...
| Y   | ~   | Store-to-load forwarding                       | 3 | Opt2 | Replace loads with prior stored values; needs alias analysis. (A block-local machine version already exists — `forward_stores_to_loads`, now `src/optimizer/opt2/peephole.rs`, gated as a **Level-1** machine peephole under the "Peephole optimization" row; the L3 entry here is the future alias-analysis-based broadening.) **~ Partial: the block-local machine version ships as an L1 peephole; this L3 row is the alias-analysis broadening, which is net-new.** |
...
| Y   | ~   | Bounds-check elimination                       | 3 | Opt2 | Remove provably-safe bounds checks; range analysis on SSA. (Real MFB checks raising `ErrIndexOutOfRange`; embryonic BCE already exists — `func_get.rs:152`, plan-86.) **~ Partial: plan-86 G1 `is_provable_index_access` / `provable_index_locals` (`builtins/collections/func_get.rs:152`) — embryonic, loop-index shapes only.** |
