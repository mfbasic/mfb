...
The **Imp** column is what is **implemented today**, audited 2026-08-24 against the tree (not a wishlist — every mark below cites its code): **Y** = implemented; **~** = partially implemented, with the gap named in the row; blank = not implemented. It is deliberately sparse — **11** rows are `Y` and **8** are `~` out of 189 (`awk -F'|' '/^\| [YN~]/ {gsub(/^ +| +$/,"",$3); if($3=="Y") y++; else if($3=="~") p++; tot++} END {print y, p, tot}' planning/optimizations.md`). Re-audit it with a command, never from memory.
