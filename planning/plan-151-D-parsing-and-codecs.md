# plan-151-D: Parsing, codecs, compression and patterns

Last updated: 2026-09-23
Effort: large (3h–1d); planning estimate for source review, metadata and rendered verification.
Depends on: plan-151-C; complete and verify it before starting this letter.

Populate reviewed per-overload costs for encoding, csv, json, compress, regex. The outcome is complete,
...
All A prerequisites apply. The preceding letter [plan-151-C-text.md](plan-151-C-text.md)
...
Files: `src/codegen/builtins/encoding/`, `src/codegen/builtins/csv/`, `src/codegen/builtins/json/`, `src/codegen/builtins/compress/`, `src/codegen/builtins/regex/`, plus helpers reached from their registrations. Evidence population:
56 literal `RegistryFunction {` occurrences in this group, split
encoding=30, csv=4, json=5, compress=7, regex=10. These are EDITING-SCOPE counts,
...
for pkg in ['encoding', 'csv', 'json', 'compress', 'regex']:
    print(pkg, sum(p.read_text().count('RegistryFunction {')
                   for p in Path('src/codegen/builtins', pkg).rglob('*.rs')))
COUNT
```

Before annotating, run `MFB_COST_PACKAGES=encoding,csv,json,compress,regex MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture`; it must identify missing rows, not pass an empty
...
- src/docs/spec/stdlib/01_regex.md
- src/docs/spec/stdlib/03_csv.md
- src/docs/spec/stdlib/04_json.md
- src/docs/spec/stdlib/08_encoding.md
- src/docs/spec/stdlib/20_compress.md
...
Consult benchmark/mfb/src/encoding.mfb, serialize.mfb and regexbench.mfb. Existing measurements can challenge a claim but cannot prove an
...
### Phase D1 — Review and populate the group

- [ ] Audit all parser/encoder loops and result construction, including input size, nesting depth, pattern size and expanded output size. Separate validation failures from successful traversal only where it improves an accurate upper bound.
- [ ] For compression, document input/output dimensions and repeated output growth; a small compressed input is not a bound on expanded size. Follow CRC/checksum helpers rather than silently excluding them.
- [ ] For regex, distinguish compile and execute behavior, input/pattern dimensions and captures/replacements. Establish the actual matching algorithm from source before assigning bounds; use ProviderDependent only for work genuinely delegated to a named provider.
- [ ] For JSON callbacks or other callable-bearing arguments, document invocation counts and failure cleanup; template/generic descriptors still need cases for all admitted types.
...
Check: `MFB_COST_PACKAGES=encoding,csv,json,compress,regex MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → no missing public overloads, nonzero denominator, exit 0
(estimate 5–10 min including incremental compilation).
Commit: —

### Phase D2 — Real output and ratcheted coverage
...
- [ ] Build and render each function via `target/debug/mfb man <package> <function>`
      and each package via `--all`; inspect named representative pages: json parse; encoding utf8Encode; compress gzipDecode; regex match.
...
- `MFB_COST_PACKAGES=encoding,csv,json,compress,regex MFB_COST_REQUIRE_COMPLETE=1 cargo test --bin mfb cost_census -- --nocapture` → complete group with actual expanded denominator (estimate 5 min).
- `MFB=./target/debug/mfb scripts/man-census.sh --memory-scope encoding csv json compress regex` →
