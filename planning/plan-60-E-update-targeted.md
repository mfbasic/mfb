...
|---|---|---|
| plan-60-B complete — `confirm` and `apply_manifest_change` exist | `grep -c 'fn confirm' src/cli/mod.rs` → 1 and `grep -c 'fn apply_manifest_change' src/cli/resolve.rs` → 1 | NOT MET at authoring |
| plan-60-C complete — flag parsing and CLI-creatable `pin: false` | `grep -c 'no_pin' src/cli/pkg.rs` → ≥ 1 | NOT MET at authoring |
