...
- [ ] `testing` — descriptor + desugar. DESIGNED (acd5c8c2 report). Trivial: `Body::Intrinsic` × 12 assertions (`expectEqual`/…/`expectTrap`/`expectNTrap`), desugar STAYS at `src/testing/desugar/expect.rs` (invoked `ir/lower.rs:773`), membership via `resolve_package("testing").function(name)`; NO return-oracle rewire (returns Nothing, bypasses aggregate dispatch). Do FIRST (pattern-prover). Adds the man2 unqualified-global skip gate (reused by general).
