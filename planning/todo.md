...
- [~] `astrings` + `term` + `strings` — PAUSED at a real INFRA gap (agent stopped clean, no code changed): registry `augment_project` is WhenImported-ONLY, but the cluster needs USAGE-GATED injection: (1) strings' WhenUsed scalar-seam gate — the ~4k `unicode_gencat.mfb` (SAME table regex imports unconditionally) injects ONLY when 1 of 7 seam members is referenced, else it bloats every `IMPORT strings`; (2) term↔astrings both-imported bridge (`term_astrings_bridge.mfb`). AWAITING user decision: add WhenUsed + AND-gated-bridge injection infra (behavior-preserving, injection subsystem only, regex untouched) vs on-import regression. Feasible otherwise: find/mid/replace collections coupling works as-is (`native_builtin_target`); term/astrings core fine; strings AttributedString Tier-A/B resolver → surviving IR-level rewrite (audio/vector idiom); astrings internal_only members → small RegistryFunction flag.
