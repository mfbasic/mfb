...
- [x] `general` — DONE (ce481f510, merged). LAST package. Relocated the descriptor to `src/codegen/builtins/general/mod.rs` (18 fns, `BUILTIN_FUNCTION_*` ids, `ResolvedCall`, bespoke `resolve_call`, `is_general_call` as bare `matches!`, all classification helpers, `resolve_return_type` + boundary `nominal_return_type`/`arity`). Real-named `general` clean-room package, `mark_unqualified_global()`; calls stay BARE (co-located resolver + `RuntimeHelper::General` bare-name codegen unchanged). Rewired the return oracle BEFORE teardown; `declares_error("general.toX",…)` wired for the `raise_error` debug-assert (conversion error sets carried on the impls). `error`'s empty-overload None-arity modeled via a `general::arity` special-case (one illustrative Intrinsic impl to satisfy add_function's non-empty guard). TWO undocumented bridges also needed: `builtins::arity` + `is_builtin_call` general branches (legacy carried these via `REGISTRY.function(<bare>)` which is gone) — without them arity-mismatch diagnostics regress + 35 tests fail. Repointed 25 man/spec citations (`:GENERAL`→`:resolve_call`), deleted `src/builtins/general.rs`. Count 1→0. 3674 tests, `artifact-gate general` 0 diffs, acceptance 1263/0 byte-identical.

**PHASE 1 COMPLETE — every builtin package migrated onto the clean-room registry; legacy `REGISTRY.modules().len()` == 0.**
