...
`os::executablePath` and the plan-157 family: `os::appResourcePath` (renamed from
`os::resourcePath`), `os::appDataPath`, `os::appCachePath`, `os::userHomePath`,
`os::userDocumentsPath`. plan-157 covers home, data, cache and documents; what remains
here is below.

* `fs::tempDirectory` moves to **`os::tempPath`**.
* ~~Add `os::homePath` (or `os::userPath`).~~ plan-157 adds it as `os::userHomePath`.
