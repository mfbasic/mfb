...
- [~] `astrings` + `term` — remaining cluster. astrings: AttributedString type STAYS (hardcoded); migrate its functions + companion; readSpans/writeSpans/scalarLen → `internal_only` flag on RegistryFunction. term: native OS-seam I/O → term/native/, TermColor/TermSize add_record (read-only), the `term_astrings_bridge.mfb` __term_drawTextAttr → `add_helper(WhenImported("astrings"))`. Do astrings THEN term (bridge needs both). Single self-contained agents (the nested-delegation pattern wiped a shared worktree — do NOT sub-delegate).
