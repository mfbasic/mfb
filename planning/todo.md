...
- [~] `term` — LAST cluster member. Native OS-seam I/O → term/native/, TermColor/TermSize add_record (read-only), LineStyle/FillStyle add_source_types, presentation-mode gate stays shared. The `term_astrings_bridge.mfb` __term_drawTextAttr → `add_helper(RegistryHelper { name:"term_astrings_bridge", gate: WhenImported("astrings"), body })`. Single self-contained agent (NO sub-delegation).
