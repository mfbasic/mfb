...
| `padChar` | `String` | Optional. The fill character prepended to reach `width`; defaults to a single space. Must be exactly one Unicode scalar value. [[src/codegen/registry/mod.rs:call_param_names]] |

## Return value

| Type | Description |
| --- | --- |
| `String` | A new `String` equal to `value` prefixed with enough copies of `padChar` to span `width` scalars, or equal to `value` when it is already that long. [[src/codegen/builtins/strings/mod.rs:register]] |
