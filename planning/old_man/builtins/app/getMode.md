...
[[src/codegen/builtins/app/mod.rs:APP]]
...
flow analysis). [[src/codegen/builtins/app/mod.rs:Mode]]

The `Mode` enum is referenced bare, like every other builtin type: write
`Mode.Console`, not `app::Mode.Console`. [[src/codegen/builtins/app/mod.rs:APP]]

## Parameters

`app::getMode` takes no parameters. [[src/codegen/builtins/app/mod.rs:APP]]

## Return value

| Type | Description |
| --- | --- |
| `Mode` | The presentation mode in effect: `Mode.Console` (the terminal-in-a-window surface, the default) or `Mode.None` (windowless). [[src/codegen/builtins/app/mod.rs:APP]] |
