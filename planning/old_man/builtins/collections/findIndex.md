...
| --- | --- | --- |
| `77050001` | `ErrIndexOutOfRange` | `start` is less than `0` or greater than `len(value)`, checked before the scan begins. [[src/codegen/builtins/errorcode/mod.rs:ErrIndexOutOfRange]] |
| `77050004` | `ErrNotFound` | The scan reaches the end of `value` without `predicate` returning `TRUE`, including the empty scan produced by `start = len(value)`. [[src/codegen/builtins/errorcode/mod.rs:ErrNotFound]] |
