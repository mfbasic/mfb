...
| Type | Description |
| --- | --- |
| `List OF T` | A new list of the same type as `value`, one element longer, with `item` at position `index` and every prior element from `index` onward shifted up by one. Inserting at `0` places `item` first; inserting at `len(value)` places it last. [[src/codegen/registry/mod.rs:resolve_call]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050001` | `ErrIndexOutOfRange` | `index` is negative, or `index` is strictly greater than `len(value)`. The append position `index == len(value)` is valid and does not raise. [[src/codegen/builtins/errorcode/mod.rs:ErrIndexOutOfRange]] [[src/target/shared/code/list_mutate.rs:lower_list_insert_collection]] |
