...
value. [[src/codegen/builtins/http/package.mfb:__http_responseDefault]]
[[src/codegen/builtins/http/func_response_default.rs:responseDefault]]
...
`responseDefault` does not. [[src/codegen/builtins/http/package.mfb:__http_responseWith]]
...
- `ok` is a plain stored field, not a computed one. `WITH resp { status := 500 }`
  ...
  anything downstream reads it. [[src/codegen/builtins/http/package.mfb:__http_responseDefault]]
- `reason` is `"OK"` rather than `""`. The server fills in a status-appropriate
  ...
  the server derive it. [[src/codegen/builtins/http/package.mfb:__http_serializeHead]]
...
`responseDefault` takes no parameters. [[src/codegen/builtins/http/mod.rs:register]]

## Return value

| Type | Description |
| --- | --- |
| `Response` | A new response with `status` `200`, `reason` `"OK"`, `httpVersion` `"1.1"`, `headers` an empty `Map OF String TO String`, `body` the UTF-8 bytes of `"OK"`, and `ok` `TRUE`. [[src/codegen/builtins/http/package.mfb:__http_responseDefault]] |
