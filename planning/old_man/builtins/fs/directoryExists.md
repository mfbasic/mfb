...
`FALSE`. [[src/codegen/builtins/fs/native/paths.rs:lower_fs_kind_exists_helper]]
...
| Type | Description |
| --- | --- |
| `Boolean` | `TRUE` when `path` exists and resolves to a directory; `FALSE` otherwise (missing path, regular file, or any other non-directory entry). [[src/codegen/builtins/fs/mod.rs:register]] |
