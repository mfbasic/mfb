...
[[src/codegen/builtins/fs/native/atomic.rs:lower_fs_read_text_path_helper]]
...
result. [[src/codegen/builtins/fs/native/atomic.rs:lower_fs_read_text_path_helper]]
...
| --- | --- | --- |
| `path` | `String` | The filesystem path of the file to read, as UTF-8 bytes; absolute or relative to the current working directory. Must be non-empty and free of embedded NUL bytes. [[src/codegen/builtins/fs/mod.rs:register]] |

## Return value

| Type | Description |
| --- | --- |
| `String` | The complete contents of the file as a UTF-8 `String`, in file order. An empty file returns an empty `String`. [[src/codegen/builtins/fs/native/atomic.rs:lower_fs_read_text_path_helper]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `path` is empty or contains an embedded NUL byte, so it cannot be turned into a valid NUL-terminated host path. [[src/codegen/builtins/fs/native/atomic.rs:lower_fs_read_text_path_helper]] |
| `77010001` | `ErrOutOfMemory` | The NUL-terminated copy of `path` or the `String` holding the file contents cannot be allocated. [[src/codegen/builtins/errorcode/mod.rs:ErrOutOfMemory]] |
...
| `77020002` | `ErrOutput` | The file cannot be opened for any other host reason not classified above. [[src/codegen/builtins/fs/native/shared.rs:emit_fs_path_errno_error_mapping]] |
| `77020001` | `ErrRead` | Determining the file's length (seek) or reading its bytes fails partway through, before the full contents have been read. [[src/codegen/builtins/fs/native/atomic.rs:ErrReadFailed]] |
| `77020004` | `ErrEncoding` | The bytes read from the file are not valid UTF-8. [[src/codegen/builtins/fs/native/atomic.rs:ErrEncoding]] |
