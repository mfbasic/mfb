...
| `bytes` | `List OF Byte` | The bytes to write, in order, taken verbatim from the list's data region. An empty list produces an empty file at `path`. [[src/codegen/builtins/fs/mod.rs:register]] |
...
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `path` is empty or contains an embedded NUL byte, so it cannot be turned into a valid NUL-terminated host path. [[src/codegen/builtins/fs/native/atomic.rs:lower_fs_atomic_write_helper]] |
...
| `77020002` | `ErrOutput` | Writing, flushing, or closing the temporary file fails, or creating the temporary file or renaming it fails for any other host reason. [[src/codegen/builtins/fs/native/atomic.rs:lower_fs_atomic_write_helper]] |
