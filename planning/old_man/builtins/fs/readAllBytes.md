...
[[src/codegen/builtins/fs/native/io.rs:emit_reconcile_read_buffer]]
...
| Type | Description |
| --- | --- |
| `List OF Byte` | The remaining contents of `file`, from the current position to end of input, in file order, as a `List OF Byte`. When `file` is already at end of input, an empty `List OF Byte` is returned. [[src/codegen/builtins/fs/mod.rs:register]] |
