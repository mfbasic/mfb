...
Representation execution paths. [[src/codegen/builtins/bits/func_sr.rs:lower_bits_sr]]
...
| `count` | `Integer` | The shift amount in bits. Must be in the range `0` to `63` inclusive; any other value raises `ErrInvalidArgument`. [[src/codegen/builtins/bits/func_sr.rs:lower_bits_sr]] |

## Return value

| Type | Description |
| --- | --- |
| `Integer` | `value` shifted right by `count` bits, with vacated high bits zero and bits below bit 0 discarded. Equal to `value` when `count` is `0`. [[src/codegen/builtins/bits/mod.rs:register]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `count` is less than `0` or greater than `63`. [[src/codegen/builtins/bits/func_sr.rs:lower_bits_sr]] [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidArgument]] |
