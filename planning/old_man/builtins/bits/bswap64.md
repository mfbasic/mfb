...
| --- | --- | --- |
| `value` | `Integer` | The value whose eight bytes are reversed. All 64 bits participate in the swap. [[src/codegen/registry/mod.rs:call_param_names]] |

## Return value

| Type | Description |
| --- | --- |
| `Integer` | The fully byte-reversed value, with byte `0` and byte `7` exchanged, byte `1` and byte `6` exchanged, byte `2` and byte `5` exchanged, and byte `3` and byte `4` exchanged. No bits are cleared. [[src/codegen/builtins/bits/mod.rs:register]] |
