...
[[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]]
...
`ErrInvalidArgument`. [[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]]
...
empty list when omitted. [[src/codegen/registry/mod.rs:default_argument_padding]]
...
empty list. [[src/codegen/registry/mod.rs:default_argument_padding]]
...
succeed. [[src/codegen/registry/mod.rs:call_param_names]]

## Parameters

| Parameter | Type | Description |
| --- | --- | --- |
| `key` | `List OF Byte` | The 256-bit AES key. Must be exactly 32 bytes. This value is secret. [[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]] |
| `nonce` | `List OF Byte` | The 96-bit GCM nonce. Must be exactly 12 bytes and unique for every message encrypted under a given key. Not secret; normally transmitted with the ciphertext. [[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]] |
| `plaintext` | `List OF Byte` | The message bytes to encrypt. May be empty. |
| `aad` | `List OF Byte` | Optional additional authenticated data: authenticated but not encrypted. Defaults to the empty list. The same `aad` must be passed to `crypto::aes256GcmOpen`. [[src/codegen/registry/mod.rs:default_argument_padding]] |

## Return value

| Type | Description |
| --- | --- |
| `crypto::Sealed` | A record with two fields: `ciphertext` (a `List OF Byte` the same length as `plaintext`) and `tag` (the 16-byte authentication tag). [[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `key` is not exactly 32 bytes, or `nonce` is not exactly 12 bytes. [[src/codegen/builtins/crypto/package.mfb:__crypto_aes256GcmSeal]] [[src/codegen/builtins/errorcode/mod.rs:ErrInvalidArgument]] |
