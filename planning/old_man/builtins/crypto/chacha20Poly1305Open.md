...
match. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]]
...
rejected. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]]
...
[[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]]
...
| --- | --- | --- |
| `key` | `List OF Byte` | The 256-bit ChaCha20 key. Must be exactly 32 bytes and identical to the sealing key. This value is secret. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]] |
| `nonce` | `List OF Byte` | The 96-bit RFC 8439 nonce. Must be exactly 12 bytes and identical to the nonce used to seal the message. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]] |
...
| Type | Description |
| --- | --- |
| `List OF Byte` | The recovered plaintext, the same length as the original message, returned only when the tag verifies. An empty message recovers an empty list. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]] |

## Errors

| Code | Name | Raised when |
| --- | --- | --- |
| `77050002` | `ErrInvalidArgument` | `key` is not exactly 32 bytes, or `nonce` is not exactly 12 bytes. Both length checks run before verification. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]] |
| `77050016` | `ErrAuthenticationFailed` | The authentication tag does not verify — the `ciphertext`, `tag`, `nonce`, or `aad` was altered or does not belong to this key. No plaintext is returned. [[src/codegen/builtins/crypto/package.mfb:__crypto_chacha20Poly1305Open]] |
