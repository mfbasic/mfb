...
`math::eFixed` is the closest Q32.32 `Fixed` to `e`, which is irrational and has no exact finite representation. Its decimal shorthand is `2.718281828459045`; the stored `Fixed` is that value rounded to the nearest representable Q32.32 step, so it carries less precision than the `Float` form `math::e`. [[src/codegen/builtins/math/mod.rs:constant_value]]
...
| Type | Description |
| --- | --- |
| `Fixed` | The `Fixed` nearest to `e`, approximately `2.718281828459045`. The same value on every reference. [[src/codegen/builtins/math/mod.rs:constant_value]] |
