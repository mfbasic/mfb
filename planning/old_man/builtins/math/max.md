...
The larger of two numeric values of the same type.

## Synopsis

```
math::max(a AS Integer, b AS Integer) AS Integer
math::max(a AS Float, b AS Float) AS Float
math::max(a AS Fixed, b AS Fixed) AS Fixed
math::max(a AS Money, b AS Money) AS Money
math::max(a AS List OF Integer, b AS List OF Integer) AS List OF Integer
math::max(a AS List OF Float, b AS List OF Float) AS List OF Float
math::max(a AS List OF Fixed, b AS List OF Fixed) AS List OF Fixed
```
...
`math::max` returns the larger of `a` and `b`. When the two compare equal, that
...
promoting overload — `math::max(1, 1.0)` does not compile.
[[src/codegen/builtins/math/mod.rs:MATH]]

`Money` stays in its dimension: the larger of two amounts is itself an amount.
`max` is one of the four `math::` functions (`abs`, `min`, `max`, `clamp`) that
...
select. The `Float` overload uses the hardware maximum-number instruction, so
...
Hardware maximum-number select. Never fails; of `+0.0` and `-0.0` it yields
...
Element-wise maximum of two same-length lists of the same element type,
...
| Type | Description |
| --- | --- |
| Same as the arguments | The larger of `a` and `b`. When they are equal, that value. For the array forms, a new list of the same element type and length holding the element-wise maximum. [[src/codegen/builtins/math/mod.rs:MATH]] |
...
`math::max` takes exactly two arguments. [[src/codegen/builtins/math/mod.rs:MATH]] They must
...
Larger of two `Integer`s, and of two `Float`s:
...
SUB main()
  LET value AS Integer = math::max(2, 4)
  LET upper AS Float = math::max(-3.5, 1.0)
  io::print(toString(value))
  io::print(toString(upper))
END SUB
```
...
SUB main()
  LET priciest AS Money = math::max(left := 4.50m, right := 3.25m)
  io::print(toString(priciest))

  LET readings AS List OF Float = [3.0, 9.0]
  LET floors AS List OF Float = [5.0, 1.0]
  LET clipped AS List OF Float = math::max(readings, floors)
  io::print(toString(collections::get(clipped, 0)))
END SUB
```
