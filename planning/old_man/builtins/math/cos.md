...
| --- | --- | --- |
| `value` | `Float` or `Fixed` | The angle whose cosine is taken, in radians. Every value is accepted, but see the note on large magnitudes. This parameter has no alternate name. [[src/codegen/builtins/math/mod.rs:call_param_names]] |
...
| Type | Description |
| --- | --- |
| Same as the argument | The cosine of `value`, in the argument's own type; within `[-1, 1]` for an argument in the accurate range. `cos(0)` is one. The array form returns a new `List OF Float` of the same length. [[src/codegen/builtins/math/mod.rs:MATH]] |
...
`math::cos` takes exactly one argument. [[src/codegen/builtins/math/mod.rs:MATH]] It must be
