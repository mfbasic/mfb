...
| --- | --- | --- |
| `a` | `Instant` | The left operand, a point on the UTC timeline. Its `seconds` field is whole seconds since `1970-01-01T00:00:00Z` (possibly negative) and its `nanos` field is the sub-second remainder. The result is `TRUE` only when `a` falls strictly earlier than `b`. [[src/codegen/builtins/datetime/mod.rs:register]] |
