...
```
vector::min(a AS Float2, b AS Float2) AS Float2
vector::min(a AS Float3, b AS Float3) AS Float3
vector::min(a AS Float4, b AS Float4) AS Float4
vector::min(a AS Fixed2, b AS Fixed2) AS Fixed2
vector::min(a AS Fixed3, b AS Fixed3) AS Fixed3
vector::min(a AS Fixed4, b AS Fixed4) AS Fixed4
vector::min(a AS Integer2, b AS Integer2) AS Integer2
vector::min(a AS Integer3, b AS Integer3) AS Integer3
vector::min(a AS Integer4, b AS Integer4) AS Integer4
```
...
`vector::min` returns a new vector whose every component is the smaller of the two
...
modified. [[src/codegen/builtins/vector/package.mfb:__vector_min_float3]]
...
[[src/codegen/builtins/vector/package.mfb:__vector_min_integer4]]
...
**`vector::min(a AS Float2/Float3/Float4, b AS ...) AS ...`**

Per-component minimum of IEEE double components via the hardware minimum
instruction.

**`vector::min(a AS Fixed2/Fixed3/Fixed4, b AS ...) AS ...`**

Per-component minimum of Q32.32 components by signed 64-bit compare and select.

**`vector::min(a AS Integer2/Integer3/Integer4, b AS ...) AS ...`**

Per-component minimum of 64-bit signed components by compare and select.
...
| Type | Description |
| --- | --- |
| the same type as `a` | A new vector of the same type and dimension whose `i`-th component is the smaller of `a`'s and `b`'s `i`-th components. Equal to an operand only when that operand is dominated by the other in every component. [[src/codegen/builtins/vector/mod.rs:VECTOR]] |
...
The component-wise minimum of two 2D vectors:
