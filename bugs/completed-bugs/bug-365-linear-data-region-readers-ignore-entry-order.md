...
Fixed at the **source**, not at the consumers — this report's fix option 2,
scoped to fixed-width element types, which is plan-57-C's design. Three mutation
sites now keep a fixed-width list's data region in index order, so
`entry[i].valueOffset == i * payloadSize` is an invariant and every linear reader
becomes correct without being touched:

- `lower_list_insert_collection` — the result block is fresh, so it is filled in
  three ordered pieces (`A[0..i)`, `B`, `A[i..n)`) instead of A-verbatim then
  B-verbatim, and the lookup table is written as the identity mapping instead of
  spliced.
- `lower_list_prepend_in_place` — the data region shifts up by one payload
  (backwards, since it overlaps itself) and the payload lands at offset 0, rather
  than the payload going to the data tail and only the entry records shifting.
  New helper `emit_block_copy_backward`.
- `lower_collection_set`'s value path — copies the block and overwrites through
  the stored offset instead of degrading to `removeAt` + `insert`. A fixed-width
  replacement is the same size by definition, so `lower_list_set_in_place`'s
  rebuild branch is unreachable and the write is always the in-place overwrite.

Why option 2 rather than this report's recommended option 1 (probe-and-repack at
each consumer): the audit below found the broken class is **exactly** the
fixed-width one, because `dataBase + i * width` is not even *expressible* for a
variable-width element. So option 1 would have meant delicate register surgery at
~15 sites to fix a class that three sites close completely. It is also strictly
less memory traffic, not a correctness-for-speed trade: for these types the
payload is 1–8 bytes where the entry record the offset-stable scheme moves
instead is 40.

`list_element_is_fixed_width` (`builder_collection_layout.rs`) is the predicate,
with `fixed_width_agrees_with_payload_alignment` pinning its stride against
`collection_payload_alignment_for_code` so the two cannot drift.

### The §Scope audit

Every consumer in the unverified table was checked. All resolved:

| consumer | element type | verdict |
|---|---|---|
| `math::` SIMD overloads | Integer/Float/Fixed | fixed-width — **fixed** (was confirmed broken) |
| `fs::writeBytes`/`writeBytesAtomic`/`readAllBytes`/`writeAllBytes` | Byte | fixed-width — **fixed** (was confirmed broken) |
| `audio::write` | Byte | fixed-width — **fixed** |
| socket send/recv, TLS read/write | Byte | fixed-width — **fixed** |
| `crypto` byte lists | Byte | fixed-width — **fixed** |
| `fs::pathJoin` | String | **was never affected** — reads `entry[i].valueOffset` (`fs_helpers_paths.rs:1736-1740`) |
| `fs::listDirectory` | String | **was never affected** — a *producer*, and it writes entries and data sequentially |

So no variable-width linear reader exists, and the defect is closed in practice.
Variable-width lists deliberately keep the offset-stable scheme — `List OF
String` sort permutes entries without moving data on purpose — and that is now
**written down** rather than assumed, which was this report's stated root cause.

### The contract

*Payload Order* added to `src/docs/spec/memory/05_collections.md`, beside the
existing capacity-vs-count warning. It states the fixed-width ordering guarantee,
the variable-width non-guarantee, that order survives every copy, the two
establish-order idioms, and — the trap that caused this — that
`list_element_padding_alignment` returning 1 guarantees no *gaps*, not *order*.

### Verification

Every line of this report's two reproductions now produces the expected result,
including `fs::writeBytes` persisting `ZABC` rather than `ABCZ`. Beyond the
reproductions, the regression test covers what a naive fix would miss:

- **Overlap**: 64+ element lists, and 70 successive prepends so the backward
  shift runs at every length. A forward copy passes at 1–3 elements and smears
  above that.
- **All three payload widths**: `Byte`=1, `Scalar`=4, `Integer`/`Float`/`Fixed`=8.
  A defect in the 1-byte case is invisible in the 8-byte case and vice versa.
- **The copy path**: a permuted list rebound to a new `LET`, since
  `copy_collection_tight` does not normalize.
- **Combinations**: 30 rounds of interleaved `prepend`/`append`/`removeAt`.
- **All three mutation sites**, including the value-path `set` reached through a
  function parameter so the in-place fast path cannot fire.

Full suite: 3105 Rust unit tests and 1025 acceptance tests pass, with **zero**
golden churn. Zero is the right answer despite this being a codegen change: the
goldens capture AST/IR (above this layer) and behavior, and the only behavior
that changed is the buggy case — which no golden covered, which is precisely why
the bug stayed latent.

One defect was found and fixed during implementation, worth recording because it
was silent: the original trailing `dataLength` store sat *after* the new branch,
so the ordered path wrote `dataLength = count`. Reading the list directly still
worked; only a subsequent *copy* read garbage. A test that only checked the
mutated list would have missed it.

Verified on macos-aarch64. The remaining targets are covered by the same
codegen path with no target-specific code.
