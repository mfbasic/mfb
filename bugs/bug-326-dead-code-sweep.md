...
Status: Open
Regression Test: `cargo check` clean + full acceptance suite. **Removing the
blanket allows is itself the regression guard** — once the file-level
`#![allow(dead_code)]` attributes are gone, the compiler resumes reporting this
entire class, so a re-introduction fails the build rather than sitting unseen.
