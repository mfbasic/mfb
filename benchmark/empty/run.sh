#!/usr/bin/env bash
# Runs the MFBASIC, Python, and C "empty entry function" benchmarks, timing
# each. With no logic, this measures process startup/shutdown time only.
set -euo pipefail
...
# Build the MFBASIC project so empty.out exists.
"$mfb" build "$here/mfb" >/dev/null

# Build the C version at two optimization levels.
cc -O0 -o "$here/c/empty-O0.out" "$here/c/main.c"
cc -O2 -o "$here/c/empty-O2.out" "$here/c/main.c"

echo "empty entry function:"
time_run "mfb"    "$here/mfb/empty.out"
