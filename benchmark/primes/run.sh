#!/usr/bin/env bash
# Runs the MFBASIC and Python "first 100 primes" benchmarks, timing each.
set -euo pipefail
...
# Build the MFBASIC project so primes.out exists.
"$mfb" build "$here/mfb" >/dev/null

# Build the C version at two optimization levels.
cc -O0 -o "$here/c/primes-O0.out" "$here/c/main.c"
cc -O2 -o "$here/c/primes-O2.out" "$here/c/main.c"

echo "first 1000 primes:"
time_run "mfb"    "$here/mfb/primes.out"
