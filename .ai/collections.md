...
- **GOTCHA — copy-insertion does not exist for a non-memcpy-copyable collection** (bug-601). `lower_value_owned` copies only when `is_freeable_flat_value`, so for a list of a recursive type or of a pointer-`String` record (`net::Address`, `udp::Datagram`, `audio::AudioDevice`) `MUT ys = xs` shares `xs`'s block, and no in-place gate checks ownership: `ys = removeAt(ys, 0)` empties `xs` too, and a growing `append` frees it (SIGSEGV). The same missing copy is why that class has no drop (bug-599, bug-536 shape C) — adding one alone double-frees. `json::parse` depends on the in-place `append` over `List OF Json`, so declining the arms for the class is not free.
