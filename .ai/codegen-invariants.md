...
A value that is not `type_is_memcpy_copyable` gets **no owning copy at a bind and no drop anywhere**. The pointer-string records were that class until plan-132 — every such value leaked (bug-599), and a `MUT` copy of a list shared its source so an in-place arm emptied or freed it (bug-601). Recursive types were in it until plan-134 (bug-536 shape C): they now get an independent graph at every store (plan-134-D/E, `needs_graph_copy`) and a graph drop at every owner's end (plan-134-G, `owns_graph`). The elements a collection discards in place (`set`, `removeAt`, `removeKey`) have their graphs freed since plan-134-H. A grown-over buffer or a builtin's intermediate block is freed shallowly: it only ever shares its elements' children. Do not add a drop for such a class on its own: it double-frees the shared block — plan-134-G's own first cut freed a caller's tree through a borrowed `MATCH` view.

## A recursive value's deep copy must never recurse on the native stack

The depth of a recursive value is data, not type: a user tree, a `json::parse` result, or a program's own linked list (a million links in `tools/recursive-value-bench`'s `deep_chain`). The per-type copy functions (`thread_copy_symbol`) used to call each other once per edge, and a chain about 60 000 deep overflowed the 8 MiB stack (plan-134-A §2.1). Since plan-134-B each per-type symbol is a shim into ONE module-level walker, `_mfb_rt_graph_copy` (`src/codegen/memory/arena/graph_copy.rs`), which copies one block at a time and keeps pending edges on an arena work stack.
...
## The MATCH scrutinee temporary is a view, and the move analysis depends on its shape

The `MATCH` desugar binds the scrutinee to a temporary (`Bind $matchN = Local(x)` or `= x.f`) and each case aliases it (`Bind c = UnionExtract(Local $matchN)`). Since plan-134-D an owning store of a recursive value deep-copies unless plan-134-C's analysis (`src/codegen/engine/analysis/last_use.rs`) says otherwise, and that bind IS an owning store. The analysis recognises the shape structurally — a local bound once from `x`/`x.f`, never assigned, read only as a `MATCH` scrutinee or inside `UnionExtract` — as a *view*: it borrows (no copy, `MoveSites::is_borrow`) when its source stays live, or owns the value (a move) when it does not, in which case `c.nxt` through the alias can move too. Without that, every `MATCH` on a recursive parameter copied the whole scrutinee: `regex_repeat` went from 0.27 s to 1.80 s (plan-134-D Corrections).

How to apply: a change to the `MATCH` desugar (another read of `$matchN`, an assignment to it, a non-`UnionExtract` case bind) silently turns the view back into a copying store — re-run `collect_last_use_moves_views_of_a_match_scrutinee` and the regex benchmark. And a borrowed view owns nothing: plan-134-G must not register a drop for it (its source's owner frees the graph).
...
- The regex matcher no longer has one (bug-510): `__regex_run` is an explicit-stack backtracker, so a group repetition costs heap, not native frames. Its bounds are the step budgets and `__REGEX_PENDING_LIMIT`. Since plan-134-I everything it holds is an integer: `__regex_flatten` turns the parse tree into op tables once per compile, a continuation is an index into an append-only table of five-integer frames, and a choice point is eight integers plus a capture snapshot in tables that a pop shrinks by lowering a counter. The bug-510 version held pattern subtrees and continuation chains by value; once plan-134 made recursive values really copied and freed, `rt_regex_bounds`' findAll program went from 1.08 s to 30.57 s. (bug-510's first version kept choice points in a growable `List OF` with side tables of recursive records and died of bug-538; tables of integers cannot alias an element.) Before bug-510 it was CPS recursion guarded by `__REGEX_DEPTH_LIMIT = 600` (~10 frames of ~300 stack slots per group repetition), which made `^(ab)*$` fail on a 200-character input; a "charge by input position" accounting would have been unsound because a frame is a frame whether or not it consumed a character.
