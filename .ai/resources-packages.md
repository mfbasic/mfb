...
A resource-union VALUE is a `{ tag@0, variant-record-ptr@8 }` block — NOT the record itself. A concrete resource value already IS its record (STATE at `+24`). So every union STATE path first loads the variant record ptr from `+8`, then reads/writes `record+24`. Helper `emit_resource_record_ptr(value_ptr, type_)` (`builder_value_semantics.rs`): union → load `+8`, concrete → identity; used by `.state` read (`lower_field_access`), `emit_resource_state_init`, `StateAssign` (`builder_control.rs`). A `MATCH` extract yields the concrete record (loads `+8`), so the extracted variant's `.state` uses the plain concrete path — but the case binding's type string must carry the STATE suffix (`File STATE Cursor`), set in lowering (`ir/lower.rs::match_case_binding` appends `STATE <T>`). `emit_free_resource_state_block` looks like a counter-example and is not: it loads `RESOURCE_OFFSET_STATE` straight off the `resource_slot` it is handed, which would be wrong for a union's 16 B `{tag, record-ptr}` value block — but the union call site passes `payload_slot`, already the variant record pointer loaded from `+8`. The indirection lives in the caller by design; do not "fix" the callee.
...
**A union-typed trap target does not get its `UnionWrap`, and at NIR that is indistinguishable from an alias.** The desugar delivers `$trap_valN` bare (see `.ai/codegen-invariants.md`, the inline-TRAP section), so `RES c AS Chan = udp::bind(…) TRAP` reached NIR as `NirValue::Local` rather than `NirValue::UnionWrap` — the ALIASING shape. `runtime/usage.rs::push_op_helpers` then declared none of the variants' close helpers while `validate/capabilities.rs::collect_bind_types`, which carries no aliasing gate, counted them all used, and the build died on a compiler-internal name. Same declared-vs-used symmetry as the three-places rule above, reached from the other end: a coercion missing in **lowering** surfaces as a capability mismatch in the backend.

**An inline `TRAP` over a SENDABLE resource runs the THREAD hand-over copy even in a thread-free program.** `materialize_current_result` (`memory/arena/builder_arena_transfer.rs`) fed the success value to `copy_resource_to_current_arena`, which allocates a second 96 B record, copies the canonical header into it, and tombstones the source `moved|closed` — correct when a worker hands a record across arenas, wrong when the record is already in THIS arena and nothing else names it. The moved bit then makes `emit_resource_block_reclaim` skip the original, so the producer's record is orphaned, one per bind. Non-sendable resources (`tls::Socket`, audio) already took the pointer-carry arm, which is why a leak of this class looks resource-KIND-specific when the deciding fact is arena ownership (`RawSuccessBlock::OwnedByThisFrame`), not kind.
...
- **A borrowed return under an inline `TRAP` has TWO more places to be right** (bug-648): the Ok-path `Result` wrap (`materialize_current_result`) must carry it by pointer (`RawSuccessBlock::BorrowedResource`) — a copy of a sendable resource runs `copy_resource_to_current_arena`, which tombstones the live element — and the desugar's value-less `MUT $trap_valN` temp must not own it. Its ownership is decided per STORE (`resource/cleanup/trap_ownership.rs`): any lent store (borrowed element, `RECOVER outer`) → a run-time owner flag, SET at the bind because the temp owns the closed default record it materializes, rewritten per store, tested by the old-value drop and the scope drop. "All borrowed → no cleanup" is WRONG: it orphans that default (96 B/iter). And `record_ownership` must judge the temp's record over its OWNED stores only (`Source::Lent`), or the drop closes the default but never frees it. A new borrowed target only needs adding to `target_returns_borrowed_resource`; all three layers read it. A test must poll in a LOOP (or an inner scope): with the list and the trap binding in one function scope a premature close is indistinguishable from the list's own.
...
**A companion may call only `collections`' native members, never its source generics** (`sort`, `distinct`, `sortBy`, … — the members whose body is `Body::Mfb`). The `collections` source is injected in `parse_project`, from the *program's* imports, before any companion exists, so a companion calling `collections::sort` links only when the program also happens to `IMPORT collections`; otherwise the build fails at `NIR call target '#collections_sort' does not resolve` (plan-147-B). The test `no_companion_calls_a_source_generic_collections_member` (`codegen::registry` tests) catches it. Write the loop in the companion (canvas's `__canvas_sortedUnique`) rather than widening the injection, which would add the whole `collections` source to every app of that package.
