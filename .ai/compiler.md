...
- Some runtime behavior needs an external peer the acceptance goldens cannot orchestrate, so it lives as a standing standalone check to run by hand. When you touch `tcp::connect` / connect-timeout code, run `scripts/check-tcp-connect-timeout.sh target/debug/mfb`: it starts a blackhole TCP server (`scripts/net_blackhole_server.py`) and proves a short `timeoutMs` fails with `ErrTimeout` well before the OS default (~75s). Nothing else invokes it, so it only guards the deadline if you run it.
