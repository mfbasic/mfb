...
- Checking topology (plan-107): the front end has TWO diagnostic passes and no separate "syntax checker". `ir::shape` (`src/ir/shape.rs`) walks the concrete post-monomorph HIR for the rules whose evidence lowering ERASES (named-argument binding, EXIT flavors, inline-TRAP boundaries, TESTING assertion arguments, literal spellings, native CONST/FREE facts, imported-package metadata) using lowering's own typing oracle (`ir::lower::expression_type`); every rule there carries a one-line justification naming the erased fact — a rule whose evidence survives lowering belongs in `ir::verify`, which runs on the source-lowered IR AND on decoded package IR. Source diagnostics render shape's stream first, then verify's. `scripts/diag-set-diff.sh <mfb>` proves a relocation preserved every fixture's diagnostic SET (SAME/REORDER/SETDIFF); it only runs fixtures whose golden already carries a diagnostic, so a NEW error on a clean fixture is seen only by `artifact-gate`/`test-accept`.
...
- Some runtime behavior needs an external peer the acceptance goldens cannot orchestrate, so it lives as a standing standalone check to run by hand. When you touch `net::connectTcp` / connect-timeout code, run `scripts/check-net-connect-timeout.sh target/debug/mfb`: it starts a blackhole TCP server (`scripts/net_blackhole_server.py`) and proves a short `timeoutMs` fails with `ErrTimeout` well before the OS default (~75s). Nothing else invokes it, so it only guards the deadline if you run it.
