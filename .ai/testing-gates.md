...
- Byte-identity fixtures `tests/byte-identity/<pkg>/golden/*.ncodesum` (×4 targets) cover **22 builtin packages** (audio, bits, collections, crypto, csv, datetime, encoding, fs, general, http, io, json, math, money, net, os, regex, strings, term, thread, tls, vector) + `rt-behavior/crypto/crypto-ec-valid`. The gate `shasum -a 256`s the built `.ncode` and compares — so a codegen change to any covered builtin flips at least one hash and goes red. All four targets regenerate on the macOS host (`-ncode` is execution-free; no box 2229 needed for sums).
...
- The byte-identity gate that MATTERS is **AArch64 + RISC-V** (`linux-aarch64` / `linux-riscv64`). SysV-x86 (boxes 2227/2228) and Win64 change and are proven by rt-behavior / execution, not bytes.
- **Windows (Win64) byte-identity is an explicit NON-GOAL.** Do not chase it, report it, gate on it, or shape a design "to keep Windows byte-identical" — that reads as fudging.
- NON-GOAL is about BYTES, not CORRECTNESS. Windows must still be EXECUTION-verified on box 2230, or a real regression hides: deleting the x86 fixpoint once made Win64 produce ZERO output (broken arena), undetected because Win64 checks were skipped as "non-goal". Gate for ANY ABI/codegen change: `cargo test` + AArch64/RISC-V byte-identity + SysV-x86 (2227/2228) AND Win64 (2230) EXECUTION.
