...
- **Per-target fixture counts (not uniform):** each of the four Unix targets carries 29 `.ncodesum` goldens; `windows-x86_64` carries **27** — only `link-const-pins` (native lib `c`, no Windows locator) and `rt-behavior/crypto/crypto-ec-valid` have none, so they are not built for Windows (measure: `find tests/byte-identity tests/rt-behavior -name '*.<target>.ncodesum' | wc -l`). `os` and `process` DO carry Windows goldens now; this line once listed them as Windows-unsupported, which went stale when bug-454 made `os.appResourcePath` lower on Windows. Adding a `<pkg>.<target>.ncodesum` under `golden/` is what makes the gate build+sum that target for that fixture — the target list per fixture is derived from the golden filenames (`artifact-gate.sh`), so a missing golden silently drops that target. `thread` gained its `windows-x86_64` golden once the `thread.*` Windows import set declared the stdin-broadcast heap/console symbols (it drives the same stdin log as `io.input`).
...
## The man-example gate (`scripts/man-examples-gate.sh`, CI job `man-examples`)
bug-472. Man prose is `&'static str` the compiler never reads, so neither `cargo test` nor the artifact-gate can see a broken example. This gate lifts every Examples block out of RENDERED `mfb man` output (via `scripts/man-run-examples.sh`, one package at a time), builds it, and runs it. `testing` goes through `--test`: `mfb build` drops TESTING blocks, so build-and-run would be vacuous there. An example that genuinely cannot run on a CI host is listed in `scripts/man-examples-not-run.txt` as `pkg::fn#N  reason`; it is still BUILT, and an entry naming an example that no longer exists fails the gate, so the list cannot rot. A run failure that is a defect is a bug, never a not-run entry. Run locally: `bash scripts/man-examples-gate.sh target/release/mfb` (logs per package under `WORK`). On Linux the runner picks `-glibc.out` and runs a `--app` build's `-glibc.AppImage` with `--appimage-extract-and-run`; `find -perm +111` is BSD-only (GNU find rejects it), use `-perm -u+x`.
...
- **A fixture sets the RUN's environment with `run.env`** (beside `project.json`; `NAME=value` per line, `#`/blank skipped). `test-accept.sh` and `linux-runtime-proof.sh` both apply it, the build never sees it, and the logged `$ <exe>` line is unchanged. It is how `tests/rt-behavior/datetime/*-tz-*` pin `TZ=America/New_York` (bug-520); before it no fixture could test host-zone DST behaviour.
...
|---|---|---|
| An MFBASIC **package** under `packages/` | `packages/<pkg>/oracle/` | `packages/yaml/oracle` (Node, eemeli/yaml pinned 1.2 Core), `packages/mustache/oracle`, `packages/jwt/oracle`, `packages/json_schema/oracle`, `packages/timezones/oracle` (Python `zoneinfo`, `tzdata` pinned to the vendored tzdb release) |
