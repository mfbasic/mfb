#!/usr/bin/env bash
# plan-85-D riscv64 EXECUTION verify: compile each fixture for linux-riscv64, ship the
# ELF to box 2229 (Alpine riscv64), run it, and diff stdout against the golden's
# execution section (between the final `$ .../<pkg>.out` line and its `[exit ...]`).
# usage: p85-riscv-verify.sh <fixture-dir>...
set -u
...
for dir in "$@"; do
  ...
  out=$("$EXE" build -q -target linux-riscv64 "$dir" 2>/dev/null | sed -n 's/^Wrote executable to //p' | grep -- '-musl.out' | head -1)
...
echo "=== p85 riscv64 verify: pass=$pass fail=$fail ==="
