...
//!
//! Both siblings re-export it — `mfb` from `src/terminal_safe.rs` and
//! `mfb_repository` from `repository/src/terminal_safe.rs` — so there is exactly
//! one implementation and one set of tests.
//!
//! It lives here because it is shared, and for no other reason. bug-489
//! originally put it in `mfb_repository` with the explanation that the registry
//! client is one of its callers and `mfb_repository` cannot depend on `mfb` —
//! true at the time, but it made the registry crate the owner of a *terminal
//! sanitizer*, which has nothing to do with a package registry. plan-126-C moved
//! it to `mfb_wire`, the crate that exists precisely to hold what both siblings
//! need, and reversed the shim direction. Do not restore that rationale: it now
//! describes a constraint that no longer binds.
//!
...
pub fn is_terminal_unsafe(ch: char) -> bool {
    ch.is_control()
        || matches!(ch,
            // Arabic letter mark.
            '\u{061C}'
